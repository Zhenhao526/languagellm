"""Fixed sender state-baseline correspondence intervention on both-scaled inputs."""
...
sys.path.insert(0,str(PROJECT/'redesign_v0.16'))
import run_branches as v16
v15=v16.v15
sys.path.insert(0,str(ROOT))
import baseline_interface as baseline
branches=v16.branches
...
select_parameters=v15.select_parameters;update_agents=baseline.update_agents;worlds=v15.worlds;evaluate=v15.evaluate
...
ARMS=['both_sender_baseline_shuffle']

def read(p):return json.loads(Path(p).read_text())
def sources():
    return list(dict.fromkeys([Path(__file__).resolve(),ROOT/'baseline_interface.py',ROOT/'固定执行方案.md',ROOT/'前置科学审查.md',ROOT/'前置审查.md',*v16.sources()]))

def source_receipt(args):
    receipt=v16.source_receipt(args)
    receipt['factorial_references']='Matched both = v15 reset_scaled; other inherited groups are preparation/background, not new factorial arms'
    return receipt
...
def fit(seed,p,arm,prepared,bank,args,hashes):
    ...
    source_inputs=dict(reset_initial=dict(path=str(reset),sha256=v8.sha(reset)),
        ...
        intervention='Both branches use original fixed alpha. Sender policy baseline is shuffled only in the intervention arm; sender value MSE targets and receiver loss formula stay original')
    ...
    cfg=dict(seed=seed,partition=p,arm=arm,updates=args.social_updates,batch=args.batch,eval_n=args.eval_n,
        ...
        sender_baseline_shuffle=arm=='both_sender_baseline_shuffle',baseline_rng_namespace=17017,
        baseline_permutation='PCG64 default_rng(SeedSequence([17017,seed,partition,zero_based_step,person])).permutation(n_sender)',
        baseline_shuffle_scope='sender policy advantage only; value MSE original target; receiver unchanged; random fixed points allowed',
    ...
    with (dest/'training.jsonl').open('w') as log:
        for step in range(args.social_updates+1):
            ...
            traced=step in (0,2100)
            updated=update_agents(agents,opts,learning,weight,seed=seed,partition=p,step=step,
                shuffle=arm=='both_sender_baseline_shuffle',return_trace=traced)
            if traced:
                info,trace=updated
                assert not (set(arr)&set(trace))
                np.savez_compressed(dest/f'train_{step+1:04d}.npz',**arr,**trace)
            else:info=updated
            if step==2100:
...
def main():
    ...
    ap.add_argument('--out',type=Path,default=ROOT/'results/baseline_001')
