...
def independent_metrics(c, logits):
    ...
    c = c / input_mass  # Match the documented roundoff-only probability normalization.
    ...
    result = dict(input_mass=input_mass, positive_message_count=int((mass>0).sum()),
        branch_nll=nf + nw, food_nll=nf, water_nll=nw,
...
def math_selftests(audit, implementation=None):
    first=np.full((1,7),-1000.);first[0,:2]=np.log([.6,.4])
    second=np.full((1,7,7),-1000.);second[0,0,:2]=np.log([.51,.49]);second[0,1,:2]=np.log([.99,.01])
    second[0,2:,0]=0.
    native,hot,sequence=sender_policies(first,second)
    audit.check(sequence.tolist()==[[0,0]] and int(native.argmax())==7 and int(hot.argmax())==0,
                'sequential_greedy_is_not_joint_argmax')
    audit.check(np.isclose(native.sum(),1.) and np.isclose(native[0,7],.396),
                'native_second_symbol_conditions_on_first')
    c = np.zeros((49, 6, 6))
...
def optimizer_check(audit, opt, names, step, label):
    ...
    audit.check((not opt['state']) if step == 0 else len(opt['state']) == len(names) and
                set(opt['state']) == set(range(len(names))) and
                all(float(v['step']) == step and torch.isfinite(v['exp_avg']).all() and
                    torch.isfinite(v['exp_avg_sq']).all() for v in opt['state'].values()),
...
def source_audit(audit, root, source_root, seed, p, direction, detail):
    ...
    prior_cfg = read(source_root / f's{seed}_p{p}_base/config.json')
    audit.check(prior_cfg['seed'] == seed and prior_cfg['partition'] == p and prior_cfg['arm']=='base' and
                prior_cfg['source_checkpoint'] is None and (seed==99510 or prior_cfg['updates']==2400),
                'source_is_unselected_formal_base_endpoint', folder.name)
...
def audit_fit(audit, root, seed, p, direction, arm, args, original, sender, contexts, hashes, full_messages, message_cache):
    ...
    source_folder = root/f'source_s{seed}_p{p}_d{direction}'
    audit.check(Path(cfg['source_folder']).resolve()==source_folder and cfg['source_config_sha256']==sha(source_folder/'config.json'),
                'fit_exact_source_manifest_reference',folder.name)
    audit.check(cfg['trainable_parameters']==sum(param.numel() for param in model.parameters() if param.requires_grad) and
                cfg['behavioral_parameters']==sum(param.numel() for name,param in model.named_parameters() if name.startswith(('receive_embedding.','actor.'))),
                'declared_behavior_and_training_parameter_budgets',folder.name)
    ...
    for step, row in enumerate(rows):
        ...
        expected_loss=(components['policy_loss']+components['value_loss']-components['entropy_weight']*components['entropy']) if arm=='warm_rl24' else components['branch_ce_sample']
        # Logged components are individual float32 scalars; their float64 recombination can differ by a few ulps.
        audit.check(np.isclose(components['loss'],expected_loss,rtol=2e-6,atol=2e-6) and
                    ((isinstance(row['action_sha256'],str) and len(row['action_sha256'])==64) if arm=='warm_rl24' else row['action_sha256'] is None),
                    'all_loss_reduction_and_actual_vs_supervised_action_scope',[folder.name,step])
        ...
        if full_messages:
            identity=(support,step)
            if identity not in message_cache:
                sent = independent_send(sender, torch.from_numpy(context['h'][indices]), np.random.default_rng(streams['send']['seed']))
                message_cache[identity]=digest_arrays(sent.numpy())
                audit.counts['unique_native_message_batches_replayed']+=1
            audit.check(message_cache[identity] == row['message_sha256'], 'all_training_native_messages_independent_replay', [folder.name, step])
        if step in (0, 2100):
...
def flatten_scalars(value, prefix=''):
    ...
    for key,item in value.items():
        ...
        if isinstance(item,dict): output.update(flatten_scalars(item,name))
        elif isinstance(item,(float,int)) and not isinstance(item,bool): output[name]=float(item)
    return output


def compact_metrics(value):
    output={k:v for k,v in value.items() if np.isscalar(v)}
    for key in ('stochastic','greedy','oracle_branch_ce','oracle_joint_map','oracle_mixed_reward'):
        output[key]={k:v for k,v in value[key].items() if np.isscalar(v)}
    return output


def fixed_action_metrics(c, actions):
    actions=np.asarray(actions)
    food=water=joint=0.
    for m,(a,b) in enumerate(actions):
        food+=float(c[m,a,:].sum());water+=float(c[m,:,b].sum());joint+=float(c[m,a,b])
    return dict(J=joint,single_accuracy=(food+water)/2,food_accuracy=food,
                water_accuracy=water,mixed_reward=(food+water)/4+joint/2)


def mean_dict(items):
    assert items and all(x.keys()==items[0].keys() for x in items)
    return {k:math.fsum(x[k] for x in items)/len(items) for k in items[0]}


def run_analysis_audit(root, output):
    """Independent read-only raw probability, oracle witness and seed-summary audit."""
    audit=Audit(); report_file=root/'receiver_analysis.json'; result=read(report_file)
    execution=read(root/'audit_execution.json')
    audit.check(execution['passed'] and execution['counts']['fits']==96 and
                sha(ROOT/'audit_receiver_execution_source.py')==execution['audit_script_sha256'],
                'prior_complete_execution_and_preserved_auditor')
    audit.check(result['status']=='complete' and result['run_count']==96 and result['source_direction_count']==24 and
                result['inherited_training_seeds']==list(SEEDS) and result['primary_endpoint']==2400 and
                result['short_budget']==600,'complete_analysis_units_and_fixed_endpoints')
    audit.check(sha(ROOT/'analyze_receiver.py')==result['analysis_sha256'] and
                sha(ROOT/'receiver_metrics.py')==result['receiver_metrics_sha256'], 'analysis_source_hashes')
    math_selftests(audit)
    lookup={}; reference_truth={}; source_baselines={}
    source_lookup={s['key']:s for s in result['sources']}
    for source in result['sources']:
        name=source['key']; folder=Path(source['path']); p=source['partition']
        audit.check(folder==root/name and sha(folder/'config.json')==source['config_sha256'] and
                    sha(folder/'source.pt')==source['source_state_file_sha256'], 'analysis_exact_verified_source',name)
        groups=partitions(p);groups.update(all=np.arange(30),train24=np.sort(np.r_[groups['old'],groups['added']]))
        raw_file=Path(source['distributions_file']); saved=arrays(raw_file)
        audit.check(sha(raw_file)==source['distributions_sha256'],'analysis_joint_distribution_asset_hash',name)
        ref=result['references'][name]; expected_refs={'by_split':{},'fit_oracle_transfer':{}}
        for split in ('fit','dev','evaluation'):
            context=arrays(folder/f'context_{split}.npz')
            audit.check(sha(folder/f'context_{split}.npz')==source['context_hashes'][split],
                        'analysis_unchanged_verified_context', [name,split])
            native,greedy,_=sender_policies(context['first_logits'],context['second_logits'])
            expected_refs['by_split'][split]={}
            for mode,probability in (('native',native),('greedy',greedy)):
                expected_refs['by_split'][split][mode]={}
                for group in (('old','added','sealed','all','train24') if split=='evaluation' else ('train24','all')):
                    key=f'{split}_{mode}_{group}'
                    truth=contingency(probability,context['positions'],np.isin(context['map_ids'],groups[group]))
                    audit.check(np.allclose(saved[key],truth,atol=2e-13,rtol=2e-12),
                                'independent_world_weighted_joint_distribution',[name,key])
                    # The serialized c is verified against independently reconstructed probabilities.
                    # Recompute argmax on that exact floating table, avoiding artificial near-tie differences.
                    c=saved[key];lookup[name,split,mode,group]=c
                    metric=independent_metrics(c,np.zeros((49,2,6)))
                    expected={k:metric[k] for k in ('joint_conditional_entropy','branch_conditional_entropy',
                        'food_conditional_entropy','water_conditional_entropy','conditional_mutual_information',
                        'message_probability','oracle_branch_ce','oracle_joint_map','oracle_mixed_reward')}
                    compare_numbers(audit,ref['by_split'][split][mode][group],expected,[name,key])
                    expected_refs['by_split'][split][mode][group]=expected
                    audit.counts['independent_joint_tables']+=1
        for support in ('train24','all'):
            expected_refs['fit_oracle_transfer'][support]={}
            for mode in ('native','greedy'):
                expected_refs['fit_oracle_transfer'][support][mode]={}
                for oracle in ('oracle_branch_ce','oracle_joint_map','oracle_mixed_reward'):
                    witness=ref['fit_oracle_transfer'][support][mode][oracle]
                    expected_action=expected_refs['by_split']['fit'][mode][support][oracle]['actions']
                    audit.check(witness['actions']==expected_action,'fit_fixed_shared_oracle_action_table',[name,support,mode,oracle])
                    scores=fixed_action_metrics(lookup[name,'fit',mode,support],expected_action)
                    outcomes={g:fixed_action_metrics(lookup[name,'evaluation',mode,g],expected_action) for g in groups}
                    compare_numbers(audit,witness['fit_scores'],scores,[name,support,mode,oracle,'fit'])
                    for group,score in outcomes.items():
                        compare_numbers(audit,witness['evaluation'][group],score,[name,support,mode,oracle,group])
                    expected_refs['fit_oracle_transfer'][support][mode][oracle]=dict(actions=expected_action,fit_scores=scores,evaluation=outcomes)
                    audit.counts['fixed_fit_oracle_tables_replayed']+=1
        reference_truth[name]=expected_refs
    all_point_metrics={};point_close={};selected_truth={};times=result['times']
    audit.check(times==[0,100,300,600,1200,1800,2400],'all_seven_fixed_analysis_checkpoints')
    for run in result['runs']:
        name=run['source_key'];folder=Path(run['path']);seed,p,d,arm=[run[k] for k in ('seed','partition','direction','arm')]
        cfg=read(folder/'config.json'); source=source_lookup[name]
        audit.check(folder==root/f's{seed}_p{p}_d{d}_{arm}' and (seed,p,d)==(source['seed'],source['partition'],source['direction']),
                    'analysis_run_source_identity',folder.name)
        audit.check(run['sealed_supervised_exposure']==(arm=='fresh_ce30') and run['fit_support']==('all' if arm=='fresh_ce30' else 'train24'),
                    'analysis_explicit_full30_supervised_scope',folder.name)
        for filename,digest in run['file_hashes'].items():
            audit.check(sha(folder/filename)==digest,'analysis_fit_artifact_hash',[folder.name,filename])
        expected_points={};original_curve=read(folder/'curve.json')
        for point in run['curve']:
            t=point['update'];path=folder/f'receiver_{t:04d}.npz';logits=arrays(path)['logits']
            audit.check(sha(path)==point['logits_sha256'],'analysis_unchanged_verified_receiver_logits',[folder.name,t])
            evaluation={mode:{} for mode in ('native','greedy')}
            for mode in evaluation:
                for group in ('old','added','sealed','all','train24'):
                    expected=compact_metrics(independent_metrics(lookup[name,'evaluation',mode,group],logits))
                    compare_numbers(audit,point['evaluation'][mode][group],expected,[folder.name,t,mode,group])
                    evaluation[mode][group]=expected;audit.counts['evaluation_metric_tables']+=1
            metrics=flatten_scalars(evaluation)
            for split in ('fit','dev'):
                expected=compact_metrics(independent_metrics(lookup[name,split,'native',run['fit_support']],logits))
                compare_numbers(audit,point[split],expected,[folder.name,t,split])
                for k in ('branch_nll','branch_conditional_entropy','joint_conditional_entropy','conditional_mutual_information','kl_sum'):
                    metrics[f'{split}.{k}']=expected[k]
            compare_numbers(audit,point['metrics'],metrics,[folder.name,t,'flattened'])
            identity=(seed,p,d,arm,t);all_point_metrics[identity]=metrics
            close=metrics['fit.kl_sum']<=1e-3;point_close[identity]=close
            audit.check(point['fit_close_to_branch_entropy']==close,'only_fit_branch_reference_precision',[folder.name,t])
            expected_points[t]=metrics
            if t==0 and arm=='warm_rl24':
                compare_numbers(audit,source['baseline'],evaluation,[name,'original_baseline'])
                source_baselines[name]=evaluation
        chosen=None if arm=='warm_rl24' else min(original_curve,key=lambda r:(r['scores']['dev']['branch_nll'],r['update']))['update']
        audit.check(run['selected_dev_update']==chosen and
                    (run['selected_dev_point'] is None if chosen is None else run['selected_dev_point']==next(x for x in run['curve'] if x['update']==chosen)),
                    'analysis_selection_only_original_dev_support',folder.name)
        selected_truth[seed,p,d,arm]=chosen
    seed_truth={}
    for cell in result['seed_cells']:
        s,a,t=cell['seed'],cell['arm'],cell['update']
        ids=[(s,p,d,a,t) for p,d in product((1,2,3),(0,1))]
        expected=mean_dict([all_point_metrics[x] for x in ids]);seed_truth[s,a,t]=expected
        compare_numbers(audit,cell['metrics'],expected,[s,a,t,'seed_cell'])
        audit.check(cell['directions_averaged']==6 and cell['fit_close_count']==sum(point_close[x] for x in ids),
                    'six_nested_directions_not_six_independent_seeds',[s,a,t])
    for arm in ARMS:
        for t in times:
            metrics=[seed_truth[s,arm,t] for s in SEEDS];actual=result['aggregate'][arm][str(t)]
            compare_numbers(audit,actual['metrics'],mean_dict(metrics),[arm,t,'macro'])
            compare_numbers(audit,actual['ranges'],{k:[min(x[k] for x in metrics),max(x[k] for x in metrics)] for k in metrics[0]},[arm,t,'range'])
            close=sum(point_close[s,p,d,arm,t] for s,p,d in product(SEEDS,(1,2,3),(0,1)))
            audit.check(actual['seed_count']==4 and actual['fit_direction_count']==24 and actual['fit_close_count']==close,
                        'four_source_macro_units_and_precision_count',[arm,t])
    for left,right in (('warm_ce24','warm_rl24'),('fresh_ce24','warm_ce24')):
        name=f'{left}_minus_{right}'
        for t in times:
            actual=result['comparisons'][name][str(t)]
            expected={k:[seed_truth[s,left,t][k]-seed_truth[s,right,t][k] for s in SEEDS] for k in seed_truth[SEEDS[0],left,t]}
            compare_numbers(audit,actual['seed_differences'],expected,[name,t,'paired_differences'])
            compare_numbers(audit,actual['mean_differences'],{k:math.fsum(v)/4 for k,v in expected.items()},[name,t,'paired_mean'])
            compare_numbers(audit,actual['difference_ranges'],{k:[min(v),max(v)] for k,v in expected.items()},[name,t,'paired_range'])
            audit.check(actual['seeds']==list(SEEDS),'paired_difference_source_order',[name,t])
    selected_cells={}
    for cell in result['selected_dev_summary']['seed_cells']:
        s,a=cell['seed'],cell['arm'];ordered=[selected_truth[s,p,d,a] for p,d in product((1,2,3),(0,1))]
        audit.check(cell['selected_updates']==ordered,'selected_dev_nested_checkpoint_ids',[s,a])
        expected=mean_dict([all_point_metrics[s,p,d,a,selected_truth[s,p,d,a]] for p,d in product((1,2,3),(0,1))])
        compare_numbers(audit,cell['metrics'],expected,[s,a,'selected_mean']);selected_cells[s,a]=expected
    for arm in ARMS[1:]:
        compare_numbers(audit,result['selected_dev_summary']['aggregate'][arm],mean_dict([selected_cells[s,arm] for s in SEEDS]),[arm,'selected_macro'])
    ref_cells={}
    for cell in result['reference_summary']['seed_cells']:
        s=cell['seed'];values=[]
        for source in result['sources']:
            if source['seed']==s:
                values.append(flatten_scalars(dict(baseline=source_baselines[source['key']],**reference_truth[source['key']])))
        expected=mean_dict(values);ref_cells[s]=expected
        compare_numbers(audit,cell['metrics'],expected,[s,'references'])
        audit.check(cell['directions_averaged']==6,'reference_seed_unit',s)
    compare_numbers(audit,result['reference_summary']['aggregate'],mean_dict(list(ref_cells.values())),'reference_macro')
    compare_numbers(audit,result['reference_summary']['ranges'],{k:[min(x[k] for x in ref_cells.values()),max(x[k] for x in ref_cells.values())] for k in ref_cells[SEEDS[0]]},'reference_range')
    receipt=dict(passed=not audit.failures,total_checks=sum(audit.checks.values()),checks=dict(audit.checks),failures=audit.failures,
        counts=dict(audit.counts),analysis_sha256=sha(report_file),analysis_source_sha256=sha(ROOT/'analyze_receiver.py'),
        audit_script_sha256=sha(__file__),execution_auditor_sha256=execution['audit_script_sha256'],
        completed_utc=datetime.now(timezone.utc).isoformat(),
        scope='Saved source probabilities and serialized joint tables independently reconstructed; target-specific metrics, all oracle witnesses and fit-table transfer, all seven checkpoints, dev-only selection and four-source aggregation. No training or model inference in this analysis audit.')
    if output.exists() and not read(output).get('passed',False):
        old=output.with_name(output.stem+'_prior_failure_'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S')+'.json')
        old.write_bytes(output.read_bytes())
    output.write_text(json.dumps(receipt,ensure_ascii=False,indent=2,allow_nan=False))
    print(json.dumps(dict(passed=receipt['passed'],checks=receipt['total_checks'],counts=receipt['counts'],output=str(output))),flush=True)
...
def main():
    ...
    parser.add_argument('--analysis', action='store_true', help='Audit completed analysis JSON without repeating execution replay')
    ...
    if args.analysis:
        assert not args.preflight and not args.skip_full_message_replay
        return run_analysis_audit(root,args.output or root/'audit_analysis_independent.json')
    ...
    expected_source=PROJECT/'redesign_v0.10/results'/('smoke_001' if args.preflight else 'generalization_001')
    audit.check(source==expected_source.resolve(),'fixed_unselected_source_batch')
    if not args.preflight:
        prior=read(source/'audit_execution.json')
        audit.check(prior['passed'] and prior['completed_social_runs']==60 and prior['personal_controls']['completed']==4,
                    'inherited_v10_execution_audit')
        for path,digest in prior['source_hashes'].items():
            audit.check(sha(path)==digest,'inherited_v10_frozen_source_hash',path)
        gate=read(ROOT/'preflight_sources/preflight_qa.json')
        audit.check(gate['passed'] and gate['runner_sha256']==hashes[str(ROOT/'run_receiver.py')] and
                    gate['frozen_source_hashes']==hashes,'formal_uses_passed_exact_preflight_sources')
        audit.check(sha(ROOT/'preflight_sources/audit_receiver.py')==gate['audit_script_sha256'],
                    'preflight_auditor_source_preserved')
    ...
    for seed,p,direction in product(invocation['seeds'],invocation['partitions'],invocation['directions']):
        ...
        for arm in ARMS:
            group[arm] = audit_fit(audit,root,seed,p,direction,arm,invocation,original,sender,contexts,hashes,
                                  not args.skip_full_message_replay,message_cache)
