...
def figures(r,out):
    ...
    def save(fig,name):
        ...
        fig.text(.5,.012,'* CE30: all 30 maps included in supervised fitting.',ha='center',va='bottom',fontsize=8)
    ...
    for ax,key,title,ylabel,mult in [(axes[0],'fit.kl_sum','Fit-support branch NLL gap','Nats/world (symmetric-log scale)',1),
        (axes[1],'native.train24.stochastic.J','Common 24 maps: native sender + stochastic receiver','Expected joint success Q (%)',100)]:
        for arm in ARMS:
    ...
    panels=[('native.train24.branch_nll','Branch NLL','Nats/world',1,'native.train24.branch_conditional_entropy'),
            ('native.train24.stochastic.J','Stochastic receiver joint success Q','Joint success (%)',100,'native.train24.oracle_joint_map.J'),
            ('native.train24.stochastic.mixed_reward','Stochastic receiver mixed reward','Expected reward',1,'native.train24.oracle_mixed_reward.mixed_reward')]
    ...
    axes[0].set_ylabel('Native sender + stochastic receiver: Q (%)')
