...
def main():
    ...
    inv=read(batch/'invocation.json');seeds=inv['args']['seeds'];parts=inv['args']['partitions'];arms=['retained','reset','reset_scaled'];source=Path(inv['args']['source_batch']);reset_batch=Path(inv['args']['reset_batch'])
    updates=inv['args']['social_updates'];assert read(batch/'training_complete.json')['status']=='complete'
    folders=[(source/f'social_s{s}_p{p}_control' if a=='retained' else reset_batch/f'social_s{s}_p{p}_reset' if a=='reset' else batch/f'social_s{s}_p{p}_reset_scaled',a) for s,p,a in product(seeds,parts,arms)]
    ...
    for seed in seeds:
        ...
        paired.append(dict(seed=seed,**means,delta=means['retained']-means['reset_scaled'],remaining=means['retained']-means['reset_scaled'],repair=means['reset_scaled']-means['reset']))
    result=dict(passed=True,combined_social_runs=len(rows),new_social_runs=len(rows)//3,reused_social_runs=2*len(rows)//3,checked_scalar_values=value_checks,rows=rows,primary_by_seed=paired,
        primary_mean_difference=float(np.mean([x['delta'] for x in paired])),repair_mean_difference=float(np.mean([x['repair'] for x in paired])),source_hashes=hashes,contrast='retained minus reset_scaled (remaining gap); repair is reset_scaled minus reset',recount_source_sha256=sha(__file__),
