...
root=Path('/Users/xia/Documents/ChatGPT/语言');batch=root/'redesign_v0.15/results/scaled_001';rec=json.loads((batch/'independent_recount.json').read_text());assert rec['passed'];out={'arms':{},'primary_by_seed':rec['primary_by_seed'],'contrast':'retained minus reset_scaled (remaining); reset_scaled minus reset (repair)'};hashes={str(batch/'independent_recount.json'):hashlib.sha256((batch/'independent_recount.json').read_bytes()).hexdigest()}
for a in ['retained','reset','reset_scaled']:
 ...
 for r in rows:
  base=root/'redesign_v0.13/results/spatial_001' if a=='retained' else root/'redesign_v0.14/results/reset_001' if a=='reset' else batch;arm='control' if a=='retained' else a;p=base/f"social_s{r['seed']}_p{r['partition']}_{arm}/curve.json";c=json.loads(p.read_text());hashes[str(p)]=hashlib.sha256(p.read_bytes()).hexdigest();t=np.array([v['update'] for v in c])
  for g in aucs:
