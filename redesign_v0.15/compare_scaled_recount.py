...
OUT=ROOT/'results/scaled_001'
def sha(p):return hashlib.sha256(Path(p).read_bytes()).hexdigest()
def read(p):return json.loads(Path(p).read_text())
a=read(OUT/'scaled_analysis.json');b=read(OUT/'independent_recount.json')
...
independent={}
for seed,arm in itertools.product(a['seeds'],('retained','reset','reset_scaled')):
...
for name,(left,right) in {'remaining':('retained','reset_scaled'),'repair':('reset_scaled','reset')}.items():
    for k in next(iter(independent.values())):
        deltas=[independent[s,left][k]-independent[s,right][k] for s in a['seeds']]
        target=a['summary']['contrasts'][name]
        for idx,s in enumerate(a['seeds']):check(target['seed_differences'][k][idx],deltas[idx],'paired_differences',str((name,s,k)))
        check(target['mean_differences'][k],np.mean(deltas),'paired_mean',name+k)
        check(target['difference_ranges'][k][0],min(deltas),'paired_range',name+k)
        check(target['difference_ranges'][k][1],max(deltas),'paired_range',name+k)
for x in b['primary_by_seed']:
    for arm in ('retained','reset','reset_scaled'):check(cell_lookup[x['seed'],arm]['metrics']['normal.sealed.N'],x[arm],'root_primary',str((x['seed'],arm)))
    for name in ('remaining','repair'):check(a['summary']['contrasts'][name]['seed_differences']['normal.sealed.N'][a['seeds'].index(x['seed'])],x[name],'root_primary',str((x['seed'],name)))
check(a['summary']['contrasts']['remaining']['mean_differences']['normal.sealed.N'],b['primary_mean_difference'],'root_primary','mean')
check(a['summary']['contrasts']['repair']['mean_differences']['normal.sealed.N'],b['repair_mean_difference'],'root_primary','repair_mean')
report_path=OUT/'report_numbers.json';numbers=read(report_path)
for arm,values in numbers['arms'].items():
    ...
    for mode in ('normal','shuffle','blank','stochastic','erase_memory'):
        for group,value in values[mode].items():
            g='all' if group=='common30' else group
            check(target[f'intervention.{mode}.{g}.joint'],value,'report_numbers',arm+mode+group)
    for group,value in values['auc'].items():check(target[f'auc.{group}.N'],value,'report_numbers',arm+group+'auc')
    for analysis_key,policy,metric in [('native_joint_oracle','native','joint_oracle'),('Q','native','Q'),('G','native','G'),('N','greedy','G'),('joint_entropy','native','joint_entropy'),('greedy_used_codes','greedy','used_messages')]:
        check(target[f'protocol.common30.{analysis_key}'],values['protocol'][policy][metric],'report_numbers',arm+analysis_key)
result=dict(passed=not errors,scalar_checks=sum(counts.values()),checks_by_category=counts,max_absolute_error=max_error,tolerance=1e-9,failures=errors,
    analysis_sha256=sha(OUT/'scaled_analysis.json'),independent_recount_sha256=sha(OUT/'independent_recount.json'),report_numbers_sha256=sha(report_path),comparison_source_sha256=sha(__file__),
    scope='All independent endpoint and protocol scalars; saved learning curves; four source-seed aggregation and both paired endpoint/AUC contrasts; independently assembled report table. No training, new inference or new estimand.')
