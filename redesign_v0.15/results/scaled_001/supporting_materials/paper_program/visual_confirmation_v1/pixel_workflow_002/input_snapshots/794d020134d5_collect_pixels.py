...
QUOTAS = {'apple': 3, 'banana': 3, 'orange': 3, 'water': 10}
LIMIT = 50 * 1024 * 1024
USER_AGENT = 'LanguageEmergenceResearch/0.14 (local academic image curation; serialized requests)'
...
def prepare():
    ...
    status = read(BASE/'pixel_workflow_001/curation_status_20260916.json')
    assert status['counts'] == {'apple': 1, 'banana': 1, 'orange': 1, 'water': 2}
    assert status['workflow_gap'] == 19 and status['usable_for_limited_workflow'] == 5
    chosen = []
    for stratum, count in QUOTAS.items():
        q = queues['reserved_queues']['reserve'][stratum]
        assert len(q) >= count and q == sorted(q, key=lambda r: r['cluster_rank'])
        for ordinal, r in enumerate(q[:count], 1):
            m = metadata[r['id']]
    ...
    assert len(chosen) == len({r['metadata']['cluster_id'] for r in chosen}) == 19
    assert len({r['blind_id'] for r in chosen}) == 19
    ...
    previous = read(BASE/'pixel_workflow_001/download_manifest.json')['records']
    assert len(previous) == 24 and all(r['status'] == 'downloaded' for r in previous)
    ...
    write(OUT/'selection_manifest.json', {'created_utc': now().isoformat(), 'quotas': QUOTAS,
        'planned': 19, 'selection_rule': 'First unused reserve entries per frozen same-stratum queue; global cluster_rank request order',
    ...
    sources = [Path(__file__), OUT/'冻结执行方案.md', OUT/'selection_manifest.json', OUT/'comparison_inputs.json',
        ...
        BASE/'pixel_workflow_001/curation_status_20260916.json',
    ...
    write(OUT/'freeze.json', {'frozen_utc': now().isoformat(), 'before_first_request': True,
        'source_hashes': hashes, 'snapshots': snapshot_rows, 'planned': 19, 'quotas': QUOTAS,
    ...
    write(OUT/'live_status.json', {'status': 'frozen_before_requests', 'planned': 19, 'processed': 0,
        'downloaded': 0, 'model_calls': 0, 'confirmation_requests': 0})
    print(json.dumps({'status': 'frozen', 'planned': 19, 'quotas': QUOTAS,
...
def finalize(records, frozen, terminal_status):
    ...
    for r in inputs['previous_batch']:
        ...
        prior.append({'id': r['blind_id'], 'group': 'workflow_001_all24',
    ...
    for r in new:
        ...
        prior.append({'id': r['blind_id'], 'group': 'workflow_002', 'original_sha1': r['original_sha1'],
                      'processed_sha256': r['processed_sha256'], **r['hashes']})
    assert comparisons == len(new)*114 + len(new)*(len(new)-1)//2
    write(OUT/'near_duplicate_flags.json', {'old_records': 101, 'old_available_pixels': 90,
        'old_unavailable': inputs['old_unavailable'], 'previous_batch_pixels': 24, 'new_pixels': len(new),
    ...
    write(OUT/'download_manifest.json', {'status': terminal_status, 'records': records, 'planned': 19,
    ...
    qa = {'data_integrity_passed': True, 'request_timing_passed': True,
        'processed_candidates': len(records), 'planned_candidates': 19, 'request_count': len(requests),
        'downloaded': len(new), 'ordinary_candidates': len(ordered), 'quarantine': [r['blind_id'] for r in new if not r['ordinary_single_image']],
        'old_available_pixels': 90, 'old_unavailable_pixels': 11, 'previous_batch_pixels': 24,
    ...
    write(OUT/'live_status.json', {'status': terminal_status, 'planned': 19, 'processed': len(records),
...
def collect():
    ...
    for index, row in enumerate(rows, 1):
        ...
        for attempt in (1, 2):
            context = {'pid': os.getpid(), 'planned': 19, 'processed': len(records),
            ...
            if code in (429, 503):
                ...
                print(json.dumps(dict(status='waiting_retry_after', processed=len(records), planned=19,
        ...
        write(OUT/'download_manifest.json', {'status': 'running', 'records': records, 'planned': 19,
            'model_calls': 0, 'confirmation_requests': 0, 'freeze_sha256': sha(OUT/'freeze.json')})
        print(json.dumps({'processed': len(records), 'planned': 19, 'status': record['status']}), flush=True)
        if stop:
            break
