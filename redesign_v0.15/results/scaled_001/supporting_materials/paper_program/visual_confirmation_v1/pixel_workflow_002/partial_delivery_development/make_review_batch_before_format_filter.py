...
def main(mode):
    ...
    if mode == 'early':
        records = manifest['records'][:10]
        assert len(records) == 10
        assert all(r['status'] == 'downloaded' and r['ordinary_single_image'] for r in records)
        dest = HERE/'review_early_round1'
        receipt = HERE/'partial_delivery_001'
        prior_ids = set()
        scope = 'First10 downloaded items in fixed request order, delivery during server cooldown; no content selection. Technical/identity and complete-batch duplicate review pending.'
    else:
