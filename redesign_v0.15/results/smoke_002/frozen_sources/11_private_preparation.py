...
def capability_statistics(logits, world, partition):
    ...
    ties = (np.asarray(logits) == np.asarray(logits).max(-1, keepdims=True)).sum(-1) > 1
...
def run(seed, partition, arm, prepared, bank, out, updates=2400, batch=512, source_hashes=None):
    ...
    cfg = dict(seed=seed, partition=partition, arm=arm, updates=updates, batch_pairs_per_person=batch,
        ...
        evaluation_argmax_tie='first physical site in0..5, raw logits exact-max ties separately reported',
...
def self_test():
    ...
    assert capability_statistics(uniform,world,1)['all']['exact_max_tie_rate']==1
