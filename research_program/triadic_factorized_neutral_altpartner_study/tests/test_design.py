...
from research_program.triadic_factorized_neutral_altpartner_study import design, remap, runner, kernel
...
def test_grid_and_alternative_partner_support():
    ...
    assert design.parse_condition(design.CONDITIONS[0]) == ('static', 'altpair_factorized', 'strict', True)
    assert design.parse_condition(design.CONDITIONS[-1]) == ('rematched', 'altpair_factorized', 'strict', False)
...
def test_factorized_kernel_shapes_and_neutral_head():
    ...
    gradients, row = kernel.training_gradients(networks, arrays['x_PL'][ix], arrays['rewards'][ix], True, uniforms, 1)
