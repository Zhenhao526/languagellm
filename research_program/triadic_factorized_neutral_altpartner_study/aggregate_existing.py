"""Recover a completed four-arm grid from per-run JSON files only."""
...
def main(source, output):
    ...
    result = dict(status='completed_after_json_only_aggregation', source=str(source), runs=runs,
                  ...
                  recovery_note='Read per-run result.json files only; no checkpoints or model outputs were read.')
