"""Train a new worker with one goal combination held out of its support."""
...
def parent_composable_from_analysis(path):
    ...
    for row in rows:
        seed = int(row["seed"])
        value = bool(row["parent_composable"])
        if seed in by_seed and by_seed[seed] != value:
...
def prepare(out, parent_root, parent_analysis):
    ...
    for rel in deps:
        ...
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(ROOT / rel, target)
    analysis_target = out / "parent_analysis.json"
    shutil.copy2(parent_analysis, analysis_target)
    ...
    plan = {
        ...
        "parent_analysis_sha256": sha(analysis_target),
        "sources": src,
        "dependencies": deps,
    ...
    (out / "freeze.json").write_text(json.dumps({"plan_sha256": sha(out / "plan.json"), "prepared_sha256": sha(out / "prepared.json"), "parent_analysis_sha256": sha(analysis_target)}, indent=2) + "\n")
    verify(out)
...
def rollout(params, episode, channel, *, sample, message_mode="natural"):
...
def gradient(params, episode, trajectory, channel, beta):
...
def metrics(params, episode, channel, message_mode="natural"):
    trajectory = rollout(params, episode, channel, sample=False, message_mode=message_mode)
    mask = trajectory["active"]
    returns = trajectory["team_return"][mask]
    return {
        "episodes": int(mask.sum()),
        "team_return_mean": float(returns.mean()) if len(returns) else None,
        "team_return_sd": float(returns.std()) if len(returns) else None,
        "positive_episode_rate": float((returns > 0).mean()) if len(returns) else None,
    }


def evaluate(params, seed, heldout, channel):
    split = {}
    for goal_kind in ("all", "seen", "heldout"):
        episode = design.balanced_eval_stream(seed + design.EVAL_SEED_OFFSET, 4096, goal_kind, heldout)
        split[goal_kind] = {
            "natural": metrics(params, episode, channel, "natural"),
            "permuted": metrics(params, episode, channel, "permuted"),
        }
    return split


def train_one(seed, condition, execution, parent_checkpoint, updates=None):
    support, channel = design.parse_condition(condition)
    ...
    replacement = base_policy.make_policy(seed + 190000, design.FORM)
    ...
    for update in range(1, updates + 1):
        ...
        episode = design.episode_stream(seed, design.BATCH_SIZE, evaluation=False, update=update, support=support, heldout=heldout)
        trajectory = rollout(params, episode, channel, sample=True)
        beta = design.entropy_coefficient(update)
        grad = gradient(params, episode, trajectory, channel, beta)
        ...
        row = {
            ...
            "support": support,
            "channel": channel,
            "heldout_goal": heldout,
            "world_sha256": design.array_sha(episode["site_type"]),
            "goal_sha256": design.array_sha(episode["goal"]),
            "partner_sha256": design.array_sha(episode["partner_id"]),
            "message_uniform_sha256": design.array_sha(episode["message_uniforms"]),
            "action_uniform_sha256": design.array_sha(episode["action_uniforms"]),
            "return_mean": float(trajectory["team_return"][trajectory["active"]].mean()) if trajectory["active"].any() else 0.0,
            "active_count": int(trajectory["active"].sum()),
            "gradient_norm": norm,
            "gradient_clip_scale": scale,
            "parameter_sha256": base_policy.combined_parameter_hash(params),
    ...
    result = {
        ...
        "support": support,
        "channel": channel,
        "heldout_goal": design.heldout_goal(seed),
        "parent_checkpoint": str(parent_checkpoint),
        "parent_checkpoint_sha256": sha(parent_checkpoint),
        "parent_parameter_sha256": parent_parameter_hash,
        "initial_parameter_sha256": initial_parameter_hash,
        "updates": updates,
        "final": evaluate(params, seed, design.heldout_goal(seed), channel),
        "checkpoints": checkpoints,
        "trajectory": rows,
        "training_log_sha256": sha(log),
        "final_parameter_sha256": base_policy.combined_parameter_hash(params),
    }
...
def run_grid(prepared, out, updates=None, seeds=None, conditions=None):
    ...
    for seed in seeds:
        for condition in conditions:
            all_results.append(train_one(seed, condition, execution, parent_path(cfg["parent_root"], seed), updates))
            (execution / "progress.json").write_text(json.dumps({"completed": len(all_results), "total": len(seeds) * len(conditions), "seeds": list(seeds), "conditions": list(conditions)}, indent=2))
    (execution / "results.json").write_text(json.dumps({"results": all_results}, ensure_ascii=False, indent=2) + "\n")
    return all_results
...
if __name__ == "__main__":
    ...
    prepare_parser.add_argument("--parent-root", required=True)
    prepare_parser.add_argument("--parent-analysis", required=True)
    execute_parser = sub.add_parser("execute")
    execute_parser.add_argument("--out", required=True)
    execute_parser.add_argument("--prepared", required=True)
    execute_parser.add_argument("--updates", type=int, default=None)
    execute_parser.add_argument("--seeds")
    execute_parser.add_argument("--conditions")
