...
def draw(data,out):
    ...
    for i,(seed,value,color,marker) in enumerate(zip(SEEDS,differences,SEED_COLORS,MARKERS)):
        ...
        primary_ax.annotate(f'{value:+.2f}',(value,i),xytext=(7 if value>=0 else -7,0),textcoords='offset points',
            va='center',ha='left' if value>=0 else 'right',fontsize=9)
    primary_ax.scatter(mean,4.25,color='#28343D',marker='D',s=85)
    primary_ax.annotate(f'{mean:+.2f}',(mean,4.25),xytext=(7 if mean>=0 else -7,0),textcoords='offset points',
        va='center',ha='left' if mean>=0 else 'right',fontsize=9,fontweight='bold')
