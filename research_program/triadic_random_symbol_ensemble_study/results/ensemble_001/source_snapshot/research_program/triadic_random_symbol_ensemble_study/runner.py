"""Execute the frozen six-permutation symbol-identity ensemble."""
...
FORMATION_PLAN_SHA = dataset.FORMATION_PLAN_SHA
FORMATION_PREPARED_SHA = dataset.FORMATION_PREPARED_SHA
...
def verify_source():
    require(sha(SOURCE / "plan.json") == FORMATION_PLAN_SHA and
            sha(SOURCE / "prepared.json") == FORMATION_PREPARED_SHA,
            "Formation source changed")
    require(read(SOURCE / "freeze.json") == {
        "plan_sha256": FORMATION_PLAN_SHA, "prepared_sha256": FORMATION_PREPARED_SHA},
            "Formation freeze changed")
    require(read(SOURCE / "execution" / "status.json").get("status") == "completed",
            "Formation execution incomplete")
    for seed in SEEDS:
...
def recode_packets(raw, mode):
    raw = np.asarray(raw)
    require(mode in MODES, "Unknown permutation mode")
    require(raw.ndim == 2 and raw.shape[1] == 4 and raw.dtype.kind in "iu" and
            np.all((raw >= 0) & (raw < 8)), "Invalid raw packets")
    permutation = np.asarray(dataset.PERMUTATIONS[mode], dtype=np.int8)
    require(np.array_equal(np.sort(permutation), np.arange(8)), "Permutation is not bijective")
...
def one_time(seed, condition, mode, step, static_maps, features, output, row):
    ...
    natural = pool["messages"][host]
    raw_packets = pool["messages"][donor, 0, senders, :]
    packets = recode_packets(raw_packets, mode)
    if live:
        ...
        payload = dict(
            action_probabilities=probabilities, action_indices=actions, messages=messages,
            ...
            source_packet_endpoint=static_maps[f"{mode}_source_packet_endpoint"],
            group_index=row["group_index"], background_index=row["background_index"],
            host_endpoint=row["host_endpoint"], donor_endpoint=row["donor_endpoint"],
            senders=senders, listeners=row["listeners"],
            candidate_receiver_actions=row["candidate_receiver_actions"],
        ...
        probabilities = pool["action_probabilities"][host]
        actions = pool["action_indices"][host]
        messages = None; path_value = None; data_hash = None; forwards = 0
    measured = metrics.margins(probabilities, row["group_index"], row["background_index"],
                               row["host_endpoint"], row["donor_endpoint"], row["listeners"],
                               row["candidate_receiver_actions"])
    outdir = Path(output) / f"seed_{seed}_{condition}"; outdir.mkdir(parents=True, exist_ok=True)
    record = dict(
        seed=seed, condition=condition, rule=rule, live=live, mode=mode, update=step,
        source_natural_path=str(source), source_natural_sha256=sha(source), path=path_value,
        data_sha256=data_hash, alias_of_source=not live, worlds=len(host), groups=48,
        backgrounds=36, host_donor_cells=16,
        off_diagonal_rows=int(np.sum(row["host_endpoint"] != row["donor_endpoint"])),
        metrics={key: value for key, value in measured.items() if key != "margins"},
        logical_module_samples=6 * len(host), neural_forward_samples=forwards,
        transformation={"mode": mode, "symbol_permutation": list(dataset.PERMUTATIONS[mode]),
                        "position_order": [0, 1, 2, 3]},
        intervention="random symbol bijection on selected donor W1 packet only; live W2 and action recomputed; silent is invisible natural alias")
    write(outdir / f"{mode}_record_{step:04d}.json", record)
    return record
...
def worker(payload):
    ...
    states = content.pack_states(source_prepared["partitions"][TARGET])
    features = intervene.observations(states)
    static_maps = dataset.load_maps(static_path); row = content.flatten_rows(arrays)
    records = []
    for mode in MODES:
        for condition in CONDITIONS:
            for step in STEPS:
...
def budget():
    rows = 48 * 36 * 16
    records = len(SEEDS) * len(CONDITIONS) * len(MODES) * len(STEPS)
    live = len(SEEDS) * 2 * len(MODES) * len(STEPS)
...
def verify(out):
    ...
    require(plan["config"]["primary"] == "same_endpoint_live_M_minus_permuted_live_M_ensemble_and_by_mode",
            "Ensemble primary config")
    verify_source()
    return plan, prepared, maps


def execute(out):
    out = Path(out).resolve(); plan, prepared, static_maps = verify(out)
    execution = out / "execution"
    require(not execution.exists(), "Never overwrite ensemble execution")
    execution.mkdir()
    started = time.perf_counter()
    write(execution / "started.json", dict(at=time.time(), plan_sha256=sha(out / "plan.json")))
    ...
    require(len(records) == budget()["records"], "Ensemble record count")
    measured = dict(
        records=len(records), live_probe_files=sum(r["path"] is not None for r in records),
        silent_alias_records=sum(r["alias_of_source"] for r in records), rows_per_record=48 * 36 * 16,
    ...
    require(measured == budget(), "Ensemble budget mismatch")
    primary = metrics.primary(records, baseline_values())
    result = dict(status="completed", completed_at=time.time(),
                  elapsed_seconds=time.perf_counter() - started,
                  plan_sha256=sha(out / "plan.json"), budget=budget(), measured_budget=measured,
                  records=records, primary=primary)
    ...
    return {"status": result["status"], "elapsed_seconds": result["elapsed_seconds"],
...
if __name__ == "__main__":
    ...
    value = dataset.prepare(args.out) if args.command == "prepare" else ({"status": "verified", "plan_sha256": sha(Path(args.out).resolve() / "plan.json")} if args.command == "verify" else execute(args.out))
