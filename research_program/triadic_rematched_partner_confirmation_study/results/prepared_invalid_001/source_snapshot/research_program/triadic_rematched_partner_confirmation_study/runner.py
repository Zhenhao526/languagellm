...
from research_program.triadic_factorial_payoff_study import runner as previous
...
def sources():
    ...
    paths += [Path(previous.__file__), Path(previous.core.__file__), Path(previous.dataset.__file__),
              Path(previous.environment.__file__), Path(previous.kernel.__file__), Path(previous.cases.__file__),
              Path(previous.factor_cases.__file__)]
...
def train_run(seed, condition, static, arrays, execution):
    ...
    def checkpoint(step):
        ...
        checkpoint_sha = previous.save_checkpoint(checkpoint_path, networks, optimizer, step, world_rng, message_rngs)
