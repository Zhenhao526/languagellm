"""Seed-level contrasts and route-mismatch summaries for the shared-routing study."""
...
def analyse(payload):
    ...
    for population, visibility, support in conditions:
        for left, right in (("tied_routed", "routed"), ("tied_routed", "holistic"),
    ...
    return {"schema": "shared_routing_seed_analysis_v1", "seed_rows": rows,
            "condition_summary": summaries, "contrasts": contrasts,
            "run_counts": {"children": len(children), "seeds": len(seeds)}}


def markdown(data):
    lines = ["# Shared-routing seed analysis", "", f"Children: {data['run_counts']['children']}; seeds: {data['run_counts']['seeds']}.", "", "## Key aligned/hidden held-out-combination cells", ""]
