...
def rollout(communities, fresh, episode, architecture, population, visibility, *, channel="live", message_mode="natural", sample=True, message_override=None):
    ...
    sender_fresh = np.zeros(n, dtype=bool) if parent else np.asarray(episode["fresh_role"], dtype=np.int8) == 0
    receiver_fresh = np.zeros(n, dtype=bool) if parent else ~sender_fresh
    ...
    if parent:
        ...
        if np.any(receiver_fresh):
            idx = np.flatnonzero(receiver_fresh); rp, ctx = policy.receiver_arrays(fresh, message_state[idx], np.asarray(episode["scene_meanings"])[idx], partner_id[idx], visibility); receiver_probs[idx] = rp; receiver_context[idx] = ctx
