...
本归档对应源代码提交 `ec3b9e19bb262b3b84eb7e64b3d22aa43e44c9ab`，远端仓库为 `git@github.com:Zhenhao526/language.git`。任务让 sender 只观察目标对象的两个三值属性，receiver 观察四个候选对象，并通过四符号、双槽位消息选择目标。parent 在两个角色间轮换；child 通过四个伙伴进行 `alternating` 或 `sender_only` 适应。aligned 与 conflict 的唯一区别是 community 1 的外部 token 双交换。
...
完整 `results.json` 保留在远端；本地只保留聚合统计、冻结方案和审计收据。原始逐步日志、checkpoint 和本地完整 payload 已在哈希核验后删除，审计和清理收据记录了其完整性。
