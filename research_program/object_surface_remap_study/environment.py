"""Two-object staged scenes with a child-only surface-label permutation."""
...
def rollout(params, episode, form, task, representation, channel, *, message_mode="natural", sample=True, partner_filter=None, message_override=None):
    design.require(form in design.FORMS and task in design.TASKS and representation in design.REPRESENTATIONS and channel in design.CHANNELS and message_mode in ("natural", "closed", "permuted"), "bad rollout factors")
    if representation == "slot_local":
        design.require(form == "dual2", "slot-local representation is only defined for dual2")
    n = len(episode["goal"])
    ...
    alphabet = design.alphabet_size(form)
    ...
    for slot in range(length):
        sp = policy.softmax(params["sender_logits"][context, slot])
        message_probs[slot] = sp
        if message_override is None and sample:
            selected[:, slot] = policy.sample(sp, episode["message_uniforms"][:, slot])
        elif message_override is None:
            selected[:, slot] = sp.argmax(axis=-1)
        else:
    ...
    messages = _schedule_messages(delivered, form)
    ...
    for t in range(design.HORIZON):
        ...
        if np.any(seen):
            if representation == "slot_local":
                ...
                if np.any(later):
                    history[later] = 1 + (history[later] - 1) * alphabet + incoming[later]
            received[seen] += 1
        state = slot_memory[:, stage] if representation == "slot_local" else history
...
def recombination_override(params, episode, form, task, *, basis="raw"):
    if form != "dual2":
        return None
    design.require(basis in ("raw", "task"), "unknown recombination basis")
    out = np.zeros((len(episode["goal"]), 2), dtype=np.int64)
    for i, goal in enumerate(episode["goal"]):
        target = np.asarray(goal, dtype=np.int8) if basis == "raw" else design.target_bits(np.asarray(goal)[None, :], task)[0]
        donor0 = np.asarray((int(target[0]), 0), dtype=np.int8)
        donor1 = np.asarray((0, int(target[1])), dtype=np.int8)
        out[i, 0] = deterministic_sequence(params, form, donor0)[0]
        out[i, 1] = deterministic_sequence(params, form, donor1)[1]
    return out
