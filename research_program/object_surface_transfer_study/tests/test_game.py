...
from research_program.object_surface_transfer_study import design, environment, policy


def main():
    assert design.parse_child_condition("incumbent_swap_live") == ("incumbent", "swap", "live")
    assert design.parse_child_condition("fresh_identity_silent") == ("fresh", "identity", "silent")
    assert design.alphabet_size("dual2") ** design.message_length("dual2") == 4
    assert design.state_count("dual2", "joint_history") == 5
    assert design.surface_permutation("identity").tolist() == [0, 1]
    assert design.surface_permutation("swap").tolist() == [1, 0]

    episode = design.episode_stream(design.SEEDS[0], 32, task="factorized", mapping="swap", update=1)
    ...
    params = policy.make_policy(4, "dual2", "joint_history")
    tr = environment.rollout(params, eval_ep, "dual2", "factorized", "joint_history", "live", sample=False)
    ...
    print("object_surface_transfer_study tests passed")
