"""Train, evaluate, freeze and replay the incumbent-transfer intervention."""
...
def prepare(out):
    ...
    (out / "freeze.json").write_text(
        json.dumps(
            {"plan_sha256": sha(out / "plan.json"), "prepared_sha256": sha(out / "prepared.json")},
            indent=2,
        )
        + "\n"
    )
...
def verify(out):
    ...
    design.require(
        sha(out / "prepared.json") == freeze["prepared_sha256"] == plan["prepared_sha256"],
...
def gradient(params, episode, trajectory, form, representation, channel, *, child=False):
    """REINFORCE gradient; child arms update only the target worker."""
...
def _metric(trajectory):
    ...
    return {
        ...
        "oracle_team_return_mean": float(environment.oracle_team_return()),
    }
...
def _eval_episode(seed, task, goal_kind, heldout, mapping):
    return design.balanced_eval_stream(
        seed + design.EVAL_SEED_OFFSET,
        design.WORKERS * 4096,
        goal_kind,
        task,
        heldout,
    ...
    for goal_kind in ("all", "seen", "heldout"):
        ...
        if channel == "live":
            ...
            raw = environment.rollout(
                ...
                channel,
                sample=False,
                message_override=environment.recombination_override(params, ep, form, task, basis="raw"),
                partner_filter=design.TARGET_WORKER,
            )
...
def save_checkpoint(path, params, update, form, representation):
    np.savez_compressed(
        path,
        update=np.array(update, dtype=np.int64),
        sender_logits=params["sender_logits"],
        worker_logits=params["worker_logits"],
        form=np.array(form),
...
def load_checkpoint(path):
    with np.load(path, allow_pickle=False) as data:
        return {
            "sender_logits": np.asarray(data["sender_logits"]).copy(),
            "worker_logits": np.asarray(data["worker_logits"]).copy(),
        }


def _stream_hashes(ep):
    return {
        "site_semantic_sha256": design.array_sha(ep["site_semantic"]),
        "site_surface_sha256": design.array_sha(ep["site_surface"]),
        "goal_sha256": design.array_sha(ep["goal"]),
...
def _update_params(params, grad, child):
    norm = float(np.sqrt(sum(float((value * value).sum()) for value in grad.values())))
    scale = min(1.0, 5.0 / max(norm, 1e-12))
    if child:
        params["worker_logits"][design.TARGET_WORKER] -= (
            design.LEARNING_RATE * scale * grad["worker_logits"][design.TARGET_WORKER]
        )
    else:
        for key in params:
...
def train_parent(seed, form, task, execution, updates=None):
    ...
    for update in range(1, updates + 1):
        ...
        norm, scale = _update_params(params, grad, child=False)
        row = {
            "update": update,
            "seed": seed,
            "form": form,
        ...
        if update in checkpoints:
            row["checkpoint_sha256"] = save_checkpoint(
                run / f"checkpoint_{update:04d}.npz", params, update, form, "joint_history"
            )
        rows.append(row)
    ...
    result = {
        ...
        "final": evaluate(params, seed, form, task, "joint_history", "live", heldout, "identity"),
...
def child_initial_params(seed, initialization, parent):
    design.require(initialization in design.INITIALIZATIONS, "bad initialization")
    params = policy.make_policy(seed + 191000, "dual2", "joint_history", sender_logits=parent["sender_logits"])
    if initialization == "incumbent":
        params["worker_logits"][design.TARGET_WORKER] = parent["worker_logits"][design.TARGET_WORKER].copy()
    return params
...
def train_child(seed, condition, execution, parent_checkpoint, updates=None):
    ...
    params = child_initial_params(seed, initialization, parent)
    parent_hash = policy.parameter_hash(parent)
    initial_hash = policy.parameter_hash(params)
    heldout = design.heldout_goal(seed)
    initial_eval = evaluate(params, seed, "dual2", "factorized", "joint_history", channel, heldout, mapping)
    checkpoints = sorted(set([u for u in design.CHECKPOINTS if u <= updates] + [updates]))
    if 0 in checkpoints:
        save_checkpoint(run / "checkpoint_0000.npz", params, 0, "dual2", "joint_history")
    rows = []
    start = time.perf_counter()
    for update in range(1, updates + 1):
        ...
        grad = gradient(params, ep, tr, "dual2", "joint_history", channel, child=True)
        norm, scale = _update_params(params, grad, child=True)
        active = tr["active"]
        row = {
            "update": update,
            "seed": seed,
            ...
            "heldout_goal": heldout,
            **_stream_hashes(ep),
            "return_mean": float(tr["team_return"][active].mean()) if active.any() else 0.0,
        ...
        if update in checkpoints:
            row["checkpoint_sha256"] = save_checkpoint(
                run / f"checkpoint_{update:04d}.npz", params, update, "dual2", "joint_history"
            )
        rows.append(row)
    log = run / "training.jsonl"
    log.write_bytes(b"".join(json_bytes(row) for row in rows))
    result = {
        ...
        "parent_parameter_sha256": parent_hash,
        "initial_parameter_sha256": initial_hash,
        "updates": updates,
        "initial": initial_eval,
        "final_parameter_sha256": policy.parameter_hash(params),
        "final": evaluate(params, seed, "dual2", "factorized", "joint_history", channel, heldout, mapping),
        "checkpoints": checkpoints,
        "training_log_sha256": sha(log),
        "final_checkpoint": str(run / f"checkpoint_{updates:04d}.npz"),
        "final_checkpoint_sha256": sha(run / f"checkpoint_{updates:04d}.npz"),
...
def run_grid(prepared, out, updates=None, seeds=None, conditions=None):
    ...
    parent_results = []
    for seed in seeds:
        parent_results.append(train_parent(seed, "dual2", "factorized", execution, updates))
    child_results = []
    for seed in seeds:
        for condition in conditions:
            child_results.append(
                train_child(
                    ...
                    condition,
                    execution,
                    parent_path(execution, seed, "dual2", "factorized", updates),
                    updates,
                )
            )
            (execution / "progress.json").write_text(
                json.dumps(
                    {
                        "completed_children": len(child_results),
                        "total_children": len(seeds) * len(conditions),
                        "seeds": list(seeds),
