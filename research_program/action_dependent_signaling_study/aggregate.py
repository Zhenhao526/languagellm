"""Aggregate action-dependent signaling runs into compact causal summaries."""
...
def write_md(path,data):
    lines=["# Action-dependent signaling compact aggregation","",f"- runs: {data['runs']}","- split: heldout","","| condition | natural | silent | Δ natural−silent | Δ natural−permuted | recombined−natural | sequence agreement |","|---|---:|---:|---:|---:|---:|---:|"]
...
def main():
    ...
    data={"schema":"action_dependent_signaling_aggregate_v1","runs":len(by),"rows":rows,"effects":effects(by),"codebook":codebook(by)}
