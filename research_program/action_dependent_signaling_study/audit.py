"""Independent replay audit for action-dependent signaling."""
...
def audit(prepared, execution):
    ...
    for path in files:
        ...
        p = load_policy(cp); form, pm, pv, ch, task, protocol = design.parse_condition(r["condition"])
        for evaluation, split in ((False, "training_support"), (True, "heldout")):
            fresh = runner.evaluate_all(p, r["seed"], form, pm, pv, task, ch, protocol, evaluation=evaluation)
    ...
    return {"schema": "action_dependent_signaling_audit_v1", "status": "passed", "runs": len(files), "training_log_rows": logs, "final_checkpoints": checkpoints, "max_abs_replay_error": max_error, "live_silent_trajectory_pairs": pair_checks, "rows": rows}
