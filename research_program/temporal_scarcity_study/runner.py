"""Train and audit the multi-round private-resource communication game."""
...
def prepare(out):
    ...
    plan={'schema':'temporal_scarcity_communication_v1','prepared_sha256':sha(out/'prepared.json'),
...
def episode(seed,scarcity,update,B):
    return design.episode_stream(seed,scarcity,B,update=update)

def evaluate(nets,seed,scarcity,memory,information,channel,*,message_mode='natural',episodes_count=4096,evaluation=True):
    ep=design.episode_stream(seed,scarcity,episodes_count,evaluation=evaluation)
    ...
    result={'episodes':episodes_count,'split':'heldout' if evaluation else 'training_support',
            ...
            'message_mode':message_mode,'message_entropy':float(_entropy(messages[:,:2,:],design.ALPHABET_SIZE)),
            'message_type_mi':float(_message_mi(messages,ep)),
            'messages_per_episode':int(len(design.MESSAGE_ROUNDS)*2),'action_histogram':np.bincount(actions.ravel(),minlength=design.ACTION_COUNT).tolist(),
            'message_histogram':np.bincount(messages[:,:2,:].ravel(),minlength=design.ALPHABET_SIZE+1).tolist(),
...
def _message_mi(messages,ep):
    ...
    for a in design.AGENTS:
      for t in design.MESSAGE_ROUNDS:
       m=messages[:,t,a]
       for state in (ep['goal'][:,a,t], ep['site_type'][:,a]):
...
def train_one(seed,condition,out,updates=None):
    memory,scarcity,information,channel=design.parse_condition(condition); updates=design.UPDATES if updates is None else updates
    ...
    for update in range(1,updates+1):
        ep=episode(seed,scarcity,update,design.BATCH_SIZE)
        ...
        row={'update':update,'seed':seed,'condition':condition,'memory':memory,'scarcity':scarcity,'information':information,'channel':channel,
    ...
    for evaluation, label in ((False,'training_support'),(True,'heldout')):
        natural=evaluate(nets,seed,scarcity,memory,information,channel,
                         message_mode='natural',evaluation=evaluation)
        if channel=='live':
            closed=evaluate(nets,seed,scarcity,memory,information,channel,
                            message_mode='closed',evaluation=evaluation)
            permuted=evaluate(nets,seed,scarcity,memory,information,channel,
    ...
    result={'seed':seed,'condition':condition,'memory':memory,'scarcity':scarcity,'information':information,'channel':channel,'updates':updates,
