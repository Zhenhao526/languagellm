...
def test_design_and_pairing():
    ...
    a = design.episode_stream(68101, "scarce", 12, update=3)
    b = design.episode_stream(68101, "abundant", 12, update=3)
    for key in ("site_type", "goal", "message_uniforms", "action_uniforms"):
        np.testing.assert_array_equal(a[key], b[key])
    assert np.all(a["capacity"] == 1) and np.all(b["capacity"] == 3)
    held = design.episode_stream(68101, "scarce", 12, evaluation=True)
...
def test_observation_masks():
    ...
    assert np.all(pi[13:18] == 0) and pi[18] == 0
    assert fi[14] == 1 and fi[16] == 1 and fi[17] == 1 and fi[18] == 1
...
def test_protocol_and_oracle():
    ep = design.episode_stream(68101, "scarce", 16, update=2)
...
def test_gradient_is_finite_and_updates():
    ep = design.episode_stream(68101, "scarce", 8, update=1)
    nets = model.make_networks(68101, "recurrent")
...
def main():
    for fn in (test_design_and_pairing, test_observation_masks,
        ...
        fn()
    print("temporal_scarcity_study tests passed")
