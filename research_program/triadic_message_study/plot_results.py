"""Fixed scientific figures from the complete read-only message summary.
...
def render(summary,output):
    ...
    def save(fig,name):
        ...
        plt.close(fig)
    try:
        fig,axes=plt.subplots(2,2,figsize=(12.6,8.6),layout='constrained')
        ...
        fig.supxlabel('全部使用更新6000的完整四格；4个配对团队初始化、同一开发划分，无显著性检验。',fontsize=9)
        ...
        fig.supxlabel('细线：各种子；粗线：四种子均值。仅0／100／500／1500／3000／6000六点；连线不定位形成时刻。\n'
                      '监测子集与完整终点分母不同，不能把更新6000的监测值替代完整评价。',fontsize=9)
