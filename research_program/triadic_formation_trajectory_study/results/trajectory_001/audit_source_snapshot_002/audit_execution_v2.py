...
def numeric_tree(expected,actual,counts,path=''):
    ...
    if isinstance(expected,dict):
        ...
        # bool is a Python int subclass; NumPy boolean subtraction is invalid.
        # Selection-change flags require exact type/value, not numeric tolerance.
        require(isinstance(actual,(bool,np.bool_)) and bool(expected)==bool(actual),'Summary boolean '+path)
        counts['scalars']+=1
...
def validate_pool_metadata(pool_meta,independent_specs,original_specs):
    """Numeric domain independently reconstructed; all annotation bytes source-bound."""
    for key,part in (('training_spec','train'),('validation_spec',PART)):
        actual=pool_meta[key];expected=independent_specs[part]
        require(actual==original_specs[part],'Complete source pool metadata '+part)
        require(set(expected)<=set(actual),'Missing independent domain fields '+part)
        require(all(actual[k]==v for k,v in expected.items()),'Independent pool semantic domain '+part)
        require(actual['partition']==part,'Pool partition label '+part)
...
def audit(run,out,expected_plan_sha):
    ...
    original_prepared=original_context/'prepared.json'
    bind(original_prepared,context_proof['artifacts_sha256'][str(original_prepared)])
    original_specs=read(original_prepared)['partitions']
    validate_pool_metadata(pool_meta,specs,original_specs)
    ...
    result=dict(status='passed',plan_sha256=expected_plan_sha,audit_source_sha256=sha(__file__),runtime=dict(python=platform.python_version(),numpy=np.__version__),
        ...
        metadata_comparison='Full pool metadata equals SHA-bound original prepared source; all eight independently reconstructed domain fields exact; descriptive annotations are provenance checked.',
