...
def summarize(payload):
    ...
    cells = {}
    for row in rows:
        key = (row["architecture"], row["support"])
        final = row["final"]
    ...
    out = {}
    for (architecture, support), cell in sorted(cells.items()):
        endpoint = {}
        for name in ("old_seen", "old_combo", "new_single", "new_double", "new_double_recombined"):
            values = cell[name]
            endpoint[name] = {"mean": float(np.mean(values)), "ci95": _ci(values),
                              "functional": _functional(values), "seeds": len(values),
                              "values": values}
        out[f"{architecture}/{support}"] = {
            "architecture": architecture, "support": support, "endpoint": endpoint,
            "incumbent_hash_count": len(set(cell["incumbent"])),
        }
    contrasts = {}
    for architecture in design.ARCHITECTURES:
        def vals(support, name):
            return np.asarray(out[f"{architecture}/{support}"]["endpoint"][name]["values"], dtype=np.float64)
        ...
        for support in ("expanded_single_sender", "expanded_single_receiver", "expanded_single_none"):
            ...
            contrasts[f"{architecture}:pair-minus-{support}:new_double"] = {
                "mean": float(diff.mean()), "ci95": _ci(diff), "values": diff.tolist()
            }
    return {"schema": "open_world_cultural_transmission_transfer_analysis_v1",
