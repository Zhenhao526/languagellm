...
def audit(prepared, execution):
    ...
    def get(seed, architecture, support):
        return logs.get((int(seed), f"{architecture}_aligned_hidden_{support}"))

    stream_fields = ["scene_sha256", "goal_sha256", "partner_sha256",
    ...
    for seed in design.SEEDS:
        for architecture in design.ARCHITECTURES:
            ...
            for left, right in combinations(supports, 2):
                paired_rows += _pair(get(seed, architecture, left),
                                     get(seed, architecture, right), stream_fields)

    expected = len(design.SEEDS) * len(design.TRANSFER_CONDITIONS)
    complete = len(payload["transfers"]) == expected
    return {
        "schema": "open_world_cultural_transmission_transfer_audit_v1",
