"""Frozen open-world cultural-transmission design.
...
extension and replaces one role at a time. The double-new meaning is absent
from every training support, so transfer is tested as zero-shot composition
after cultural learning rather than as inheritance of parameters.
"""
...
def parse_transfer_condition(condition):
    ...
    parts = condition[len(architecture) + 1:].split("_", 2)
    require(len(parts) == 3, f"bad transfer condition {condition}")
    population, visibility, support = parts
...
def balanced_eval_stream(seed, count, goal_kind, support, combo):
    ...
    require(support in ALL_SUPPORTS and count > 0, "bad eval factors")
    rng = np.random.default_rng(np.random.SeedSequence([
...
def prepare():
    return {
        "schema": "open_world_cultural_transmission_study_v1",
        "task": TASK,
