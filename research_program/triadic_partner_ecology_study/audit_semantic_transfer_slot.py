"""Independent replay audit for the confirmation slot-transfer probe.
...
def audit_policy(payload):
    ...
    checkpoint = condition_dir / "checkpoint_6000.npz"; source_result = condition_dir / "result.json"; data_path = condition_dir / f"final_{PARTITION}_natural.npz"
    ...
    n = int(spec["world_count"]); require(saved["states"].shape[0] == n and saved["messages"].shape == (n, 2, 3, 4), "Natural data shape mismatch")
    require(saved["action_probabilities"].shape == (n, 3, 17), "Natural action shape mismatch")
    first_all = saved["messages"][:, 0]; require(array_sha(first_all) == policy["target_first_messages_sha256"], "First-message hash mismatch")
    require(len(policy["policy_rows"]) == len(cases) * len(MASKS), "Policy row count")
    expected_order = [(case["case_id"], mask_name) for case in cases for mask_name, _ in MASKS]
    require([(row["case_id"], row["mask"]) for row in policy["policy_rows"]] == expected_order, "Case/mask order mismatch")
    need_lookup = {tuple(need): index for index, need in enumerate(spec["needs"])}; nphysical = len(spec["layouts"]) * len(spec["private_sites"])
    live = condition.endswith("_live"); networks = core.load_networks(checkpoint) if live else None; checked = live_rows = aliases = evaluations = forwards = 0; route_checks = 0
    for case_index, case in enumerate(cases):
        source_index = need_lookup[tuple(case["source_needs"])]
        target_index = need_lookup[tuple(case["target_needs"])]
        backgrounds = np.arange(nphysical, dtype=np.int64); source_ids = source_index * nphysical + backgrounds; target_ids = target_index * nphysical + backgrounds
        require(np.array_equal(saved["states"][source_ids, 3:], saved["states"][target_ids, 3:]), "Background mismatch")
        target_first = first_all[target_ids]; source_sender = first_all[source_ids, case["changed_person"]]
        natural = saved["action_probabilities"][target_ids, case["listener"]]
        for mask_name, slots in MASKS:
            row = policy["policy_rows"][case_index * len(MASKS) + next(i for i, item in enumerate(MASKS) if item[0] == mask_name)]
            require(row["case_id"] == case["case_id"] and row["mask"] == mask_name and row["replaced_slots"] == list(slots), "Mask metadata mismatch")
            require(array_sha(target_ids) == row["target_indices_sha256"] and array_sha(source_ids) == row["source_indices_sha256"], "Index hash mismatch")
            require(array_sha(target_first) == row["target_first_messages_sha256"] and array_sha(source_sender) == row["source_sender_messages_sha256"], "Message hash mismatch")
            mixed = target_first[:, case["changed_person"], :].copy()
            if slots:
                mixed[:, slots] = source_sender[:, slots]
            require(array_sha(mixed) == row["mixed_sender_messages_sha256"], "Mixed message hash mismatch")
            if live and slots:
                all_intervened = action_probabilities_after_transfer(networks, arrays["x_PI"][target_ids], target_first, mixed, case["changed_person"])
                intervened = all_intervened[:, case["listener"]]; require(row["alias_of_natural"] is False and row["model_forward_samples"] == 6 * nphysical, "Live metadata mismatch")
                live_rows += 1; evaluations += 1; forwards += 6 * nphysical
            else:
                intervened = natural.copy(); require(row["alias_of_natural"] is True and row["model_forward_samples"] == 0, "Alias metadata mismatch"); aliases += 1
            values = case_metrics(intervened, natural, case["source_action_set"], case["target_action_set"])
            compare(values, {key: row[key] for key in ("natural", "intervened", "delta", "source_pull", "greedy_source_pull", "worlds")}, f"{seed}/{condition}/{case['case_id']}/{mask_name}/")
            if not live or not slots:
                compare(row["natural"], row["intervened"], "alias/")
            checked += 1
    sample = np.arange(3 * 4, dtype=np.int8).reshape(1, 3, 4) % 8; source_tokens = np.full((1, 4), 7, dtype=np.int8); own = route_transfer(sample, source_tokens, 1, False); live_route = route_transfer(sample, source_tokens, 1, True); live_natural = route_transfer(sample, sample[:, 1], 1, True)
    require(np.array_equal(own, route_transfer(sample, source_tokens, 1, False)), "Silent route nondeterminism")
    require(np.array_equal(live_route[:, 1, 32:64], own[:, 1, 32:64]), "Sender self-view changed")
    require(np.array_equal(live_route[:, :, 96:99], live_natural[:, :, 96:99]), "Viewer bits changed unexpectedly"); route_checks += 3
    return dict(rows=checked, live_rows=live_rows, alias_rows=aliases, evaluations=evaluations, forwards=forwards, max_error=0.0, route_checks=route_checks)
...
def main(source, probe, output, workers=4):
    ...
    require(saved.get("status") == "completed_json_only_semantic_transfer_slot_probe", "Invalid slot-transfer status")
    require(saved.get("policy_blocks") == 8 and saved.get("masks") == len(MASKS) and saved.get("intervention_rows") == expected_rows, "Invalid slot-transfer grid")
    require(saved.get("live_rows") == 960 * (len(MASKS) - 1) and saved.get("alias_rows") == 960 * (len(MASKS) + 1), "Invalid live/alias counts")
    ...
    require(len(saved["policies"]) == 8 and all(len(policy["policy_rows"]) == 240 * len(MASKS) for policy in saved["policies"]), "Expected eight slot policy blocks")
    ...
    verification = dict(
        ...
        masks=len(MASKS),
        independent_replay="Duplicate candidate filtering, slot-subset message routing, W2/action rollout and directional metric accumulation; slot probe implementation not imported",
    )
