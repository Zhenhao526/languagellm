"""Independent replay audit for the independent confirmation transfer probe.
...
def main(source, probe, output, workers=4):
    ...
    require(saved.get("status") == "completed_json_only_semantic_transfer_confirmation_probe", "Invalid confirmation semantic-transfer status")
    ...
    verification = dict(
        ...
        independent_replay="Duplicate candidate filtering, route, W2/action rollout and directional metric accumulation; confirmation probe implementation not imported",
    )
