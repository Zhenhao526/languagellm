"""Train the paired strict/reciprocal communication experiment."""
...
CONFIG = dict(
    ...
    rules=list(RULES), information=list(INFORMATIONS), schedules=list(SCHEDULES),
    ...
    settlement_rules=('strict: matching pair plus neutral third; reciprocal: matching pair with third ignored'),
    ...
    primary='rule_by_routed_communication_interaction_on_target_pair_legal_rate_conditional_on_physical_execution_centered_time_AUC',
    selection='fixed8 new seeds ×2 rules ×2 information ×2 schedules ×2 channels ×6000 updates; no early stopping',
...
def evaluate_factorized(networks, arrays, spec, information, live, rule):
    ...
    for start in range(0, n, CONFIG['evaluation_batch_size']):
        ...
        settled = execution_env.settle(states, actions, rule)
...
def _budget():
    ...
    return dict(
        runs=runs, independent_seed_blocks=len(SEEDS), rules=2, information_conditions=2,
