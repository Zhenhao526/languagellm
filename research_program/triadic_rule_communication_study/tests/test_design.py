...
from research_program.triadic_rule_communication_study import design, kernel, remap, runner


def test_design():
    prepared = design.make_prepared()
    assert len(design.CONDITIONS) == 16
    assert len(prepared['runs']) == 8 * 16
...
def _brute_expected(prob_intent, prob_proposal, packed_state, rule):
    ...
    for actions in product(range(17), repeat=3):
        ...
        settled = execution_env.settle(packed_state[None], np.asarray(actions, dtype=np.int16)[None], rule)
        total += prob * float(settled['greedy_reward'][0])
    return total
...
def test_rule_objective():
    static = design.make_prepared()
    spec = static['partitions']['train']
    arrays = dataset.make_arrays(spec, information='PL', indices=np.asarray([0, 1], dtype=np.int64))
...
def test_evaluation_shapes():
    ...
    networks = runner.make_networks(66801)
    result = runner.evaluate_factorized(networks, arrays, spec, 'PL', False, 'strict')
...
if __name__ == '__main__':
    ...
    print('triadic_rule_communication_study tests passed')
