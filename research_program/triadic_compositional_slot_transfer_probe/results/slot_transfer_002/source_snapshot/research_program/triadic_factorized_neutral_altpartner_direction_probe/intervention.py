...
def intervene(networks, receiver_x, receiver_messages, changed_person, donor_packets):
    ...
    # Every viewer receives all three generated W2 packets.  The flattened
    # sender/token one-hot block has 3*4*8=96 entries per viewer; broadcast it
    # across the three viewers before appending visibility bits.
    second_flat = np.eye(8, dtype=np.float64)[second_tokens].reshape(n, 96)
    second_routes = np.concatenate((np.broadcast_to(second_flat[:, None, :], (n, 3, 96)),
                                    np.ones((n, 3, 3), dtype=np.float64)), axis=-1)
    second_delivered = np.broadcast_to(second_tokens[:, None, :, :], (n, 3, 3, 4)).copy()
    ...
    return dict(generated_messages=np.stack((native[:, 0], second_tokens), axis=1),
                delivered_tokens=np.stack((first_tokens, second_delivered), axis=1),
