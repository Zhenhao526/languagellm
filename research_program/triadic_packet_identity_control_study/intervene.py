"""Duplicate first-window intervention for the identity-control runner."""
...
def route(tokens):
    t = np.asarray(tokens); require(t.ndim == 3 and t.shape[1:] == (3, 4) and t.dtype.kind in "iu" and np.all((t >= 0) & (t < 8)), "Invalid messages")
...
def intervene_first(networks, x, natural, senders, donor):
    x = np.asarray(x, dtype=np.float64); n = len(x); m = np.asarray(natural); s = np.asarray(senders); d = np.asarray(donor)
    ...
    for viewer in range(3):
        rows = np.flatnonzero(s != viewer); slots = 32 * s[rows, None] + np.arange(32); r1[rows[:, None], viewer, slots] = enc[rows]
    ...
    return dict(messages=np.stack((m[:, 0], second), axis=1), action_probabilities=p, action_indices=np.argmax(p, axis=-1).astype(np.int16), first_routes=r1, second_routes=r2, neural_forward_samples=6 * n)
...
def observations(states):
    ...
    packed = np.asarray(states); require(packed.ndim == 2 and packed.shape[1] == 10 and packed.dtype.kind in "iu", "Invalid states")
    worlds = [env.State(tuple(map(int, row[:3])), tuple(map(int, row[3:7])), tuple(map(int, row[7:10]))) for row in packed]
    return td.encode_observations([{a: env.observe(w, a, information="PL") for a in env.AGENTS} for w in worlds])
