...
The v4 environment, runner, replay auditor, and tests are `factorized_environment.py`, `calibration_v4.py`, `audit_calibration_v4.py`, and `tests/test_calibration_v4.py`. Implementation and mock-replay tests pass. The 96-episode direct-order gate on seed `20261001` passed all thresholds (96/96 joint successes, actions, and waits; all output schemas valid) and its independent replay audit passed. It took 37.3 minutes. See the [gate report](results/calibration_v4_gate_20261001_analysis.md), [episode record](results/calibration_v4_gate_20261001.json), [audit](results/calibration_v4_gate_20261001_audit.json), and [hash manifest](results/calibration_v4_gate_20261001_manifest.json). The paired matrix on seeds `20261002`–`20261004` is now authorized by the frozen protocol and remains to be run.
