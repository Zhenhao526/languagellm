...
def prepared():
    ...
    static['budget']['intervention_module_samples'] = (static['budget']['cross_case_rows'] + static['budget']['sham_case_rows']) * 6
...
def native_bank(networks, arrays):
    ...
    for start in range(0, n, design.CHUNK_SIZE):
        stop = min(start + design.CHUNK_SIZE, n); trace = core.rollout(networks, arrays['x_PL'][start:stop], True)
...
def run_policy(networks, arrays, spec, cases):
    bank = native_bank(networks, arrays); case_rows = []
    ...
    for sender in range(3):
        ...
        for start in range(0, n, design.CHUNK_SIZE):
            ...
            nat = natural_prob[sl]; inter = result['action_probabilities']; states = bank['states'][ri]
    ...
    return dict(case_count=int(len(cases['receiver_state_indices'])), by_sender=by_sender, sham=sham_errors, natural_bank_worlds=len(bank['states']), natural_module_samples=9 * len(bank['states']), intervention_module_samples=len(cases['receiver_state_indices']) * 6 + 3 * design.SHAM_ROWS_PER_SENDER * 6)
...
def worker(payload):
    ...
    for condition in CONDITIONS:
        ...
        networks = core.load_networks(path); started = time.perf_counter(); summary = run_policy(networks, arrays, spec, cases)
