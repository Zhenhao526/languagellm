...
def summarize(rows):
    ...
    keys = ('partner_transfer', 'natural_physical', 'natural_q', 'natural_conditional_q',
            'intervention_physical', 'intervention_q', 'intervention_conditional_q')
    ...
    for seed in seeds:
        ...
        row = dict(seed=seed, plan_transfer_by_condition={
    ...
    return dict(
        name='rematching_by_communication_signed_directional_plan_transfer_factorized',
        primary=dict(mean=float(primary.mean()), statistics=stats(primary), by_seed=per_seed,
                     scope='At final checkpoint, average three senders within seed, then compute rematched×communication difference in signed donor-only minus receiver-only full-plan transfer.',
