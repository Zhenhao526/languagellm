"""Frozen open-world expansion design.

Parents learn a closed 3x3 ontology. Children are then evaluated on the
held-out old conjunction and on meanings containing a fourth value that no
parent has observed. The only child intervention is whether new meanings are
learned with an incumbent partner (alternating) or by a fresh sender and
receiver together (pair).
"""
...
ARCHITECTURES = ("holistic", "factorized", "tied_routed")
POPULATIONS = ("aligned",)
VISIBILITIES = ("hidden",)
SUPPORTS = ("old_combo", "expanded_alternating", "expanded_pair",
...
def _target_positions(scene, uniforms, support, combo):
    ...
    elif support in ("expanded_single_alternating", "expanded_single_pair"):
...
def episode_stream(seed, count, *, support="old_world", update=0, evaluation=False):
    valid_supports = set(SUPPORTS) | {"old_world"}
    require(count > 0 and support in valid_supports, "bad episode factors")
    require(int(seed) in SEEDS or evaluation, "unknown seed")
    combo = heldout_combo(seed) if int(seed) in SEEDS else 0
    expanded = support in ("expanded_alternating", "expanded_pair",
                           "expanded_single_alternating", "expanded_single_pair")
    ...
    if support.endswith("_pair"):
        fresh_role[is_new_meaning(goal_meaning)] = 2
    return {
...
def balanced_eval_stream(seed, count, goal_kind, support, combo):
    ...
    if support.endswith("_pair"):
        role[is_new_meaning(meanings)] = 2
    return {
        "scene_meanings": scene_meanings,
        "scene": attrs_from_meaning(scene_meanings),
        "target_index": target_index,
...
def prepare():
    return {
        "schema": "open_world_expansion_study_v1",
        "task": TASK,
        ...
        "supports": list(SUPPORTS), "eval_kinds": list(EVAL_KINDS),
        "roles": list(ROLES), "channels": list(CHANNELS),
        "parent_support": "old_world",
        "child_intervention": "expanded_pair assigns both fresh roles for all new meanings; expanded_single_pair does so only for new-single meanings and leaves new-double out of training; alternating arms keep one fresh role against an incumbent",
        "message_symbols": ["@", "#", "￥", "%"],
