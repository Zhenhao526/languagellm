"""Train, evaluate and freeze the compositional signaling control."""
...
def prepare(out):
    ...
    plan = {"schema": "compositional_signaling_v1", "prepared_sha256": sha(out / "prepared.json"), "sources": sources, "runtime": {"python": platform.python_version(), "numpy": np.__version__}, "config": cfg}
...
def train_one(seed, condition, out, updates=None):
    form, pm, pv, ch, task = design.parse_condition(condition); updates = design.UPDATES if updates is None else updates
    ...
    for update in range(1, updates + 1):
        ...
        tr = environment.rollout(p, ep, form, pm, pv, task, ch, sample=True)
        ...
        for slot in range(length):
            ...
            adv = center(future_all[:, slot + 1:].sum(axis=1), context)
        ...
        row = {"update": update, "seed": seed, "condition": condition, "world_sha256": design.array_sha(ep["site_type"]), "goal_sha256": design.array_sha(ep["goal"]), "target_sha256": design.array_sha(ep["target_bits"]), "partner_sha256": design.array_sha(ep["partner_id"]), "message_uniform_sha256": design.array_sha(ep["message_uniforms"]), "action_uniform_sha256": design.array_sha(ep["action_uniforms"]), "return_mean": float(tr["team_return"].mean()), "gradient_norm": norm, "gradient_clip_scale": scale, "parameter_sha256": policy.combined_parameter_hash(p), "elapsed_seconds": time.perf_counter() - start}
        rows.append(row)
    ...
    for evaluation, label in ((False, "training_support"), (True, "heldout")):
        final[label] = evaluate_all(p, seed, form, pm, pv, task, ch, evaluation=evaluation)
    result = {"seed": seed, "condition": condition, "form": form, "partner_mode": pm, "partner_visibility": pv, "channel": ch, "task": task, "updates": updates, "initial_parameter_sha256": init_hash, "trajectory": rows, "final": final, "checkpoints": checkpoints}
...
def evaluate_one(p, ep, form, pm, pv, task, ch, mode, worker, *, message_override=None):
    tr = environment.rollout(p, ep, form, pm, pv, task, ch, message_mode=mode, sample=False, partner_filter=worker, message_override=message_override)
...
def evaluate_recombined(p, ep, form, pm, pv, task, ch, worker):
    if form != "dual2": return {"episodes": 0, "team_return_mean": None, "defined": False}
    override = recombination_override(p, ep, form, pv, task)
    return {**evaluate_one(p, ep, form, pm, pv, task, ch, "natural", worker, message_override=override), "defined": True}


def _semantic_codebook(p, form, pm, pv, task):
    ...
    for goal_idx, goal in enumerate(local_patterns):
        for wi, worker in enumerate(active):
            ...
            for locals_ in local_patterns:
                ...
                tr = environment.rollout(p, ep, form, pm, pv, task, "live", sample=False, partner_filter=None)
...
def evaluate_all(p, seed, form, pm, pv, task, ch, *, evaluation):
    count = 4096 if pm == "fixed" else 4096 * design.WORKERS
    ep = design.episode_stream(seed, pm, pv, task, count, evaluation=evaluation)
    out = {"split": "heldout" if evaluation else "training_support", "workers": {}, "codebook": _semantic_codebook(p, form, pm, pv, task)}
    ...
    for worker in worker_ids:
        ...
        modes = {"natural": evaluate_one(p, ep_worker, form, pm, pv, task, ch, "natural", selected_worker)}
        if ch == "live":
            modes["closed"] = evaluate_one(p, ep_worker, form, pm, pv, task, ch, "closed", selected_worker)
            modes["permuted"] = evaluate_one(p, ep_worker, form, pm, pv, task, ch, "permuted", selected_worker)
        else:
            modes["closed"] = dict(modes["natural"], reused_natural=True)
            modes["permuted"] = dict(modes["natural"], reused_natural=True)
        if form == "dual2" and ch == "live": modes["recombined"] = evaluate_recombined(p, ep_worker, form, pm, pv, task, ch, selected_worker)
