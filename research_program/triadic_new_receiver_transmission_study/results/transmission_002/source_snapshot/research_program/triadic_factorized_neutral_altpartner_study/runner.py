...
CONFIG = dict(
    ...
    payoffs=['altpair_factorized'], rules=['strict'], lives=['live', 'silent'], conditions=list(CONDITIONS),
    ...
    primary='rematched_by_communication_interaction_on_target_pair_legal_rate_conditional_on_physical_execution_centered_time_AUC',
...
def evaluate_factorized(networks, arrays, spec, live):
...
def train_run(seed, condition, static, arrays, execution):
    ...
    def checkpoint(step):
        ...
        evaluated = evaluate_factorized(networks, arrays['train'], spec, live); evaluated['update'] = step
    ...
    with (directory / 'training.jsonl').open('x', encoding='utf-8') as stream:
        for update in range(1, CONFIG['updates'] + 1):
            ...
            canonical_x = arrays['train']['x_PL'][ids]; canonical_rewards = arrays['train']['rewards'][ids]; canonical_states = arrays['train']['packed_states'][ids]
    ...
    final = evaluate_factorized(networks, arrays['new_layouts'], static['partitions']['new_layouts'], live)
