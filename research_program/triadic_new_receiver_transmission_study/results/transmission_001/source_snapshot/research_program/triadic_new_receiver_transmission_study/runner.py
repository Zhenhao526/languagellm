"""Execute the frozen-pair/new-receiver transmission experiment.

The implementation deliberately reuses the already audited factorized PL
task kernel.  It trains only the freshly initialized C modules; A and B are
copied from a completed source checkpoint and receive exact zero gradients.
"""
...
CONFIG = dict(
    ...
    conditions=list(design.CONDITIONS), task="new_receiver_transmission_on_altpair_coordination",
    source_observation="PL_own_need_and_public_layout_only",
    source_policy="factorized neutral/engage source checkpoint",
    adaptation_modules=["C_sender1", "C_sender2", "C_action"],
    frozen_modules=["A_sender1", "A_sender2", "A_action", "B_sender1", "B_sender2", "B_action"],
    optimizer="Adam; zero gradients supplied for frozen A/B modules",
    pairing="same source checkpoint, C initialization, world uniforms, batch indices, message uniforms and rematch assignments for live/silent within each seed/schedule",
...
def fresh_c_networks(seed, schedule_index):
    """Fresh C initialization, independent of the source C checkpoint."""
    return [
...
def source_code_paths():
    paths = [HERE / name for name in (
        "__init__.py", "design.py", "runner.py", "audit.py", "summarize.py", "plot_results.py", "plan.md", "README.md",
...
def zero_frozen_gradients(gradients, networks):
    require(len(gradients) == 9 and len(networks) == 9, "Expected nine source/new networks")
    zeros = [{key: np.zeros_like(value) for key, value in networks[i].items()} for i in range(6)]
    return zeros + gradients[6:]


def train_run(seed, schedule, live, source_networks, arrays, static, execution, baseline):
    ...
    frozen_ab_hash = parameter_hash(networks, range(6))
    source_c_hash = parameter_hash(networks, range(6, 9))
    new_c = fresh_c_networks(seed, schedule_index)
    new_c_hash = parameter_hash(new_c)
    require(new_c_hash != source_c_hash, "Fresh C initialization accidentally equals source C")
    networks[6:9] = new_c
    ...
    def save_and_evaluate(step):
        ...
        row = dict(
            ...
            parameter_sha256=parameter_hash(networks), frozen_ab_parameter_sha256=parameter_hash(networks, range(6)),
            new_agent_parameter_sha256=parameter_hash(networks, range(6, 9)),
    ...
    with (directory / "training.jsonl").open("w", encoding="utf-8") as stream:
        for update in range(1, design.UPDATES + 1):
            ...
            applied = zero_frozen_gradients(gradients, networks)
            norm, scale = core.base.adam_step(networks, applied, optimizer, update)
            require(parameter_hash(networks, range(6)) == frozen_ab_hash, "Frozen A/B changed during update")
            row.update(
                ...
                frozen_ab_parameter_sha256=parameter_hash(networks, range(6)),
                new_agent_parameter_sha256=parameter_hash(networks, range(6, 9)),
                elapsed_seconds=time.perf_counter() - started,
            )
            stream.write(core.base.json_bytes(row).decode())
    ...
    result = dict(
        seed=seed, schedule=schedule, condition=condition, live=bool(live), updates=design.UPDATES,
        source_checkpoint_sha256=sha(source_run_path(seed, schedule) / "checkpoint_6000.npz"),
        source_full_parameter_sha256=source_full_hash, source_c_parameter_sha256=source_c_hash,
        frozen_ab_parameter_sha256=frozen_ab_hash, new_agent_initial_parameter_sha256=new_c_hash,
        ...
        frozen_ab_parameter_unchanged=True, no_external_model=True, elapsed_seconds=time.perf_counter() - started,
    )
...
def worker(payload):
    ...
    for schedule in design.SCHEDULES:
        ...
        source_cache[schedule] = source_networks
        ...
        for live in design.LIVES:
            runs.append(train_run(seed, schedule, bool(live), source_networks, arrays, static, execution,
                                  baseline_cache[schedule][bool(live)]))
...
def verify_pairing(execution, runs):
    ...
    for seed in design.SEEDS:
        for schedule in design.SCHEDULES:
            live = by[(seed, f"{schedule}_new_receiver_PL_live")]
            silent = by[(seed, f"{schedule}_new_receiver_PL_silent")]
            require(live["source_checkpoint_sha256"] == silent["source_checkpoint_sha256"], "Source checkpoint pairing failed")
            require(live["new_agent_initial_parameter_sha256"] == silent["new_agent_initial_parameter_sha256"], "C initialization pairing failed")
            require(live["frozen_ab_parameter_sha256"] == silent["frozen_ab_parameter_sha256"], "Frozen A/B pairing failed")
