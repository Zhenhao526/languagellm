...
def main(probe,out,workers=4):
    ...
    out.mkdir(parents=True); verification=dict(status="passed",probe=str(probe),policy_blocks=96,rows_replayed=96*76032,live_rows_replayed=48*76032,worlds_per_policy=int(static["partition"]["world_count"]),model_forward_samples=result["model_forward_samples"],optimizer_updates=0,max_abs_error=float(max_abs),checkpoint_hashes_checked=96,placebo_mapping_checks=76032,independent_replay="Producer implementation not imported; natural bank, aligned/placebo routing, legal plan mass, partner transfer, strict settlement and group summaries recomputed independently")
    (out/"verification.json").write_text(json.dumps(verification,ensure_ascii=False,sort_keys=True,indent=2)+"\n",encoding="utf8"); receipt=dict(status="passed",verification_sha256=sha(out/"verification.json"),policy_blocks=96,rows_replayed=96*76032,max_abs_error=float(max_abs),optimizer_updates=0); (out/"receipt.json").write_text(json.dumps(receipt,ensure_ascii=False,sort_keys=True,indent=2)+"\n",encoding="utf8"); print(json.dumps(verification,ensure_ascii=False))
