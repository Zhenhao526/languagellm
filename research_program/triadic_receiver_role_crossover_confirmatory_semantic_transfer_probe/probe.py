...
def worker(payload):
    ...
    for condition in design.CONDITIONS:
        meta=inputs["checkpoints"][f"{seed}:{condition}"]; path=Path(meta["path"]); require(sha(path)==meta["sha256"],"Checkpoint binding mismatch"); networks=source_runner.load_networks(path); role, schedule, live = role_runner.design.parse_condition(condition); started=time.perf_counter(); summary=run_policy(networks,arrays,cases,live); rows.append(dict(seed=seed,role=role,condition=condition,schedule=schedule,live=bool(live),checkpoint_sha256=sha(path),parameter_sha256=source_runner.parameter_hash(networks),elapsed_seconds=time.perf_counter()-started,**summary)); write(out/f"{condition}.json",rows[-1])
    return rows
