...
def _evaluation_paths(run, static):
    ...
    for step in design.STEPS:
        ...
        rows.append((f"trajectory_{step:04d}", design.TARGET, row["evaluation"], row["checkpoint_path"], row["checkpoint_sha256"]))
    for part in design.PARTS:
        if part != design.TARGET:
            rows.append((f"final_{part}", part, run["final"][part], run["final_checkpoint_sha256"], run["final_checkpoint_sha256"]))
...
def replay_evaluation(run, label, part, record, checkpoint, checkpoint_sha256, arrays, static):
    ...
    expected_data = dict(
        state_indices=ids, messages=messages, wire_messages=wire_messages,
    ...
    truth = environment.truth_from_rewards(states, arrays[part]["rewards"][ids])
    metrics = dict(
        ...
        need_response=dict(
            native=formation.cases.metrics(static["need_response_cases"][part], native["actual_pair_index"]),
            common_reciprocal=formation.cases.metrics(static["need_response_cases"][part], common["actual_pair_index"]),
        ),
    )
    for section in ("native", "strict", "common_reciprocal"):
        for key in ("greedy_reward", "physical_execution_rate", "full_success_rate"):
            if key in metrics[section] and key in record[section]:
                require(abs(float(metrics[section][key]) - float(record[section][key])) == 0.0,
                        f"Stored {section}.{key} summary changed: {path}")
    require(sha(checkpoint) == checkpoint_sha256,
...
def audit(out):
    ...
    for run in results["runs"]:
        ...
        for label, part, record, _, checkpoint_sha256 in _evaluation_paths(run, static):
            checkpoint = _checkpoint_path(run, label, execution)
            require(checkpoint.is_file(), f"Missing checkpoint: {checkpoint}")
            reports.append(replay_evaluation(run, label, part, record, checkpoint, checkpoint_sha256, arrays, static))
...
def _load_verified_static(out):
    ...
    # The training freeze is authoritative for the producer sources.  The
    # auditor itself may receive a bug-fix after execution; its independent
    # replay is checked below against the frozen artifacts rather than making
    # the completed run unverifiable because of an audit-only patch.
    require(static == design.make_prepared(), "Frozen design changed before audit")
