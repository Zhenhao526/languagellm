...
def worker(payload):
    """Spawn-safe worker using the rematched four-arm condition grid."""
    seed, static, execution = payload
    execution = Path(execution)
    out = execution / f'seed_{seed}'
    out.mkdir(parents=True, exist_ok=False)
    spec, cases = static['partition'], static['cases']
    arrays = make_arrays(spec)
    rows = []
    for condition in design.CONDITIONS:
        path = Path(static['inputs']['checkpoints'][f'{seed}:{condition}']['path'])
        if sha(path) != static['inputs']['checkpoints'][f'{seed}:{condition}']['sha256']:
...
def execute(out):
    """Execute the four-arm probe without delegating to the old two-arm worker."""
    ...
    if execution.exists():
        raise ValueError('Never overwrite probe execution')
    ...
    try:
        ...
        if len(rows) != 64:
            raise ValueError('Incomplete 64-policy probe grid')
