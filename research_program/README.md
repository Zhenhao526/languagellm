...
两个 signaling 包的 `results/formal_20260917/` 只保留紧凑轨迹、终点汇总、冻结 JSON、失效收据和独立审计；temporal 包保留源码、冻结方案、紧凑审计和结果报告，不保留原始日志或 checkpoint。完整阶段历史仍可从 Git 历史中的旧提交恢复；当前工作树只展开继续实验所需的代码和记录。
