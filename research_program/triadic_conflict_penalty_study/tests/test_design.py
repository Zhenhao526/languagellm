...
from research_program.triadic_conflict_penalty_study import design, kernel, remap, runner


def test_design():
    prepared = design.make_prepared()
    assert len(design.RULES) == 6
    assert len(design.CONDITIONS) == 48
    assert len(prepared['runs']) == 8 * 48
    assert prepared['need_count'] == 1560
    assert prepared['pair_set_counts'] == {'AB_AC': 520, 'AB_BC': 520, 'AC_BC': 520}
    assert design.PENALTIES['c0'] == 0 and design.PENALTIES['c100'] == 1
...
def test_rule_objective():
    ...
    # c0 is exactly the old reciprocal objective; c100 has the same rewarding
    # events and derivatives as the hard strict objective.
    ...
    hard = kernel.objective_terms(logits, arrays['rewards'], 'strict')
    full_cost = kernel.objective_terms(logits, arrays['rewards'], 'c100')
    for key in ('J', 'log_J', 'intent_log_gradient', 'proposal_log_gradient'):
        assert np.max(np.abs(hard[key] - full_cost[key])) < 1e-12
...
def test_evaluation_shapes():
    ...
    spec = static['partitions']['train']
    arrays = runner.make_arrays(spec, 'PL')
    networks = runner.make_networks(66901)
    for rule in design.RULES:
...
if __name__ == '__main__':
    test_design(); test_remap(); test_rule_objective(); test_evaluation_shapes()
    print('triadic_conflict_penalty_study tests passed')
