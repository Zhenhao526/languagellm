"""Frozen design for a continuous third-agent conflict-penalty study.
...
SEEDS = tuple(range(66901, 66909))
# cX means reciprocal execution with a cost of X for an engaged third
# proposal; strict is retained as a hard-constraint anchor.
RULES = ('c0', 'c25', 'c50', 'c75', 'c100', 'strict')
PENALTIES = {'c0': 0.0, 'c25': 0.25, 'c50': 0.50, 'c75': 0.75,
             'c100': 1.0, 'strict': None}
INFORMATIONS = ('PL', 'FI')
...
def make_prepared():
    ...
    runs = [dict(seed=seed, rule=rule, information=information, schedule=schedule,
            ...
            for schedule in SCHEDULES for live in (True, False)]
    return dict(
        schema='triadic_conflict_penalty_v1', seeds=list(SEEDS), rules=list(RULES),
        ...
        scientific_question=(
            'Does a continuous cost on an engaged unmatched third actor produce a monotone '
            'change in the routed-message effect on partner selection, and is the response '
            'different under private versus full information?'),
        primary=('penalty-dose × routed-communication interaction on target-pair legal rate '
                 'conditional on physical execution, with strict as a hard-constraint anchor'),
        ...
        settlement=(
            'c0–c100 execute a matching pair while multiplying reward by one minus the declared '
            'penalty when the third actor is engaged; strict requires a neutral third actor.'),
        ...
        development_status=('Independent seed block 66901–66908; six penalty/constraint levels '
                             'crossed with information/schedule/channel before training.'),
    )
