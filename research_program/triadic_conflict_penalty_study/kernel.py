"""Exact policy-gradient kernel for a continuous third-agent conflict cost.

``c0`` through ``c100`` use reciprocal physical execution. When a matching
pair executes, an engaged unmatched third actor multiplies the native reward
by ``1 - penalty``. ``strict`` is the hard constraint anchor and requires a
neutral third actor in the rewarding event itself.
"""
...
def objective_terms(logits, rewards, rule):
    ...
    for event in range(n_events):
        structural = event if rule == 'strict' else event // 2
        branch = None if rule == 'strict' else event % 2
        for actor in range(3):
