"""Train the continuous third-agent conflict-penalty experiment."""
...
CONFIG = dict(
    ...
    settlement_rules=('c0/c25/c50/c75/c100: reciprocal matching pair with a declared cost on an '
                      'engaged unmatched third; strict: matching pair plus neutral third'),
    ...
    primary='penalty_dose_by_routed_communication_interaction_on_target_pair_legal_rate_conditional_on_physical_execution_centered_time_AUC',
    selection='fixed8 new seeds ×6 constraint levels ×2 information ×2 schedules ×2 channels ×3000 updates; no early stopping',
...
def settle_factorized(states, actions, rule):
    """Settle greedy proposals and attach the declared third-agent cost."""
    if rule == 'strict':
        settled = execution_env.settle(states, actions, rule='strict')
        settled['penalized_reward'] = settled['greedy_reward'].copy()
        settled['conflict'] = np.zeros(len(actions), dtype=bool)
        return settled
