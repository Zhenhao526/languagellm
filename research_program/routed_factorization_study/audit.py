"""Independent replay audit for routed factor-sharing executions."""
...
def _parent_replay(execution, result):
    ...
    population = result["population"]; community = int(result["community"]); updates = int(result["updates"])
    ...
    for update, row in enumerate(rows, 1):
        episode = design.episode_stream(seed, design.BATCH_SIZE, support="full", update=update)
        ...
        max_error = max(max_error, abs(row["return_mean"] - float(tr["team_return"].mean())),
                         abs(row["gradient_norm"] - norm), abs(row["gradient_clip_scale"] - scale))
...
def _child_replay(execution, result):
    ...
    for update, row in enumerate(rows, 1):
        ...
        goals = np.asarray(episode["goal_meaning"])
        if support == "heldout_combo":
            assert not np.any(goals == design.heldout_combo(seed))
        if support == "heldout_value":
            assert not np.any((goals // design.VALUES) == design.heldout_value(seed))
        _check_stream(row, episode)
...
def audit(prepared, execution):
    ...
    parent_rows, child_rows, parent_checkpoints, child_checkpoints = 0, 0, 0, 0
    max_error = 0.0
    parent_log = {}; child_log = {}
    for result in payload["parents"]:
        ...
        key = (int(result["seed"]), result["architecture"], result["population"], int(result["community"]))
        parent_log[key] = _read_rows(Path(execution) / "parents" /
                                     f"seed_{result['seed']}_{result['architecture']}_{result['population']}_community_{result['community']}" /
                                     "training.jsonl")
    ...
    architecture_pairs = visibility_pairs = population_pairs = support_pairs = 0
            ...
            for visibility in design.VISIBILITIES:
                for support in design.SUPPORTS:
                    for left, right in combinations(design.ARCHITECTURES, 2):
                        architecture_pairs += _pair(child(seed, left, population, visibility, support),
                                                     child(seed, right, population, visibility, support), fields)
            for architecture in design.ARCHITECTURES:
                for support in design.SUPPORTS:
                    visibility_pairs += _pair(child(seed, architecture, population, "hidden", support),
                                              child(seed, architecture, population, "visible", support), fields)
        for architecture in design.ARCHITECTURES:
            for visibility in design.VISIBILITIES:
                for support in design.SUPPORTS:
                    population_pairs += _pair(child(seed, architecture, "aligned", visibility, support),
                                              child(seed, architecture, "conflict", visibility, support), fields)
        for architecture in design.ARCHITECTURES:
            for population in design.POPULATIONS:
                for visibility in design.VISIBILITIES:
                    support_fields = [field for field in fields if field != "goal_sha256"]
                    support_pairs += _pair(child(seed, architecture, population, visibility, "full"),
                                           child(seed, architecture, population, visibility, "heldout_combo"), support_fields)
                    support_pairs += _pair(child(seed, architecture, population, visibility, "full"),
                                           child(seed, architecture, population, visibility, "heldout_value"), support_fields)
    ...
    return {"schema": "routed_factorization_audit_v1",
            "status": "passed" if complete and max_error <= 1e-12 else ("partial" if max_error <= 1e-12 else "failed"),
            "prepared_schema": cfg["schema"], "parent_runs": len(payload["parents"]),
            "child_runs": len(payload["children"]), "expected_parent_runs": expected_parent,
            "expected_child_runs": expected_child, "parent_training_log_rows": parent_rows,
            "child_training_log_rows": child_rows, "parent_checkpoints": parent_checkpoints,
            "child_checkpoints": child_checkpoints, "paired_architecture_rows": architecture_pairs,
            "paired_visibility_rows": visibility_pairs, "paired_population_rows": population_pairs,
            "paired_support_rows": support_pairs, "max_abs_replay_error": float(max_error),
            "raw_execution_tree": str(execution)}


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--prepared", required=True); parser.add_argument("--execution", required=True); parser.add_argument("--out", required=True)
