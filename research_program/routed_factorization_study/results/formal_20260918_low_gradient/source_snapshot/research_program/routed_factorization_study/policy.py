...
which slot carries which attribute. A row-wise softmax routing matrix is
learned independently by the sender and receiver.
"""
...
def make_policy(seed, architecture, *, visible=False):
    ...
    rng = np.random.default_rng(
        np.random.SeedSequence([int(seed), design.POLICY_SALT,
                                int(architecture == "factorized"), int(architecture == "routed")])
    )
    ...
    sr = _normal(rng, (design.MESSAGE_LENGTH, design.ATTRIBUTES))
    rf = _normal(rng, (design.MESSAGE_LENGTH, design.ALPHABET_SIZE, design.ATTRIBUTES, design.VALUES))
    rr = _normal(rng, (design.MESSAGE_LENGTH, design.ATTRIBUTES))
    if visible:
...
def sender_arrays(params, meanings, partner_id, visibility):
    ...
    for partner in range(design.PARTNERS):
        ...
        if architecture == "holistic":
            ...
            if visibility == "hidden":
                factors, routes = params["sender_hidden"], params["sender_route_hidden"]
            else:
                context[idx] = partner * design.MEANINGS + meanings[idx]
                factors, routes = params["sender_visible"][partner], params["sender_route_visible"][partner]
...
def receiver_arrays(params, message_state, scene_meanings, partner_id, visibility):
    ...
    for partner in range(design.PARTNERS):
        ...
        if architecture == "factorized":
            ...
            continue
        if visibility == "hidden":
            table, routes = params["receiver_hidden"], softmax(params["receiver_route_hidden"], axis=-1)
        else:
            context[idx] = partner * design.MESSAGE_STATE_COUNT + state[idx]
            table, routes = params["receiver_visible"][partner], softmax(params["receiver_route_visible"][partner], axis=-1)
        for slot in range(design.MESSAGE_LENGTH):
...
def routing_probabilities(params, *, side="sender", partner_id=0, visibility="hidden"):
    if params["architecture"] != "routed":
        return None
    if side == "sender":
        key = "sender_route_hidden" if visibility == "hidden" else "sender_route_visible"
    elif side == "receiver":
        key = "receiver_route_hidden" if visibility == "hidden" else "receiver_route_visible"
    else:
