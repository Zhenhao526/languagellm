...
def receiver_arrays(params, message_state, scene_meanings, partner_id, visibility):
    ...
    for partner in range(design.PARTNERS):
        ...
        if architecture == "factorized":
            ...
            continue
        if visibility == "hidden":
            table, routes = params["receiver_hidden"], softmax(design.ROUTE_TEMPERATURE * params["receiver_route_hidden"], axis=-1)
        else:
            context[idx] = partner * design.MESSAGE_STATE_COUNT + state[idx]
            table, routes = params["receiver_visible"][partner], softmax(design.ROUTE_TEMPERATURE * params["receiver_route_visible"][partner], axis=-1)
        for slot in range(design.MESSAGE_LENGTH):
