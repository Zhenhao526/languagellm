...
def prepare(out):
    ...
    plan = {"schema": "routed_factorization_study_v1", "prepared_sha256": sha(out / "prepared.json"),
...
def _sender_update(grad, tr, ep, mask, params, visibility, selected):
    ...
    for owner, local in groups:
        ...
        if visibility == "hidden":
            ...
            routes = policy.softmax(design.ROUTE_TEMPERATURE * params["sender_route_hidden"], axis=-1)
            grad_factors = grad["sender_hidden"]
            grad_routes = grad["sender_route_hidden"]
        else:
            factors = params["sender_visible"][owner]
            routes = policy.softmax(design.ROUTE_TEMPERATURE * params["sender_route_visible"][owner], axis=-1)
            grad_factors = grad["sender_visible"][owner]
            grad_routes = grad["sender_route_visible"][owner]
        for slot in range(design.MESSAGE_LENGTH):
...
def _receiver_update(grad, tr, ep, mask, params, visibility):
    ...
    for owner, local in groups:
        ...
        if visibility == "hidden":
            ...
            routes = policy.softmax(design.ROUTE_TEMPERATURE * params["receiver_route_hidden"], axis=-1)
            grad_table = grad["receiver_hidden"]
            grad_routes = grad["receiver_route_hidden"]
        else:
            table = params["receiver_visible"][owner]
            routes = policy.softmax(design.ROUTE_TEMPERATURE * params["receiver_route_visible"][owner], axis=-1)
            grad_table = grad["receiver_visible"][owner]
            grad_routes = grad["receiver_route_visible"][owner]
        for slot in range(design.MESSAGE_LENGTH):
            tok = tokens[local, slot]
            valid = tok >= 0
            if not np.any(valid):
...
def _eval_episode(seed, goal_kind, support, combo, value):
    return design.balanced_eval_stream(seed + design.EVAL_SEED_OFFSET, design.PARTNERS * 2 * 512,
                                       goal_kind, support, combo, value)
...
def evaluate_parent(params, seed, architecture, population, community):
    combo, value = design.heldout_combo(seed), design.heldout_value(seed)
    result = {"architecture": architecture, "population": population, "community": community}
    for kind in ("all", "seen"):
        ep = _eval_episode(seed, kind, "full", combo, value)
...
def evaluate_child(communities, fresh, seed, architecture, population, visibility, support):
    ...
    for kind in ("all", "seen", "heldout_combo", "heldout_value"):
        ep = _eval_episode(seed, kind, support, combo, value)
...
def train_parent(seed, architecture, population, community, execution, updates=None):
    ...
    for update in range(1, updates + 1):
        ep = design.episode_stream(seed, design.BATCH_SIZE, support="full", update=update)
    ...
    result = {"kind": "parent", "seed": seed, "architecture": architecture, "population": population,
              ...
              "heldout_combo": design.heldout_combo(seed), "heldout_value": design.heldout_value(seed),
...
def train_child(seed, condition, execution, parent_checkpoints, updates=None):
    ...
    for update in range(1, updates + 1):
        ...
        if support == "heldout_value":
            design.require(not np.any((np.asarray(ep["goal_meaning"]) // design.VALUES) == design.heldout_value(seed),),
                           f"heldout value leakage {condition}/{update}")
