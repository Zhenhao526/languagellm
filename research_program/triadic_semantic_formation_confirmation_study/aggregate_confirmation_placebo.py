...
def analyze(probe, audit, out):
    ...
    require(result["status"] == "completed_json_only_semantic_transfer_confirmation_placebo_probe" and result["policy_blocks"] == 8 and result["intervention_rows"] == 1920, "Incomplete confirmation result")
    ...
    lines += ["", "aligned−placebo 若为正，才支持正确源消息比同层错配消息更能把动作推向源端；区间为四种子的描述性 t(3) 区间。PI-silent 两种干预均逐行等于自然路由。", "", "[JSON](summary.json)；[执行结果](../execution/results.json)；[独立审计](../audit_placebo_001/verification.json)。"]
