...
def summary(row):
    ...
    return {
        ...
        "error_correcting_candidate": bool(row["form"] == "triple2" and new >= 0.60 and final["pairwise_min_hamming"] >= 2),
    }
...
def analyze(parents_path, results_path):
    ...
    for form in design.FORMS:
        for noise_key in design.NOISE_KEYS:
            pairs = [
                next(row for row in rows if row["seed"] == seed and row["form"] == form and row["noise_key"] == noise_key and row["adaptation"] == "coadapt")["natural"]
                - next(row for row in rows if row["seed"] == seed and row["form"] == form and row["noise_key"] == noise_key and row["adaptation"] == "worker_only")["natural"]
                for seed in design.SEEDS
            ]
            incumbent_pairs = [
                next(row for row in rows if row["seed"] == seed and row["form"] == form and row["noise_key"] == noise_key and row["adaptation"] == "coadapt")["incumbent_natural"]
                - next(row for row in rows if row["seed"] == seed and row["form"] == form and row["noise_key"] == noise_key and row["adaptation"] == "worker_only")["incumbent_natural"]
                for seed in design.SEEDS
            ]
    ...
    for noise_key in design.NOISE_KEYS:
        for adaptation in design.ADAPTATIONS:
            pairs = [
                next(row for row in rows if row["seed"] == seed and row["form"] == "triple2" and row["adaptation"] == adaptation and row["noise_key"] == noise_key)["natural"]
                - next(row for row in rows if row["seed"] == seed and row["form"] == "atomic8" and row["adaptation"] == adaptation and row["noise_key"] == noise_key)["natural"]
                for seed in design.SEEDS
            ]
            form_effects.append({"noise_key": noise_key, "adaptation": adaptation, "triple2_minus_atomic8": float(np.mean(pairs)), "ci95_t": ci(pairs)})
    return {"schema": "error_correcting_signaling_analysis_v1", "rule": {"functional_natural_min": 0.60, "error_correcting_candidate": "triple2 natural >= 0.60 and minimum sender-code Hamming distance >= 2"}, "parents": parent_rows, "rows": rows, "groups": groups, "adaptation_effects": adaptation_effects, "form_effects": form_effects}


def write_md(path, data):
    groups = {(row["form"], row["adaptation"], row["noise_key"]): row for row in data["groups"]}
    lines = ["# Emergent redundancy and noisy protocol repair", "", "`triple2` and `atomic8` each have eight raw message states; `dual2` has four. The atomic corruption probability is matched to the probability that at least one of three binary coordinates flips.", "", "| form | adaptation | noise | natural | clean | silent | live−silent | min Hamming | functional | error-correcting candidate |", "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|"]
    for form in design.FORMS:
        for adaptation in design.ADAPTATIONS:
            for noise_key in design.NOISE_KEYS:
                row = groups[(form, adaptation, noise_key)]
                lines.append(f"| `{form}` | `{adaptation}` | {row['noise_p']:.2f} | {row['natural_mean']:.3f} | {row['clean_mean']:.3f} | {row['silent_mean']:.3f} | {row['live_minus_silent_mean']:.3f} | {row['min_hamming_mean']:.2f} | {row['functional_count']}/{row['n']} | {row['error_correcting_candidate_count']}/{row['n']} |")
