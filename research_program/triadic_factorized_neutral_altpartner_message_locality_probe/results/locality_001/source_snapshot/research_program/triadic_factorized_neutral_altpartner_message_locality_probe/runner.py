...
def prepare_metadata(source_hashes, input_hashes, source_prepared):
    # Keep the exact evaluation partition in the frozen probe metadata.  The
    # probe must not silently reconstruct a different need/layout ordering.
    return dict(schema=config()["schema"], config=config(), source_sha256=source_hashes, input_sha256=input_hashes,
                partitions=source_prepared["partitions"], prepared_at=core.base.now(), no_training=True, no_model_calls=True)


def prepare(out):
    out = Path(out).resolve(); require(not out.exists(), "Never overwrite preparation")
    _source_plan, source_prepared, _source_result = verify_source()
    ...
    prepared = prepare_metadata(source_hashes, input_hashes, source_prepared)
...
def verify(out):
    ...
    _source_plan, source_prepared, _source_result = verify_source()
    require(prepared["partitions"] == source_prepared["partitions"], "Evaluation partition changed")
    require(int(prepared["partitions"]["new_layouts"]["world_count"]) == config()["new_layout_worlds"], "Unexpected new-layout world count")
...
def execute(out):
    ...
    write_new(execution / "receipt.json", dict(status="passed", output_results_sha256=sha(execution / "results.json"), rows=len(rows), worlds=budget()["worlds"],
