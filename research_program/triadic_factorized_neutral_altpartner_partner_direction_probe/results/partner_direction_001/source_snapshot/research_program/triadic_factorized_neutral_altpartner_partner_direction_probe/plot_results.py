...
def main(results_path, output):
    ...
    transfer = []
    for seed in seeds:
        # The execution rows store one signed full-plan transfer per sender.
        # Average senders within a seed before forming the four-arm contrast.
        cell = {(s, live): float(np.mean([
                    z['plan_transfer'] for z in by[seed, s, live]['by_sender']
    ...
    for schedule, live in (('static', False), ('static', True), ('rematched', False), ('rematched', True)):
        # Average across all frozen seeds and senders.  The previous draft
        # accidentally reused the last seed from the forest loop here.
        endpoint.append(float(np.mean([
            np.mean([z['plan_transfer'] for z in by[seed, schedule, live]['by_sender']])
            for seed in seeds
        ])) * 100)
