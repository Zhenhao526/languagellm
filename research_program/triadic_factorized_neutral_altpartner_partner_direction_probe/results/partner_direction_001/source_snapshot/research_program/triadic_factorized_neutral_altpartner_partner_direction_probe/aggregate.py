"""JSON-only aggregation for the factorized directional probe."""
...
def main(source, output):
    ...
    if raw.get('status') != 'completed' or raw.get('posthoc') is not True:
        raise ValueError('Incomplete posthoc execution')
    rows = raw['rows']; expected = len(design.SEEDS) * len(design.CONDITIONS)
    if len(rows) != expected:
        raise ValueError(f'Expected {expected} policy rows')
    result = dict(status='completed_json_only_posthoc_aggregation', source=str(source), rows=rows,
                  primary=metrics.summarize(rows), no_training_updates=True,
                  new_model_forwards=True,
                  recovery_note='Read policy rows only; no optimizer updates or training calls.')
    result_path = output / 'results.json'
    result_path.write_text(json.dumps(result, ensure_ascii=False, indent=2) + '\n')
    receipt = dict(status='passed', input_rows=len(rows),
                   model_forwards=raw.get('budget', {}).get('total_new_module_samples', 0),
                   optimizer_updates=0,
