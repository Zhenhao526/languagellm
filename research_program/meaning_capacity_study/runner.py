"""Train clean parents and noisy children across an eight-meaning capacity curve."""
...
def prepare(out):
    ...
    plan = {"schema": "meaning_capacity_study_v1", "prepared_sha256": sha(out / "prepared.json"), "sources": sources, "runtime": {"python": platform.python_version(), "numpy": np.__version__}, "config": cfg}
...
def sender_codebook(params, form):
    out = []
    for goal in design.goal_table():
        context = int(design.goal_index(goal))
...
def task_class_hamming(codebook, task):
    classes = design.meaning_classes(task)
    if task != "shared8":
        return pairwise_min_hamming(codebook), 0
    within = 0
    for cls in classes:
        within = max(within, max(sum(a != b for a, b in zip(codebook[i], codebook[j])) for i in cls for j in cls if i < j))
    cross = min(sum(a != b for a, b in zip(codebook[i], codebook[j])) for i in classes[0] for j in classes[1])
