"""Frozen design for a paired global-reward-scale null control.
...
SEEDS = tuple(range(67001, 67009))
# c75 penalizes only an engaged unmatched third proposal.  global25 multiplies
# every native reward by .25, including non-conflict worlds.  c0 is the
# reciprocal baseline.  These three rules are paired on the same streams.
RULES = ('c0', 'c75', 'global25')
PENALTIES = {'c0': 0.0, 'c75': 0.75, 'global25': 0.75}
GLOBAL_SCALES = {'c0': 1.0, 'c75': 1.0, 'global25': 0.25}
INFORMATIONS = ('PL', 'FI')
SCHEDULES = ('static', 'rematched')
LIVES = (True, False)
PARTS = ('train', 'new_layouts')
UPDATES = (0, 100, 500, 1500, 3000)
PAIR_NAMES = ('AB', 'AC', 'BC')
...
def make_prepared():
    ...
    runs = [dict(seed=seed, rule=rule, information=information, schedule=schedule,
            ...
            for schedule in SCHEDULES for live in (True, False)]
    return dict(
        schema='triadic_global_reward_control_v1', seeds=list(SEEDS), rules=list(RULES),
        penalty_values=PENALTIES, global_reward_scales=GLOBAL_SCALES,
        ...
        training_updates=len(runs) * 3000,
        training_world_samples=len(runs) * 3000 * 256,
        ...
        scientific_question=(
            'Does the conflict-specific c75 effect survive a global reward-scale control '
            'whose log-reward policy gradient should be invariant?'),
        primary=('paired c75-minus-global25 contrast on target-pair legal rate and physical '
                 'execution, with c0 as the reciprocal baseline'),
        ...
        settlement=(
            'c0 and c75 execute a matching pair; c75 multiplies reward by one minus the declared '
            'penalty only when the third actor is engaged. global25 executes c0 and multiplies '
            'every native reward by .25.'),
        ...
        pairing=('Within each seed, every condition shares initial parameters, world uniforms, '
                 'batch indices and message uniforms. Live/silent and penalty/strict therefore '
        ...
        development_status=('Independent seed block 67001–67008; c0/c75/global25 crossed with '
                             'information/schedule/channel before training.'),
    )
