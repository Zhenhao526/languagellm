...
def summarize(runs):
    ...
    for seed in seeds:
        ...
        endpoint_json = {f"{'live' if live else 'silent'}_{key}": value for (live, key), value in endpoint_rows.items()}
        seed_rows.append(dict(seed=seed, by_metric=live_rows, endpoint=endpoint_json,
