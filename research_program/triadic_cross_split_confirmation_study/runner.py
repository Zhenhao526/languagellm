...
def _evaluate_partition(networks, arrays, ids, rule, live):
    ...
    return ids, actual, target


def _cross_metrics(static, evaluations):
    # The compact evaluator returns only the states touched by a cross edge.
    # Expand the two small endpoint arrays to partition-indexed vectors here,
    # so the edge reducer uses the same indexing convention as the audited
    # post-hoc probe.
    full = {}
    for part, (ids, actual, target) in evaluations.items():
        count = static['partitions'][part]['world_count']
        actual_full = np.empty(count, dtype=np.int8)
        target_full = np.empty(count, dtype=np.int8)
        actual_full[ids] = actual
        target_full[ids] = target
        full[part] = (actual_full, target_full)
    result = {}
    for direction in ('train_to_heldout', 'heldout_to_train'):
        split = static['cross_splits'][direction]
        source_actual, source_target = full[split['source']]
        destination_actual, destination_target = full[split['destination']]
