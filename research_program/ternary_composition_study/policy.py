"""Tabular sender and worker policies for ternary signaling."""
...
def make_policy(seed, form):
    ...
    rng = np.random.default_rng(np.random.SeedSequence([int(seed), 771, k, length]))
    hidden = rng.normal(0.0, 0.02, size=(design.OBJECT_TYPES ** 2, length, k))
    return {"sender_logits_hidden": hidden.copy(), "sender_logits_visible": np.repeat(hidden, design.WORKERS, axis=0).copy(), "worker_logits": rng.normal(0.0, 0.02, size=(design.WORKERS, design.state_count(form), design.HORIZON, design.OBJECT_TYPES, design.OBJECT_TYPES * design.CAPACITY + 1, design.ACTION_COUNT))}
