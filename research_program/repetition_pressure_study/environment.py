"""Two-subtask environment with simultaneous noisy message delivery."""
...
def _corrupt(selected, form, noise_p, noise_uniforms):
    ...
    if p <= 0.0:
        return delivered
    if form in ("dual2", "triple2"):
        flip = noise_uniforms[:, :, 0] < p
        delivered[flip] = 1 - delivered[flip]
        return delivered
    require(form == "atomic8", "unknown form")
    q = design.atomic_corruption_probability(p)
    replace = noise_uniforms[:, 0, 0] < q
    # A deterministic non-identity replacement keeps the corruption event
    # observable while the second uniform is retained in the paired stream.
    delivered[replace, 0] = (delivered[replace, 0] + 1 + (noise_uniforms[replace, 0, 1] * 7).astype(np.int64)) % 8
    return delivered
