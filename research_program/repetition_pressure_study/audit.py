...
def compare_final(stored, fresh):
    ...
    assert stored["task_class_hamming"] == fresh["task_class_hamming"]
    assert stored["within_class_hamming"] == fresh["within_class_hamming"]
    return max_error
...
def audit(prepared, parents_path, execution):
    ...
    for path in parent_files:
        ...
        fresh = runner.evaluate(params, result["seed"], result["form"], result["task"], 0.0)
        max_error = max(max_error, compare_final(result["final"], fresh))
        key = (int(result["seed"]), result["task"], result["form"])
    ...
    for path in child_files:
        ...
        child_checkpoints += 1
        params = runner.load_checkpoint(checkpoint)
        fresh = runner.evaluate(params, result["seed"], result["form"], result["task"], result["noise_p"])
        max_error = max(max_error, compare_final(result["final"], fresh))
        key = (int(result["seed"]), result["task"], result["form"], result["adaptation"], result["noise_key"])
        assert key not in child_by
        child_by[key] = result
        if result["adaptation"] != "scratch":
            parent = parent_by[(int(result["seed"]), result["task"], result["form"])]
    ...
    present_tasks = sorted({key[1] for key in present_child_keys}, key=design.TASKS.index)
    present_forms = sorted({key[2] for key in present_child_keys}, key=design.FORMS.index)
    present_adaptations = sorted({key[3] for key in present_child_keys}, key=design.ADAPTATIONS.index)
    for seed in present_seeds:
        for task in present_tasks:
            for form in present_forms:
                for adaptation in present_adaptations:
                    arms = [child_by[(seed, task, form, adaptation, key)] for key in design.NOISE_KEYS if (seed, task, form, adaptation, key) in child_by]
                    if len(arms) < 2:
                        continue
