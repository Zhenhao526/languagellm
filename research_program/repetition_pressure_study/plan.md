...
The formal matrix varies only the reuse pattern of task meanings while keeping four action stages, two private binary factors, capacity, partner rotation, channel noise and training budget fixed. `unique4` uses `[a, b, a xor b, a and b]`, `repeat2` uses `[a, a, b, b]`, and `shared4` uses `[a xor b]` in all four stages. The task factor is crossed with `dual2`, `triple2`, `atomic8`, `scratch`, `worker_only`, `coadapt`, and noise p=0, 0.10, 0.25.
