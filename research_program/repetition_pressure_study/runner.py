...
def prepare(out):
    ...
    plan = {"schema": "error_correcting_signaling_study_v1", "prepared_sha256": sha(out / "prepared.json"), "sources": sources, "runtime": {"python": platform.python_version(), "numpy": np.__version__}, "config": cfg}
...
def task_class_hamming(codebook):
    # Goals 00/11 have parity 0; goals 01/10 have parity 1.  We report the
    # closest cross-class distance and separately require within-class
    # agreement when labelling a repetition-like code.
...
def evaluate(params, seed, form, noise_p):
    episode = design.episode_stream(int(seed) + design.EVAL_SEED_OFFSET, design.WORKERS * 4096, form, evaluation=True)
    per_worker = {}
    ...
    class_hamming, within_class_hamming = task_class_hamming(codebook)
    new = per_worker["0"]
...
def _run_train(params, seed, form, noise_p, run, *, updates, train_sender, train_workers, partner_filter=None):
    ...
    for update in range(1, updates + 1):
        episode = design.episode_stream(seed, design.BATCH_SIZE, form, update=update)
...
def train_parent(seed, form, parents_root, updates=None):
    updates = design.PARENT_UPDATES if updates is None else int(updates)
    run = Path(parents_root) / f"seed_{seed}_{form}"
    run.mkdir(parents=True, exist_ok=False)
    params = policy.make_policy(int(seed) + 100000, form)
    initial_hash = policy.combined_parameter_hash(params)
    rows, checkpoints, log_hash = _run_train(params, seed, form, 0.0, run, updates=updates, train_sender=True, train_workers="all")
    final_checkpoint = run / f"checkpoint_{updates:04d}.npz"
    final = evaluate(params, seed, form, 0.0)
    result = {"seed": int(seed), "form": form, "updates": updates, "initial_parameter_sha256": initial_hash, "final_parameter_sha256": policy.combined_parameter_hash(params), "final_checkpoint_sha256": sha(final_checkpoint), "training_log_sha256": log_hash, "checkpoints": checkpoints, "final": final, "trajectory": rows}
...
def train_child(seed, condition, execution, parent_result, parent_params, updates=None):
    form, adaptation, noise_p, noise_key = design.parse_condition(condition)
    ...
    rows, checkpoints, log_hash = _run_train(params, seed, form, noise_p, run, updates=updates, train_sender=train_sender, train_workers=train_workers, partner_filter=None if adaptation == "scratch" else 0)
    final_checkpoint = run / f"checkpoint_{updates:04d}.npz"
    final = evaluate(params, seed, form, noise_p)
    result = {"seed": int(seed), "condition": condition, "form": form, "adaptation": adaptation, "noise_p": float(noise_p), "noise_key": noise_key, "updates": updates, "parent_parameter_sha256": parent_parameter_hash, "parent_sender_codebook": parent_codebook, "replaced_worker": replace_worker_flag, "initial_parameter_sha256": initial_hash, "final_parameter_sha256": policy.combined_parameter_hash(params), "final_checkpoint_sha256": sha(final_checkpoint), "training_log_sha256": log_hash, "checkpoints": checkpoints, "final": final, "trajectory": rows, "sender_codebook_distance_to_parent": codebook_distance(final["sender_codebook"], parent_codebook)}
    (run / "result.json").write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n")
    return result
...
def run_grid(prepared, out, updates=None, parent_updates=None, seeds=None, conditions=None):
    ...
    parent_records = []
    parent_by = {}
    total_parents = len(seeds) * len(set(design.parse_condition(c)[0] for c in conditions))
    parent_forms = sorted(set(design.parse_condition(c)[0] for c in conditions), key=design.FORMS.index)
    for seed in seeds:
        for form in parent_forms:
            record = train_parent(seed, form, parents, updates=parent_updates)
            parent_records.append(record)
            parent_by[(seed, form)] = record
            (out / "parent_progress.json").write_text(json.dumps({"completed": len(parent_records), "total": total_parents, "seeds": list(seeds), "forms": parent_forms}, indent=2))
    child_records = []
    total = len(seeds) * len(conditions)
    for seed in seeds:
        for condition in conditions:
            form = design.parse_condition(condition)[0]
            parent_path = parents / f"seed_{seed}_{form}" / f"checkpoint_{(design.PARENT_UPDATES if parent_updates is None else int(parent_updates)):04d}.npz"
            parent_params = load_checkpoint(parent_path)
            child_records.append(train_child(seed, condition, execution, parent_by[(seed, form)], parent_params, updates=updates))
            (execution / "progress.json").write_text(json.dumps({"completed": len(child_records), "total": total, "seeds": list(seeds), "conditions": list(conditions)}, indent=2))
