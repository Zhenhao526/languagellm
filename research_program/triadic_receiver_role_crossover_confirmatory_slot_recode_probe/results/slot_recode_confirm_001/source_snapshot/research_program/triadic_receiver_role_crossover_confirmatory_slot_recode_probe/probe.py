"""Slot-selective and fixed-token-recoding transfer over frozen policies."""
...
from research_program.triadic_receiver_role_crossover_confirmatory_study import runner as role_runner

from . import design

HERE = Path(__file__).resolve().parent
ROOT = HERE.parents[1]
SOURCE_RUN = ROOT / "research_program/triadic_receiver_role_crossover_confirmatory_study/results/role_crossover_confirm_001"
SOURCE_AUDIT = ROOT / "research_program/triadic_receiver_role_crossover_confirmatory_study/results/audit_role_crossover_confirm_001/verification.json"
...
def source_manifest():
    ...
    paths += [
        ...
        Path(action_environment.__file__), Path(execution_environment.__file__),
        Path(role_runner.__file__), Path(role_runner.design.__file__),
        SOURCE_RUN / "plan.json", SOURCE_RUN / "prepared.json", SOURCE_RUN / "freeze.json",
        SOURCE_RUN / "execution/results.json", SOURCE_AUDIT,
    ]
...
def frozen_inputs():
    role_runner.verify(SOURCE_RUN)
    audit = json.loads(SOURCE_AUDIT.read_text(encoding="utf8"))
    ...
    return dict(
        ...
        source_results_sha256=sha(SOURCE_RUN / "execution/results.json"), audit_sha256=sha(SOURCE_AUDIT), checkpoints=checkpoints,
    )
...
def prepared():
    ...
    intervention_samples = live_policy_count * (case_rows + 3 * design.SHAM_ROWS_PER_SENDER) * 12 * variant_count
    static["budget"] = dict(
        policy_rows=policy_count, variants=variant_count, natural_worlds=policy_count * natural_worlds,
...
def native_bank(networks, arrays, live):
    ...
    for start in range(0, n, design.CHUNK_SIZE):
        ...
        bank["action_probabilities"][start:stop] = joint_action_probabilities(trace["action_logits"])
...
def physical_metrics(states, probabilities, legal):
    ...
    return dict(
        physical=float(physical.mean()), q=float(q.mean()),
...
def transformed_packet(packet, recode):
    ...
    raise ValueError(f"Unknown recode: {recode}")


def masked_packets(receiver, donor, variant):
    receiver = np.asarray(receiver, dtype=np.int8)
    donor = transformed_packet(donor, variant["recode"])
...
def run_group(networks, arrays, bank, case_indices, cases, sender, legal_cache, pair_cache, live, variant):
    aligned_transfer = []
    placebo_transfer = []
    aligned_partner = []
    placebo_partner = []
    ...
    for start in range(0, len(receiver_ids), design.CHUNK_SIZE):
        ...
        ri = receiver_ids[start:stop]
        di = donor_ids[start:stop]
        pi = placebo_ids[start:stop]
        natural = bank["action_probabilities"][ri]
        states = bank["states"][ri]
        receiver_packet = bank["messages"][ri, 0, sender]
        aligned_packet = masked_packets(receiver_packet, bank["messages"][di, 0, sender], variant)
        placebo_packet = masked_packets(receiver_packet, bank["messages"][pi, 0, sender], variant)
        if live:
        ...
        for row in range(len(ri)):
            target = legal_cache[int(ri[row])]
            source = legal_cache[int(di[row])]
            donor_only = source - target
            ...
            target_pairs = pair_cache[int(ri[row])]
            source_pairs = pair_cache[int(di[row])]
            effects_a = []
            effects_p = []
            ...
            aligned_partner.append(_mean(effects_a))
            placebo_partner.append(_mean(effects_p))
        nm = physical_metrics(states, natural, [legal_cache[int(i)] for i in ri])
        am = physical_metrics(states, aligned, [legal_cache[int(i)] for i in ri])
        pm = physical_metrics(states, placebo, [legal_cache[int(i)] for i in ri])
        ...
        aligned_change.append(float(np.any(am["actions"] != nm["actions"], axis=1).mean()))
        placebo_change.append(float(np.any(pm["actions"] != nm["actions"], axis=1).mean()))
    return dict(
        variant=variant["name"], slots=list(variant["slots"]), recode=variant["recode"], rows=len(receiver_ids),
        aligned_plan_transfer=_mean(aligned_transfer), placebo_plan_transfer=_mean(placebo_transfer),
        aligned_minus_placebo_plan_transfer=_mean(np.asarray(aligned_transfer) - np.asarray(placebo_transfer)),
        aligned_partner_transfer=_mean(aligned_partner), placebo_partner_transfer=_mean(placebo_partner),
        aligned_minus_placebo_partner_transfer=_mean(np.asarray(aligned_partner) - np.asarray(placebo_partner)),
        natural_physical=_mean(natural_physical), aligned_physical=_mean(aligned_physical), placebo_physical=_mean(placebo_physical),
        natural_q=_mean(natural_q), aligned_q=_mean(aligned_q), placebo_q=_mean(placebo_q),
        natural_conditional_q=_mean(natural_cq), aligned_conditional_q=_mean(aligned_cq), placebo_conditional_q=_mean(placebo_cq),
        aligned_action_change=_mean(aligned_change), placebo_action_change=_mean(placebo_change),
    )


def run_policy(networks, arrays, cases, live):
    bank = native_bank(networks, arrays, live)
    legal_cache = legal_sets(bank["states"])
    pair_cache = pair_sets(bank["states"])
    sender = np.asarray(cases["sender"], dtype=np.int8)
    axis = np.asarray(cases["axis"], dtype=object)
    groups = {}
    for who in range(3):
        for axis_name in design.AXES:
            ids = np.flatnonzero((sender == who) & (axis == axis_name))
            for variant in design.VARIANTS:
                groups[f"{axis_name}/{who}/{variant['name']}"] = run_group(networks, arrays, bank, ids, cases, who, legal_cache, pair_cache, live, variant)
    sham = []
    for who in range(3):
        ...
        for variant in design.VARIANTS:
            if live:
                # Sham uses the unchanged own packet for every variant, including
                # recode variants, so this checks the intervention plumbing only.
                same = intervention.intervene(networks, arrays["x_PL"][ri], bank["messages"][ri], np.full(len(ri), who), receiver)
                natural = bank["action_probabilities"][ri]
                sham.append(dict(
                    sender=who, variant=variant["name"], rows=len(ids),
                    action_equal=bool(np.array_equal(same["action_indices"], natural.argmax(-1))),
                    message_equal=bool(np.array_equal(same["generated_messages"][:, 0], bank["messages"][ri, 0])),
                    max_probability_error=float(np.max(np.abs(same["action_probabilities"] - natural))),
                ))
            else:
    ...
    case_count = len(cases["receiver_state_indices"])
    per_variant = case_count + 3 * design.SHAM_ROWS_PER_SENDER
    return dict(
        worlds=len(bank["states"]), variants=list(design.VARIANT_NAMES),
        natural_module_samples=9 * len(bank["states"]),
        intervention_module_samples=per_variant * 12 * len(design.VARIANTS) if live else 0,
        groups=groups, sham=sham,
    )
...
def prepare(out):
    ...
    write(out / "plan.json", dict(
        status="prepared_without_probe_forward", created_at=core.base.now(),
        config=static.get("config", {}), prepared_sha256=sha(out / "prepared.json"),
        inputs_sha256=sha(out / "inputs.json"), source_sha256=static["source_sha256"],
        policy_blocks=static["policy_blocks"], variants=list(design.VARIANT_NAMES),
        no_training=True,
    ))
    ...
    verify(out)
    return dict(status="prepared_without_probe_forward", output=str(out), case_rows=static["cases"]["case_count"], variants=list(design.VARIANT_NAMES))


def verify(out):
    out = Path(out).resolve()
    static = json.loads((out / "prepared.json").read_text(encoding="utf8"))
    ...
    require(sha(out / "prepared.json") == freeze["prepared_sha256"], "Prepared hash mismatch")
    require(sha(out / "plan.json") == freeze["plan_sha256"], "Plan hash mismatch")
    require(sha(out / "inputs.json") == freeze["inputs_sha256"], "Inputs hash mismatch")
    require(static == prepared(), "Static preparation changed")
    require(inputs == frozen_inputs(), "Frozen source changed")
...
def worker(payload):
    ...
    out = execution / f"seed_{seed}"
    out.mkdir(parents=True, exist_ok=False)
    arrays = make_arrays(static["partition"])
    rows = []
    for condition in design.CONDITIONS:
        meta = inputs["checkpoints"][f"{seed}:{condition}"]
        path = Path(meta["path"])
        require(sha(path) == meta["sha256"], "Checkpoint binding mismatch")
        networks = source_runner.load_networks(path)
        role, schedule, live = role_runner.design.parse_condition(condition)
        started = time.perf_counter()
        summary = run_policy(networks, arrays, static["cases"], live)
        row = dict(seed=seed, role=role, condition=condition, schedule=schedule, live=bool(live), checkpoint_sha256=sha(path), parameter_sha256=source_runner.parameter_hash(networks), elapsed_seconds=time.perf_counter() - started, **summary)
        rows.append(row)
        write(out / f"{condition}.json", row)
    return rows
...
def execute(out, workers=4):
    ...
    try:
        ...
        result = dict(
            status="completed_role_crossover_slot_recode_probe", created_at=core.base.now(),
            elapsed_seconds=time.perf_counter() - started, plan_sha256=sha(out / "plan.json"),
            rows=rows, policy_blocks=len(rows), case_count=static["cases"]["case_count"],
            worlds=static["partition"]["world_count"], variants=list(design.VARIANT_NAMES),
            model_forward_samples=static["budget"]["total_new_module_samples"], optimizer_updates=0,
            no_training_updates=True, posthoc=True,
        )
        write(execution / "results.json", result)
        result_sha = sha(execution / "results.json")
        write(execution / "status.json", dict(status="completed", results_sha256=result_sha))
        write(execution / "receipt.json", dict(status="passed", results_sha256=result_sha, policy_blocks=len(rows), case_count=static["cases"]["case_count"], variants=list(design.VARIANT_NAMES), optimizer_updates=0))
        return result
