...
def error(expected, actual):
    if isinstance(expected, list):
        require(isinstance(actual, list) and len(expected) == len(actual), "Replay list mismatch")
        return max((error(left, right) if isinstance(left, (dict, list)) else (abs(float(left) - float(right)) if isinstance(left, (int, float)) else (0.0 if left == right else float("inf"))) for left, right in zip(expected, actual)), default=0.0)
