"""Multi-round private-demand cooperation environment and causal controls."""
    ...
    for t in range(H):
      ...
      for i in range(n):
        ...
        for site in (0,1):
          ...
          for a in actors:
            goal=int(episodes['goal'][i,1-a,t]); typ=int(site_types[i,site])
            if a in winners and typ==goal: rewards[i,t,a]=design.CORRECT_REWARD; last[i,a]=1
            elif a in winners: rewards[i,t,a]=design.WRONG_REWARD; last[i,a]=2
            else: rewards[i,t,a]=design.WRONG_REWARD; last[i,a]=2
                    ...
                    for a1 in actions:
                        ...
                        for a, act in enumerate(chosen):
                            if act == 0:
                                continue
                            site = act - 1
                            correct = site_types[:, site] == goals[:, 1-a, t]
                            reward += np.where(wins[a] & correct,
                                               design.CORRECT_REWARD,
                                               design.WRONG_REWARD)
