...
def summarize(payload):
    ...
    cells = {}
    for row in rows:
        key = (row["architecture"], row.get("surface_mapping", "identity"), row["support"])
        final = row["final"]
    ...
    out = {}
    for (architecture, mapping, support), cell in sorted(cells.items()):
        endpoint = {}
        for name in ("old_seen", "old_combo", "new_single", "new_double", "new_double_recombined", "initial_new_double"):
            ...
            endpoint[name] = {"mean": float(np.mean(values)), "ci95": _ci(values),
                              ...
                              "values": values}
        out[f"{architecture}/{mapping}/{support}"] = {
            "architecture": architecture, "surface_mapping": mapping, "support": support, "endpoint": endpoint,
        ...
        out[f"{architecture}/{mapping}/{support}"]["repair_gain"] = {
    ...
    for architecture in design.ARCHITECTURES:
      for mapping in design.OBJECT_SURFACE_MAPPINGS:
        def vals(support, name):
            return np.asarray(out[f"{architecture}/{mapping}/{support}"]["endpoint"][name]["values"], dtype=np.float64)
        ...
        for support in ("expanded_single_sender", "expanded_single_receiver", "expanded_single_none"):
            ...
            contrasts[f"{architecture}/{mapping}:pair-minus-{support}:new_double"] = {
                "mean": float(diff.mean()), "ci95": _ci(diff), "values": diff.tolist()
            }
      for support in design.TRANSFER_SUPPORTS:
        identity = np.asarray(out[f"{architecture}/identity/{support}"]["endpoint"]["new_double"]["values"], dtype=np.float64)
        reverse = np.asarray(out[f"{architecture}/reverse/{support}"]["endpoint"]["new_double"]["values"], dtype=np.float64)
        diff = reverse - identity
        contrasts[f"{architecture}:reverse-minus-identity/{support}:new_double"] = {
            "mean": float(diff.mean()), "ci95": _ci(diff), "values": diff.tolist()
        }
    return {"schema": "open_world_grounding_transfer_analysis_v1",
