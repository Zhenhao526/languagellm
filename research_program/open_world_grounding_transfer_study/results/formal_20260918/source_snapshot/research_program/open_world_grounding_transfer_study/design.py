"""Open-world cultural transmission under a child-only surface remapping.
...
extension and replaces one role at a time. A fresh receiver can see either the
canonical object surfaces or a stable reversal of all attribute labels. The
double-new meaning is absent from every training support, so transfer is tested
as zero-shot composition after cultural learning rather than as inheritance of
parameters.
"""
...
TRANSFER_CONDITIONS = tuple(
    f"{architecture}_{population}_{visibility}_{mapping}_{support}"
    for architecture in ARCHITECTURES
    for population in POPULATIONS
    for visibility in VISIBILITIES
...
def parse_transfer_condition(condition):
    ...
    population, visibility, mapping, support = parts
    require(population in POPULATIONS and visibility in VISIBILITIES
            and mapping in OBJECT_SURFACE_MAPPINGS and support in TRANSFER_SUPPORTS,
            f"bad transfer condition {condition}")
    return architecture, population, visibility, mapping, support
...
def prepare():
    return {
        "schema": "open_world_grounding_transfer_study_v1",
        "task": TASK,
