"""Train the expansion stage and the frozen-incumbent transfer stage."""
...
def source_hashes():
    rels = ("__init__.py", "README.md", "design.py", "policy.py", "environment.py", "runner.py",
            "aggregate.py", "audit.py", "transfer_audit.py", "transfer_analysis.py", "plan.md",
            "tests/test_game.py")
...
def prepare(out):
    ...
    plan = {"schema": "open_world_grounding_incumbent_study_v1", "prepared_sha256": sha(out / "prepared.json"),
...
def _receiver_update(grad, tr, ep, mask, params, visibility):
    ...
    partner = np.asarray(ep["partner_id"], dtype=np.int64)[idx]
    # The fresh receiver observes the child-specific surface mapping; parent
    # and incumbent updates remain on canonical semantic scenes.
    scene_source = ep.get("fresh_scene_meanings", ep["scene_meanings"])
    scene = np.asarray(scene_source, dtype=np.int64)[idx]
...
def _stream_hashes(ep):
    return {"scene_sha256": design.array_sha(ep["scene_meanings"]),
            "fresh_scene_sha256": design.array_sha(ep.get("fresh_scene_meanings", ep["scene_meanings"])),
...
def _eval_episode(seed, goal_kind, support, combo, surface_mapping="identity"):
    return design.balanced_eval_stream(seed + design.EVAL_SEED_OFFSET,
                                       design.PARTNERS * 2 * 512,
                                       goal_kind, support, combo, surface_mapping)
...
def evaluate_parent(params, seed, architecture, population, community):
    ...
    for kind in design.EVAL_KINDS:
        support = "old_combo" if kind in ("old_seen", "old_combo") else "expanded_single_pair"
...
def _metric_subset(tr, mask):
    ...
    if len(values) == 0:
        return {"episodes": 0, "team_return_mean": None, "team_return_sd": None,
                "positive_episode_rate": None, "oracle_return_mean": float(environment.oracle_return())}
    return {"episodes": int(len(values)), "team_return_mean": float(values.mean()),
            "team_return_sd": float(values.std()), "positive_episode_rate": float((values > 0).mean()),
            "oracle_return_mean": float(environment.oracle_return())}
...
def evaluate_child(communities, fresh, seed, architecture, population, visibility, support,
    ...
    for kind in design.EVAL_KINDS:
        ep = _eval_episode(seed, kind, support, combo, surface_mapping)
    ...
    result["surface_mapping"] = surface_mapping
    return result
...
def transfer_path(execution, seed, condition, updates=None):
    updates = design.CHILD_UPDATES if updates is None else int(updates)
    return (Path(execution) / "transfers" / f"seed_{seed}_{condition}"
            / f"checkpoint_{updates:04d}.npz")


def expansion_checkpoint(expansion_execution, seed, architecture, population,
                         visibility="hidden", updates=None):
    condition = f"{architecture}_{population}_{visibility}_expanded_single_pair"
    run = Path(expansion_execution) / "children" / f"seed_{seed}_{condition}"
    if updates is None:
        result_path = run / "result.json"
        design.require(result_path.is_file(), f"missing expansion result {result_path}")
        updates = int(json.loads(result_path.read_text())["updates"])
    return run / f"checkpoint_{int(updates):04d}.npz"


def train_transfer(seed, condition, execution, incumbent_checkpoint, updates=None):
    """Train one fresh role against a frozen expanded incumbent."""
    architecture, population, visibility, surface_mapping, receiver_initialization, support = design.parse_transfer_condition(condition)
    updates = design.CHILD_UPDATES if updates is None else int(updates)
    if support.endswith("_none"):
        updates = 0
    run = Path(execution) / "transfers" / f"seed_{seed}_{condition}"
    run.mkdir(parents=True, exist_ok=False)
    incumbent_communities, incumbent_fresh = load_checkpoint(incumbent_checkpoint, architecture)
    design.require(incumbent_fresh is not None, "expansion checkpoint has no fresh incumbent")
    # The expanded child is the incumbent culture. Two identical community
    # copies preserve hidden partner rotation without allowing the incumbent
    # parameters to update.
    incumbent = policy.clone(incumbent_fresh)
    communities = [policy.clone(incumbent), policy.clone(incumbent)]
    fresh = policy.make_policy(seed + 191000 + 7000, architecture,
                               visible=visibility == "visible")
    if receiver_initialization == "incumbent_receiver":
        # Copy only the receiver side. The fresh sender remains random, so the
        # pair arm still tests whether a grounded receiver and a new sender
        # can recreate a held-out joint meaning.
        for key in tuple(fresh):
            if key.startswith("receiver_"):
                fresh[key] = np.asarray(incumbent[key]).copy()
        if architecture == "tied_routed":
            raise ValueError("tied_routed is excluded from the incumbent-only intervention")
    incumbent_hash = policy.parameter_hash(incumbent)
    initial = policy.parameter_hash(fresh)
    initial_eval = evaluate_child(communities, fresh, seed, architecture, population,
                                  visibility, support, surface_mapping)
    checkpoints = sorted(set([u for u in design.CHECKPOINTS if u <= updates] + [updates]))
    if 0 in checkpoints:
        save_checkpoint(run / "checkpoint_0000.npz", communities, fresh, 0)
    rows = []
    ...
    for update in range(1, updates + 1):
        ...
        design.require(not np.any(np.asarray(ep["goal_meaning"]) == design.NEW_VALUE * design.VALUES + design.NEW_VALUE),
                       f"double-new leakage {condition}/{update}")
        tr = environment.rollout(communities, fresh, ep, architecture, population, visibility)
        grad = child_gradient(fresh, ep, tr, visibility)
        norm, scale = update_params(fresh, grad)
        row = {"update": update, "seed": seed, "condition": condition,
               "architecture": architecture, "population": population,
               "visibility": visibility, "surface_mapping": surface_mapping,
               "receiver_initialization": receiver_initialization,
               "support": support, **_stream_hashes(ep),
               "return_mean": float(tr["team_return"].mean()),
               "fresh_sender_count": int(np.asarray(tr["sender_fresh"]).sum()),
               "fresh_receiver_count": int(np.asarray(tr["receiver_fresh"]).sum()),
               "gradient_norm": norm, "gradient_clip_scale": scale,
               "parameter_sha256": policy.parameter_hash(fresh),
               "community_parameter_sha256": [policy.parameter_hash(params) for params in communities],
               "incumbent_parameter_sha256": incumbent_hash,
               "elapsed_seconds": time.perf_counter() - start}
        design.require(row["community_parameter_sha256"] == [incumbent_hash, incumbent_hash],
                       "incumbent changed during transfer")
        if update in checkpoints:
            row["checkpoint_sha256"] = save_checkpoint(run / f"checkpoint_{update:04d}.npz",
                                                        communities, fresh, update)
        rows.append(row)
    log = run / "training.jsonl"
    log.write_bytes(b"".join(json_bytes(row) for row in rows))
    result = {"kind": "transfer", "stage": "incumbent_replacement", "seed": seed,
              "condition": condition, "architecture": architecture,
              "population": population, "visibility": visibility,
              "surface_mapping": surface_mapping, "support": support,
              "receiver_initialization": receiver_initialization,
              "incumbent_checkpoint": str(incumbent_checkpoint),
              "incumbent_checkpoint_sha256": sha(incumbent_checkpoint),
              "incumbent_parameter_sha256": incumbent_hash,
              "initial_parameter_sha256": initial, "final_parameter_sha256": policy.parameter_hash(fresh),
              "final_community_parameter_sha256": [policy.parameter_hash(params) for params in communities],
              "initial": initial_eval,
              "final": evaluate_child(communities, fresh, seed, architecture, population, visibility,
                                       support, surface_mapping),
              "updates": updates, "checkpoints": checkpoints, "training_log_sha256": sha(log),
              "final_checkpoint": str(run / f"checkpoint_{updates:04d}.npz"),
              "final_checkpoint_sha256": sha(run / f"checkpoint_{updates:04d}.npz")}
    (run / "result.json").write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n")
    return result
...
def run_transfer_grid(prepared, expansion_execution, out, updates=None, seeds=None,
                      conditions=None):
    """Run role-replacement children from frozen expansion checkpoints."""
    _, cfg = verify(prepared)
    out = Path(out).resolve()
    execution = out / "execution"
    (execution / "transfers").mkdir(parents=True, exist_ok=True)
    seeds = tuple(design.SEEDS if seeds is None else seeds)
    conditions = tuple(design.TRANSFER_CONDITIONS if conditions is None else conditions)
    design.require(set(seeds) <= set(design.SEEDS)
                   and set(conditions) <= set(design.TRANSFER_CONDITIONS),
                   "invalid transfer subset")
    transfers = []
    for seed in seeds:
        for condition in conditions:
            architecture, population, visibility, _, _, _ = design.parse_transfer_condition(condition)
            source = expansion_checkpoint(expansion_execution, seed, architecture,
                                          population, visibility)
            design.require(source.is_file(), f"missing expansion checkpoint {source}")
            transfers.append(train_transfer(seed, condition, execution, source, updates))
            (execution / "progress.json").write_text(json.dumps(
                {"completed_transfers": len(transfers),
                 "total_transfers": len(seeds) * len(conditions),
                 "seeds": list(seeds), "conditions": list(conditions)}, indent=2))
    payload = {"transfers": transfers,
               "expansion_execution": str(Path(expansion_execution).resolve()),
               "expansion_conditions": list(design.CHILD_CONDITIONS)}
    (execution / "results.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n")
    return payload
...
if __name__ == "__main__":
    ...
    trn = sub.add_parser("transfer")
    trn.add_argument("--out", required=True)
    trn.add_argument("--prepared", required=True)
    trn.add_argument("--expansion-execution", required=True)
    trn.add_argument("--updates", type=int)
    trn.add_argument("--seeds")
    trn.add_argument("--conditions")
    ...
    if args.cmd == "prepare":
        ...
        seeds = None if args.seeds is None else tuple(int(x) for x in args.seeds.split(",") if x)
        conditions = None if args.conditions is None else tuple(x for x in args.conditions.split(",") if x)
        payload = run_transfer_grid(args.prepared, args.expansion_execution, args.out,
                                    args.updates, seeds, conditions)
        print(json.dumps({"status": "completed", "transfers": len(payload["transfers"])},
                         ensure_ascii=False))
