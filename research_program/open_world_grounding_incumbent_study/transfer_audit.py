...
def audit(prepared, execution):
    ...
    def get(seed, architecture, mapping, initialization, support):
        return logs.get((int(seed), f"{architecture}_aligned_hidden_{mapping}_{initialization}_{support}"))
            ...
            for initialization in design.RECEIVER_INITIALIZATIONS:
                for mapping in design.OBJECT_SURFACE_MAPPINGS:
                    for left, right in combinations(supports, 2):
                        paired_rows += _pair(get(seed, architecture, mapping, initialization, left),
                                             get(seed, architecture, mapping, initialization, right), stream_fields)
                for support in supports:
                    paired_rows += _pair(get(seed, architecture, "identity", initialization, support),
                                         get(seed, architecture, "reverse", initialization, support),
                                         [field for field in stream_fields if field != "fresh_scene_sha256"])
            for mapping in design.OBJECT_SURFACE_MAPPINGS:
                for support in supports:
                    paired_rows += _pair(get(seed, architecture, mapping, "fresh", support),
                                         get(seed, architecture, mapping, "incumbent_receiver", support), stream_fields)

    expected = len(design.SEEDS) * len(design.TRANSFER_CONDITIONS)
    complete = len(payload["transfers"]) == expected
    return {
        "schema": "open_world_grounding_incumbent_audit_v1",
