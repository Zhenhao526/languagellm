...
from research_program.open_world_grounding_incumbent_study import design, environment, policy, runner


def main():
    assert design.parse_child_condition("factorized_aligned_hidden_expanded_single_pair") == (
        "factorized", "aligned", "hidden", "expanded_single_pair"
    )
    assert design.parse_transfer_condition("factorized_aligned_hidden_reverse_incumbent_receiver_expanded_single_sender") == (
        "factorized", "aligned", "hidden", "reverse", "incumbent_receiver", "expanded_single_sender"
    )
    ...
    incumbent = policy.make_policy(9003, "factorized")
    fresh = policy.make_policy(9004, "factorized")
    for key in tuple(fresh):
        if key.startswith("receiver_"):
            fresh[key] = incumbent[key].copy()
    assert np.array_equal(fresh["receiver_hidden"], incumbent["receiver_hidden"])
    assert not np.array_equal(fresh["sender_hidden"], incumbent["sender_hidden"])
    print("open_world_grounding_incumbent_study tests passed")
