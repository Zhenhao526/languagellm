"""Tabular sender and worker policies for the remapping study."""
...
def make_policy(seed, form, representation="joint_history", *, sender_logits=None):
    ...
    if representation == "slot_local":
        design.require(form == "dual2", "slot-local is only defined for dual2")
    k = design.alphabet_size(form)
    length = design.message_length(form)
    rng = np.random.default_rng(
        np.random.SeedSequence([int(seed), 883, k, length, int(representation == "slot_local")])
    )
    sender = (
        rng.normal(0.0, 0.02, size=(len(design.GOAL_PAIRS), length, k))
        if sender_logits is None
        else np.asarray(sender_logits, dtype=np.float64).copy()
    )
    workers = rng.normal(
        0.0,
        0.02,
        size=(
            design.WORKERS,
            design.state_count(form, representation),
            design.HORIZON,
            design.SCENE_COUNT,
