"""Train, evaluate, freeze and replay the heterogeneous-grounding study."""
...
def prepare(out):
    ...
    plan = {"schema": "heterogeneous_grounding_study_v1", "prepared_sha256": sha(out / "prepared.json"), "sources": sources, "runtime": {"python": platform.python_version(), "numpy": np.__version__}, "config": cfg}
...
def _eval_episode(seed, task, goal_kind, heldout, population_mode, mapping=None):
    return design.balanced_eval_stream(seed + design.EVAL_SEED_OFFSET, design.WORKERS * 4096, goal_kind, task, heldout, population_mode, mapping)


def evaluate(params, seed, population_mode, form, task, representation, channel, heldout, mapping=None, *, partner_filter=None):
    ...
    for goal_kind in ("all", "seen", "heldout"):
        ep = _eval_episode(seed, task, goal_kind, heldout, population_mode, mapping)
        natural = environment.rollout(params, ep, form, task, representation, channel, sample=False, message_mode="natural", partner_filter=partner_filter)
        modes = {"natural": _metric(natural)}
        if channel == "live":
            permuted = environment.rollout(params, ep, form, task, representation, channel, sample=False, message_mode="permuted", partner_filter=partner_filter)
            modes["permuted"] = _metric(permuted)
            if form == "dual2":
                raw = environment.rollout(params, ep, form, task, representation, channel, sample=False, message_override=environment.recombination_override(params, ep, form, task, basis="raw"), partner_filter=partner_filter)
...
def parent_path(execution, seed, population_mode, form, task, updates=None):
    updates = design.PARENT_UPDATES if updates is None else int(updates)
    return Path(execution) / "parents" / f"seed_{seed}_{population_mode}_{form}_{task}" / f"checkpoint_{updates:04d}.npz"
...
def train_parent(seed, population_mode, form, task, execution, updates=None):
    updates = design.PARENT_UPDATES if updates is None else int(updates)
    run = Path(execution) / "parents" / f"seed_{seed}_{population_mode}_{form}_{task}"
    ...
    for update in range(1, updates + 1):
        ep = design.episode_stream(seed, design.BATCH_SIZE, task=task, update=update, support="full", population_mode=population_mode)
        ...
        row = {"update": update, "seed": seed, "population_mode": population_mode, "form": form, "task": task, "mapping": "identity", **_stream_hashes(ep), "return_mean": float(tr["team_return"].mean()), "gradient_norm": norm, "gradient_clip_scale": scale, "parameter_sha256": policy.parameter_hash(params), "elapsed_seconds": time.perf_counter() - start}
    ...
    result = {"kind": "parent", "seed": seed, "population_mode": population_mode, "form": form, "task": task, "mapping": "identity", "representation": "joint_history", "updates": updates, "initial_parameter_sha256": initial_hash, "final_parameter_sha256": policy.parameter_hash(params), "final": evaluate(params, seed, population_mode, form, task, "joint_history", "live", heldout, None, partner_filter=None), "heldout_goal": heldout, "checkpoints": checkpoints, "training_log_sha256": sha(log), "final_checkpoint": str(run / f"checkpoint_{updates:04d}.npz"), "final_checkpoint_sha256": sha(run / f"checkpoint_{updates:04d}.npz")}
...
def train_child(seed, condition, execution, parent_checkpoint, updates=None):
    ...
    initial_hash = policy.parameter_hash(params)
    heldout = design.heldout_goal(seed)
    initial_eval = evaluate(params, seed, population_mode, form, task, representation, channel, heldout, mapping, partner_filter=design.TARGET_WORKER)
    checkpoints = sorted(set([u for u in design.CHECKPOINTS if u <= updates] + [updates]))
    if 0 in checkpoints:
    ...
    start = time.perf_counter()
    for update in range(1, updates + 1):
        ep = design.episode_stream(seed, design.BATCH_SIZE, task=task, update=update, support=support, heldout=heldout, population_mode=population_mode, child_mapping=mapping)
...
def run_grid(prepared, out, updates=None, seeds=None, conditions=None):
    ...
    parent_conditions = sorted({(design.parse_child_condition(c)[0], design.parse_child_condition(c)[2], design.parse_child_condition(c)[3]) for c in conditions})
    parent_results = []
    for seed in seeds:
        for population_mode, form, task in parent_conditions:
            parent_results.append(train_parent(seed, population_mode, form, task, execution, updates))
    child_results = []
    for seed in seeds:
        for condition in conditions:
            population_mode, _, form, task, _, _, _ = design.parse_child_condition(condition)
            child_results.append(train_child(seed, condition, execution, parent_path(execution, seed, population_mode, form, task, updates), updates))
