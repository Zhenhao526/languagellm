"""Independent replay of the heldout joint-combination probe."""
...
def arrays(spec):
    a=dataset.make_arrays(spec,information="PL"); require(a["x_PL"].shape==(spec["world_count"],3,54),"PL shape"); require(np.all(a["x_PL"][:,:,53]==0),"FI leak"); return a


def probs(logits):
    intent,_,proposal,_=kernel.factorized_distribution(logits); out=np.zeros((len(logits),3,17)); out[:,:,0]=intent[:,:,0]; out[:,:,1:]=intent[:,:,1,None]*proposal; require(np.allclose(out.sum(-1),1,atol=1e-12,rtol=0),"normalization"); return out


def greedy(logits):
    intent,_,proposal,_=kernel.factorized_distribution(logits); return np.where(intent.argmax(-1)==1,proposal.argmax(-1)+1,0).astype(np.int16)
...
def bank(networks,a,live):
    ...
    require(np.array_equal(act,p.argmax(-1)),"natural action mismatch"); return dict(states=a["packed_states"].copy(),messages=msg,probs=p,actions=act)
...
def pairs(states):
    out=[]
    for row in states:
        out.append({tuple(plan[:2]) for plan in action_environment.full_success_plans(tuple(map(int,row[:3])),tuple(map(int,row[3:7])))})
    return out


def mass(p,actions):
    if not actions: return np.zeros(len(p))
    out=np.zeros(len(p))
    for act in actions: out += p[:,0,act[0]]*p[:,1,act[1]]*p[:,2,act[2]]
    return out


def partner(p,sender,recipient): return p[:,recipient,execution_environment.PROPOSAL_ROLES[recipient]==sender].sum(1)
...
def physical(states,p,allowed):
    ...
    return float(executed.mean()),float(q.mean()),float(q[executed].mean()) if executed.any() else 0.,act
...
def empty_group():
    """Match the lower-level runner's explicit zero-row destination group."""
    return dict(rows=0, aligned_plan_transfer=0.0, placebo_plan_transfer=0.0,
                aligned_minus_placebo_plan_transfer=0.0,
                aligned_partner_transfer=0.0, placebo_partner_transfer=0.0,
                aligned_minus_placebo_partner_transfer=0.0,
                natural_physical=0.0, aligned_physical=0.0,
                placebo_physical=0.0, natural_q=0.0, aligned_q=0.0,
                placebo_q=0.0, natural_conditional_q=0.0,
                aligned_conditional_q=0.0, placebo_conditional_q=0.0,
                aligned_action_change=0.0, placebo_action_change=0.0)
    ...
    for start in range(0,len(rid),2048):
        ...
        for j in range(len(r)):
            ...
            for recipient in range(3):
                ...
                if direction: ea.append(direction*(partner(aligned[j:j+1],sender,recipient)[0]-partner(nat[j:j+1],sender,recipient)[0])); ep.append(direction*(partner(placebo[j:j+1],sender,recipient)[0]-partner(nat[j:j+1],sender,recipient)[0]))
...
def replay(task):
    probe=Path(task["probe"]); static=json.loads((probe/"prepared.json").read_text()); inputs=json.loads((probe/"inputs.json").read_text()); cases=static["cases"]; path=Path(inputs["checkpoints"][f"{task['seed']}:{task['condition']}"]["path"]); require(sha(path)==task["checkpoint_sha256"],"checkpoint hash"); networks=source_runner.load_networks(path); a=arrays(static["heldout_spec"]); bk=bank(networks,a,bool(task["live"])); legal_cache=legal(bk["states"]); pair_cache=pairs(bk["states"]); sender=np.asarray(cases["sender"],dtype=np.int8); axis=np.asarray(cases["axis"],dtype=object); groups={}
    for who in range(3):
        for name in design.AXES: groups[f"{name}/{who}"]=group(networks,a,bk,np.flatnonzero((sender==who)&(axis==name)),cases,who,legal_cache,pair_cache,bool(task["live"]))
        # The shared lower-level runner always emits destination groups.  This
        # holdout design has no destination cases, so the corresponding rows
        # are explicit zeros rather than omitted keys.
...
def main(probe,out,workers=4):
    probe=Path(probe).resolve(); out=Path(out).resolve(); require(not out.exists(),"Refuse audit"); result=json.loads((probe/"execution/results.json").read_text()); require(result["status"]=="completed_compositional_holdout_semantic_transfer_probe" and len(result["rows"])==32,"result grid")
    tasks=[]
    ...
    out.mkdir(parents=True); verification=dict(status="passed",probe=str(probe),policy_blocks=32,rows_replayed=32*3456,live_rows_replayed=16*3456,worlds_per_policy=11232,model_forward_samples=result["model_forward_samples"],optimizer_updates=0,max_abs_error=float(max_abs),checkpoint_hashes_checked=32,placebo_mapping_checks=3456,independent_replay="Producer implementation not imported; natural bank, heldout legal sets, aligned/placebo routes, plan mass, partner metrics and strict settlement replayed independently")
    (out/"verification.json").write_text(json.dumps(verification,ensure_ascii=False,sort_keys=True,indent=2)+"\n",encoding="utf8"); receipt=dict(status="passed",verification_sha256=sha(out/"verification.json"),rows_replayed=32*3456,max_abs_error=float(max_abs)); (out/"receipt.json").write_text(json.dumps(receipt,ensure_ascii=False,sort_keys=True,indent=2)+"\n",encoding="utf8"); print(json.dumps(verification,ensure_ascii=False))
