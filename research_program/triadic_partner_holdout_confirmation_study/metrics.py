...
def _cells_for_seed(by, seed, rule, key):
    ...
    for payoff in ('partial', 'all_or_nothing'):
        for live in (False, True):
            rows = sorted(by[seed, payoff, rule, live]['trajectory'], key=lambda row: row['update'])
            values[payoff, live] = np.array([row['target_trajectory'][key] for row in rows], dtype=np.float64)
    return values
