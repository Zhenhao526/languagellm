"""Recover a completed partner-holdout grid from per-run JSON files only."""
...
def main(source, output):
    ...
    if len(runs) != 128:
        raise ValueError('Expected 128 completed runs')
    result = dict(status='completed_after_json_only_aggregation', source=str(source), runs=runs,
                  ...
                  recovery_note='Read per-run result.json files after the training workers completed; no checkpoints or model outputs were read.')
    ...
    receipt = dict(status='passed', input_runs=len(runs), model_forwards=0, optimizer_updates=0,
                   output_results_sha256=__import__('hashlib').sha256(result_path.read_bytes()).hexdigest())
