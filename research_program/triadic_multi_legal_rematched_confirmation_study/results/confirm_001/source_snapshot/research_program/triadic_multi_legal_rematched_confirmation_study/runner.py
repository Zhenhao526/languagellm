"""Train and evaluate multi-legal coordination with static or rematched needs."""
...
from research_program.triadic_multi_legal_coordination_study import kernel as multi_kernel
from research_program.triadic_rematched_partner_confirmation_study import remap as base_remap
from . import design, metrics, remap
...
CONFIG = dict(
    ...
    conditions=list(CONDITIONS), partitions=list(PARTS), task='multi_legal_coordination_under_need_rematching',
    ...
    primary='rematched_by_communication_interaction_on_Q_conditional_on_physical_execution_centered_time_AUC',
    ...
    selection='fixed16 new seeds ×2 schedules ×2 communication channels ×6000 updates; no early stopping',
...
def sources():
    paths = [HERE / n for n in ('__init__.py', 'design.py', 'remap.py', 'metrics.py', 'runner.py',
                                'plan.md', 'README.md', 'tests/test_design.py')]
    paths += [Path(dataset.__file__), Path(environment.__file__), Path(core.__file__), Path(core.base.__file__),
              Path(multi_design.__file__), Path(multi_kernel.__file__), Path(base_remap.__file__)]
...
def prepare(out):
    ...
    verify(out)
    return dict(status='prepared_without_training', output=str(out), plan_sha256=core.base.sha(out / 'plan.json'), budget=static['budget'])
...
def verify(out):
    ...
    require(core.base.sha(out / 'plan.json') == freeze['plan_sha256'] and
            core.base.sha(out / 'prepared.json') == freeze['prepared_sha256'] == plan['prepared_sha256'],
...
def make_arrays(spec):
    ...
    arrays['rewards_partial_multi'] = arrays['rewards'].copy()
    ...
    require(np.all((arrays['native_rewards'] == 1).sum(axis=1) == 2), 'Every multi-legal world must have two full plans')
    return arrays


def parameter_hash(networks):
    return core.base.json_hash({f'agent{a}_{module}_{key}': core.base.array_sha(value)
        for a in range(3) for m, module in enumerate(('sender1', 'sender2', 'action'))
...
def evaluate_multi(networks, arrays, spec, payoff, rule, live):
    ...
    for start in range(0, n, 1024):
        ...
        actual_pair = physical['actual_pair_index']; physical_ok = actual_pair >= 0
        ...
        q_count += int(valid.sum())
        third_wait_count += int(np.sum((actions == 0) * (1 - mask)))
    conditional_denominator_zero = int(physical_count == 0)
    conditional_q = float(q_count / physical_count) if physical_count else 0.0
    return dict(
        value=float(legal_actor_hits.sum() / max(legal_actor_total.sum(), 1)),
        ...
        q_rate=float(q_count / n), conditional_q_rate=conditional_q,
        physical_execution_rate=float(physical_count / n), third_actor_wait_rate=float(third_wait_count / n),
        legal_plan_selection_counts=plan_selection_counts.tolist(),
        legal_plan_selection_total=int(plan_selection_counts.sum()),
        conditional_q_denominator_worlds=int(physical_count),
        conditional_q_numerator_worlds=int(q_count),
        conditional_q_zero_denominator=conditional_denominator_zero,
        legal_plan_count=int(full.sum(axis=1).min()), worlds=n, partition=spec['partition'], payoff=payoff,
        rule=rule, live=bool(live))
...
def train_run(seed, condition, static, arrays, execution):
    ...
    train_spec = static['partitions']['train']; target_spec = train_spec
    ...
    def checkpoint(step):
        ...
        evaluated = evaluate_multi(networks, arrays['train'], target_spec, payoff, rule, live)
    ...
    with (directory / 'training.jsonl').open('x', encoding='utf-8') as stream:
        for update in range(1, CONFIG['updates'] + 1):
            ...
            canonical_x = arrays['train']['x_PL'][ids]; canonical_rewards = arrays['train']['rewards_partial_multi'][ids]
            ...
            gradients, row = multi_kernel.training_gradients(networks, x_batch, reward_batch, live, message_uniforms, update)
    ...
    result = dict(seed=seed, schedule=schedule, condition=condition, payoff=payoff, rule=rule, live=live, updates=CONFIG['updates'],
                  initial_parameter_sha256=initial, final_parameter_sha256=parameter_hash(networks),
                  final_checkpoint_sha256=core.base.sha(directory / 'checkpoint_6000.npz'),
                  training_log_sha256=core.base.sha(directory / 'training.jsonl'), trajectory=trajectory,
                  final={'new_layouts': final}, rematch_histogram=rematch_counts.tolist(),
...
def worker(payload):
    ...
    keys = ('packed_states', 'native_rewards', 'x_PL', 'rewards_partial_multi')
    ...
    require(before == after, 'Arrays mutated')
    return runs
        ...
        try:
            ...
            for lines in zip_longest(*streams):
                ...
                for schedule in SCHEDULES:
                    ...
                    require(len({row['effective_batch_states_sha256'] for row in group}) == 1 and
                            len({row['permutation_indices_sha256'] for row in group}) == 1,
                            'Live/silent rematch stream differs')
...
def measured_budget(runs):
    ...
    return dict(training_forward_module_samples=sum(run['updates'] for run in runs) * CONFIG['batch_size'] * 2 * 9,
                ...
                compact_evaluation_worlds=sum(row['compact_worlds'] for row in trajectory) + sum(row['worlds'] for row in final),
...
def execute(out):
    ...
    (execution / 'started.json').write_text(json.dumps(dict(started_at=core.base.now(), pid=os.getpid(),
                                                             plan_sha256=core.base.sha(out / 'plan.json')), ensure_ascii=False, indent=2) + '\n')
    try:
        ...
        require([(r['seed'], r['condition']) for r in runs] == [(seed, condition) for seed in SEEDS for condition in CONDITIONS],
        ...
        result = dict(status='completed', completed_at=core.base.now(), elapsed_seconds=time.perf_counter() - started,
                      ...
                      runs=runs, primary=metrics.summarize(runs),
                      experiment_type='multi_legal_coordination_with_random_need_rematching',
        ...
        (execution / 'status.json').write_text(json.dumps(dict(status='completed', at=core.base.now(),
                                                                results_sha256=core.base.sha(execution / 'results.json')), ensure_ascii=False, indent=2) + '\n')
        return dict(status='completed', output=str(execution), elapsed_seconds=result['elapsed_seconds'], primary=result['primary'])
    except BaseException as error:
        (execution / 'failure.json').write_text(json.dumps(dict(status='failed', at=core.base.now(), error=repr(error),
                                                                elapsed_seconds=time.perf_counter() - started), ensure_ascii=False, indent=2) + '\n')
        raise
