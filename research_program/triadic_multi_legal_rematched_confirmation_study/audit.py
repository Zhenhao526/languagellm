"""Independent checkpoint replay audit for conditional-Q trajectories."""
...
def main(source, output):
    ...
    runs = []; max_error = 0.0; checkpoints = 0; worlds = 0
    for seed in design.SEEDS:
        ...
        for condition in design.CONDITIONS:
            ...
            for row in run['trajectory']:
                ...
                networks = core.load_networks(path)
                replay = runner.evaluate_multi(networks, arrays['train'], static['partitions']['train'], payoff, rule, live)
                checkpoints += 1; worlds += replay['worlds']
                for key in ('value', 'target_pair_actor_legal_action_rate', 'q_rate', 'conditional_q_rate',
                            'physical_execution_rate', 'third_actor_wait_rate', 'conditional_q_denominator_worlds',
                            'conditional_q_numerator_worlds'):
                    max_error = max(max_error, abs(float(replay[key]) - float(row['target_trajectory'][key])))
                for actual, saved in zip(replay['actor_legal_action_rates'], row['target_trajectory']['actor_legal_action_rates']):
            ...
            require(len(histogram) == 6 and sum(histogram) == expected and run['rematch_assignments'] == expected,
    ...
    verification = dict(status='passed', source=str(source), runs=64, checkpoint_evaluations=checkpoints,
                         compact_worlds=worlds, model_forwards=checkpoints * 1452 * 18 * 6 * 9,
                         optimizer_updates=0, no_optimizer_updates=True, trajectory_replayed=True,
                         pairing_verified=True, max_abs_error=float(max_error), conditional_q_replayed=True,
                         legal_plan_multiplicity_verified=True, rematch_histograms_checked=True,
                         rematched_permutation_assignments=sum(run['rematch_assignments'] for run in runs))
