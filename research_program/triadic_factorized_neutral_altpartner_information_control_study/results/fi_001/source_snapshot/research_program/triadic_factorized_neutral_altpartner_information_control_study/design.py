...
INFORMATION = ('FI',)
CONDITIONS = tuple(
    f'{schedule}_altpair_factorized_strict_FI_{"live" if live else "silent"}'
...
def parse_condition(condition):
    ...
    require(schedule in SCHEDULES and suffix in ('altpair_factorized_strict_FI_live',
                                                  'altpair_factorized_strict_FI_silent'),
            'Malformed condition')
    return schedule, 'altpair_factorized', 'strict', 'FI', condition.endswith('_live')
...
def make_prepared():
    ...
    runs = [dict(seed=seed, schedule=schedule, payoff='altpair_factorized', rule='strict', live=live,
                 ...
                 directory=f'seed_{seed}_altpair_factorized_strict_FI_{"live" if live else "silent"}')
    ...
    return dict(
        ...
        schedules=list(SCHEDULES), information=['FI'], lives=['live', 'silent'], conditions=list(CONDITIONS),
        ...
        observations='FI: all three private needs and public layout are visible; no cross-agent message channel beyond the live/silent window.',
