...
def audit(folder):
    ...
    assert config["conditions"] == ["fixed"]
    assert len(output) == 7
    ...
    report = {"status": "passed", "runs": len(output), "trace_files": trace_count, "trace_cases": case_count,
              ...
              "limits": ["Six pair scores within one population share agents and are not independent replicates.",
                         "Only fixed training origins are audited here; the original cross pairs did not train together.",
    ...
    (folder / "独立新起点通信核查.json").write_text(json.dumps(report, ensure_ascii=False, indent=2))
    lines = ["# 独立新起点通信核查", "", f"核查通过：{len(output)} 个群体运行、{trace_count} 份轨迹、{case_count:,} 个最终评估案例。未训练或修改参数。", "",
    ...
    (folder / "独立新起点通信核查.md").write_text("\n".join(lines) + "\n")
    return report
...
if __name__ == "__main__":
    ...
    parser.add_argument("--directory", type=Path, default=ROOT / "results/mechanism_origins_001")
