"""Render v0.31 partner-schedule design and outcomes from independent analysis."""
...
def diagram(directory):
    ...
    fig.suptitle('v0.31：固定伙伴与轮换伙伴中的共同符号形成', fontsize=17, y=.98)
    fig.text(.5, .025, '两种条件共享目标图、训练图、视觉输入和采样流；只改变主体与伙伴的配对时序。', ha='center', fontsize=10.5, color='#3f4b57')
...
def result_figure(directory, analysis):
    ...
    fig.suptitle('v0.31：伙伴轮换促进跨主体复用，但多伙伴区分仍很弱', fontsize=17, y=.985)
