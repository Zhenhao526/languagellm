...
def restore_population(seed,p,prepared,source):
    ...
    for i,d in enumerate(PRIVATE_TYPES):
        a=remake_agents(seed,prepared,7,2,'identity')
...
def update_population(agents,opts,cache,table,seed,p,step,condition):
    ...
    for pair_no,(i,j) in enumerate(support.matching(condition,step)):
        for sender,receiver in ((i,j),(j,i)):
            d=PRIVATE_TYPES[sender];f=support.fixture(seed,p,d,step,condition,table);idx=f['indices']
            sl,rl,trace=communication.direction_loss(agents[sender],agents[receiver],cache['train',d][idx],f['uniforms'],table['positions'][idx],.02 if step<2100 else 0.)
