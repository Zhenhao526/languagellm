...
def audit(out,c):
    ...
    for seed,p in itertools.product(seeds,panels):
        ...
        for condition in stats.CONDITIONS:
            ...
            # Endpoint replay loads the final state into the shared audit copies;
            # restore the paired initialization before auditing the other condition.
            for a,st in zip(agents,initial):a.load_state_dict(st)
