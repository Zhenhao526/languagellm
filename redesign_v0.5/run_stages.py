...
def training_schedule(seed, updates, kind):
    """Shuffle whole pre-drawn batch identities, not just difficulty labels."""
    ...
    if kind == 'mixed':
        ...
        order[:boundary] = np.random.default_rng(seed + 7110000).permutation(order[:boundary])
    elif kind != 'course':
        levels[:] = 4
...
def training_batch_plan(seed, batch_identity, level, plan):
    sites = np.random.default_rng(seed + 7120000 + int(batch_identity)).permutation(4)[:int(level)]
    pool = [i for i, p in enumerate(MAPS) if set(p).issubset(set(sites))]
    return dict(plan, map_pool=pool, allowed_sites=sites.tolist())
...
def rollout(agents, banks, bank, plan, seed, n, *, training=False, greedy=True,
    ...
    for scout in range(2):
        ...
        for t in range(h):
            ...
            if training and 'allowed_sites' in plan:
                # A public physical-access constraint, identical for both agents.
                # The early curriculum changes which sites can be visited, not
                # a hidden resource-label answer. Its site subset is drawn first.
...
def train(seed, name, plan, agents, bank, out, updates, batch, eval_n, checkpoints, source=None):
    ...
    stage_offset = {'A': 0, 'B': 10000000, 'C': 20000000}[out.parent.name]
    ...
    write_json(out / 'config.json', dict(seed=seed, condition=name, plan=plan, updates=updates,
        ...
        entropy_coefficient=.02, entropy_off_after=updates - min(300, updates // 6), gamma=1,
        source_hashes={p.name: sha(p) for p in (ROOT / 'camp.py', ROOT / 'run_stages.py')},
        initial_sha256=init, warmstart_source=source, training_stage_seed_offset=stage_offset,
    ...
    levels, order = training_schedule(seed, updates, plan.get('schedule', 'direct'))
    write_json(out / 'training_schedule.json', dict(levels=levels.tolist(), batch_identities=order.tolist(),
        public_access='random site subset per batch; all four sites appear from earliest stage'))
    with (out / 'training.jsonl').open('w') as f:
        for update in range(updates + 1):
            ...
            if update == updates:
                break
            batch_identity = int(order[update])
            train_plan = training_batch_plan(seed, batch_identity, int(levels[update]), plan)
            stats, learning, _ = rollout(agents, banks, bank, train_plan,
                seed * 100000 + stage_offset + batch_identity + 1, batch, training=True, greedy=False, split='train')
            weight = .02 if update < updates - min(300, updates // 6) else 0.
            info = []
            for i in range(2):
            ...
            f.write(json.dumps(dict(update=update + 1, active_sites=int(levels[update]), reward=stats['mean_reward'],
                 batch_identity=batch_identity, allowed_sites=train_plan['allowed_sites'],
...
def main():
    ...
    manifest = dict(seeds=args.seeds, stages=args.stages, updates_a=args.updates_a, updates_b=args.updates_b,
        ...
        hashes={str(p): sha(p) for p in [ROOT/'camp.py', ROOT/'run_stages.py',
                ROOT/'渐进探索_固定执行方案.md', ROOT/'bounds_audit_verified.json',
    ...
    snapshot = args.out / ('sources_' + manifest['hashes'][str(ROOT/'camp.py')][:8] + '_' +
                            manifest['hashes'][str(ROOT/'run_stages.py')][:8])
    snapshot.mkdir(exist_ok=True)
    for filename in ('camp.py', 'run_stages.py', 'bounds_audit.py', '渐进探索_固定执行方案.md'):
    ...
    for stage in args.stages:
        ...
        checkpoints = sorted(set(x for x in [0, 100, 300, 400, 800, updates // 2,
                                  updates - min(300, updates // 6), updates] if x <= updates))
