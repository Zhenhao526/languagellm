...
A = {
    'known_sequence': dict(known=True, vocab=5, length=2),
    'hidden_sequence': dict(known=False, vocab=5, length=2),
    'known_atomic': dict(known=True, vocab=25, length=1),
    'hidden_atomic': dict(known=False, vocab=25, length=1),
    'hidden_single': dict(known=False, vocab=5, length=1),
    'hidden_blocked': dict(known=False, vocab=5, length=2, blocked=True),
...
def train(seed, name, plan, agents, bank, out, updates, batch, eval_n, checkpoints):
    ...
    write_json(out / 'config.json', dict(seed=seed, condition=name, plan=plan, updates=updates,
        ...
        initial_sha256=init, trainable_parameters=[sum(p.numel() for p in a.parameters() if p.requires_grad)
                                                for a in agents]))
    ...
    with (out / 'training.jsonl').open('w') as f:
        for update in range(updates + 1):
            ...
            if update == updates:
                break
            stats, learning, _ = rollout(agents, banks, bank, plan,
            ...
            f.write(json.dumps(dict(update=update + 1, reward=stats['mean_reward'],
    ...
    for stage in args.stages:
        ...
        for seed in args.seeds:
            ...
            for name, plan in conditions.items():
                ...
                train(seed, name, plan, agents, bank, out, updates, args.batch, args.eval_n, checkpoints)
