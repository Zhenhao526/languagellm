...
def initial_world(rng, n, heldout_training=False, map_pool=None):
    pool = TRAIN_MAPS if heldout_training else np.arange(12)
    if map_pool is not None:
        pool = np.asarray(map_pool, dtype=np.int64)
        if heldout_training:
            pool = np.intersect1d(pool, TRAIN_MAPS)
...
class CampAgent(nn.Module):
    ...
    def __init__(self, prepared_project, vocab=5, length=2, width=96):
        ...
        self.actor = nn.Sequential(nn.Linear(context, width), nn.Tanh(), nn.Linear(width, 4))
    ...
    def receive(self, messages, own_goal, inventory, history, menu):
        ...
        # A standard categorical policy over the four public world locations.
        # Menu order is local to the collector; gather merely relabels actions.
        logits = self.actor(context).gather(1, menu)
