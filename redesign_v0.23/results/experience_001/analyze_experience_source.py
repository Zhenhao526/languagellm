...
def private_update(folder,a,head,projected,cfg,w,step,row,check):
    ...
    # Preserve float32 backward accumulation order: sampling uses a detached
    # probability, and the entropy exp node is created after the policy term.
    lp=F.log_softmax(selected,-1);prob=lp.detach().exp();action=(prob.cumsum(-1)<torch.from_numpy(data['uniforms'])).sum(-1).clamp(max=5)
    target=world['positions'][np.arange(len(h)),goal];reward=(action.numpy()==target).astype(np.float32)
    policy=-(lp.gather(1,action[:,None]).squeeze(1)*torch.from_numpy(reward-.5)).mean();ent=-(lp.exp()*lp).sum(-1).mean();weight=.02 if step<2100 else 0.
...
def social_case(seed,p,arm,prepared,private_states,cache,bank,tables,out,sample,check):
    ...
    check.exact((cfg['seed'],cfg['partition'],cfg['arm'],cfg['batch_per_direction']),(seed,p,arm,256),'social declared identity and batch')
    check.exact(result['status'],'complete','social complete')
...
def main():
    ...
    try:
        ...
        check.exact(complete['new_dino_inferences'],0,'no new DINO inference')
        ...
        check.exact(complete['private_actions'],complete['private_updates']*512,'private action budget')
        check.exact(complete['messages'],complete['pair_updates']*512,'social message budget');check.exact(complete['actions'],complete['pair_updates']*1024,'social action budget')
        check.exact((complete['initial_preparation_updates'],complete['initial_preparation_actions']),(len(inv['seeds'])*400,len(inv['seeds'])*400*64),'initial resource preparation budget')
