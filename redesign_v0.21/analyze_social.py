...
def self_test():
    manifest={'images':[dict(split='test',category=k) for k in ('food','water') for _ in range(4)]};w=expected_world(1,manifest);n=len(w['mover'])
    uniform=statistics(np.zeros((n,49)),np.zeros((49,2,6)),np.zeros((n,2),np.int64),w,1)['common30']
    assert np.isclose(uniform['Q'],1/36) and uniform['J']==uniform['history_pair_J']==uniform['static_message_cmi_bits']==0
    assert uniform['route_code_agreement']==1 and uniform['route_code_entropy_bits']==0 and np.isclose(uniform['native_sender_entropy_bits'],np.log2(49))
    lp=np.full((n,49),-1000.);code=w['target_map'];lp[np.arange(n),code]=0;rl=np.full((49,2,6),-1000.)
    for c in range(49):rl[c,np.arange(2),MAPS[c%30]]=0
    perfect=statistics(lp,rl,np.column_stack((code//7,code%7)),w,1)
    for group,num in [('common30',5),('old',3)]:
        x=perfect[group];assert x['J']==x['Q']==x['history_pair_J']==x['history_pair_both_joint_J']==x['route_code_agreement']==1
        assert np.isclose(x['static_message_cmi_bits'],np.log2(num)) and np.isclose(x['conditional_shuffle_J'],.2) and np.isclose(x['global_shuffle_J'],1/30)
    assert np.isclose(perfect['common30']['constant00_J'],1/30)
    return dict(passed=True,tests=['Uniform sender/receiver exact independent-action Q1/36; greedy constant code has no history information',
        'Perfect holistic map-code protocol J/Q/historypair1, common CMIlog2(5), old CMIlog2(3)',
        'Common-support global shuffle1/30 and conditional shuffle1/5, even when scoring old subgroup',
        'Duplicate-route removal: perfect terminal code agreement1 and residual route entropy0'],production_functions_called=False)
...
def figures(data,out):
    ...
    plt.rcParams.update({'font.size':11,'axes.titlesize':11,'axes.labelsize':11,'legend.fontsize':9,'savefig.dpi':180})
    directory=out/'figures';directory.mkdir(exist_ok=True);colors={'immediate':'#2878B5','delayed':'#D76B27'};paths=[]
    fig,axes=plt.subplots(1,2,figsize=(8.3,3.8))
    for ax,key,title in zip(axes,['J','history_pair_J'],['Same-message joint success','Correct stationary-history pairs']):
        for row in data['seed_rows']:
            ax.plot(data['times'],[c['scores']['common30'][key] for c in row['curve']],color=colors[row['mode']],alpha=.24,lw=.8)
        for row in data['aggregate']:
            ax.plot(data['times'],[c['scores']['common30'][key] for c in row['curve']],color=colors[row['mode']],lw=2,label=row['mode'].replace('_',' ').title())
        ax.set(title=title,xlabel='Communication updates',ylim=(0,1));ax.yaxis.set_major_formatter(PercentFormatter(1));ax.grid(alpha=.15)
    axes[0].legend(loc='lower right');fig.text(.5,.02,f'Common 30 maps; thin lines: {len(data["seeds"])} source seeds; thick lines: source means. Greedy natural communication.',ha='center',fontsize=8.5)
    fig.tight_layout(rect=(0,.055,1,1))
    for suffix in ('png','pdf'):
        path=directory/f'01_social_learning.{suffix}';fig.savefig(path);paths.append(path)
    plt.close(fig)
    fig,axes=plt.subplots(1,2,figsize=(8.3,3.8));seedcolors=['#2878B5','#D76B27','#548A45','#8B63A9']
    for ax,key,title in zip(axes,['J','route_code_agreement'],['Final same-message joint success','Same terminal: code agreement across routes']):
        for seed,color in zip(data['seeds'],seedcolors):
            values=[next(r for r in data['seed_rows'] if r['seed']==seed and r['mode']==mode)['curve'][-1]['scores']['common30'][key] for mode in ('immediate','delayed')]
            ax.plot([0,1],values,'o-',color=color,lw=1.3,ms=4,label=str(seed))
        ax.set(xticks=[0,1],xticklabels=['Immediate','Delayed'],xlim=(-.2,1.2),ylim=(0,1),title=title);ax.yaxis.set_major_formatter(PercentFormatter(1));ax.grid(axis='y',alpha=.15)
    axes[0].legend(title='Source seed',loc='best');fig.text(.5,.02,f'Paired source means; {len(data["partitions"])} partitions and 2 directions nested per source. Different unique routes only.',ha='center',fontsize=8.5)
    fig.tight_layout(rect=(0,.055,1,1))
    for suffix in ('png','pdf'):
        path=directory/f'02_paired_social_endpoints.{suffix}';fig.savefig(path);paths.append(path)
    plt.close(fig);return {str(p.relative_to(out)):sha(p) for p in paths}
...
def main():
    parser=argparse.ArgumentParser(description=__doc__);parser.add_argument('--out',type=Path);parser.add_argument('--self-test',action='store_true');args=parser.parse_args()
    if args.self_test:print(json.dumps(self_test()),flush=True);return
    ...
    write(out/'comparison.json',comparison);write(out/'figure_files.json',dict(analysis_sha256=sha(out/'analysis.json'),files=figures(analysis,out),visual_review_pending=True));print(json.dumps(comparison),flush=True)
