...
def main():
    ...
    inv=read(batch/'invocation.json');seeds=inv['args']['seeds'];parts=inv['args']['partitions'];arms=['retained','reset'];source=Path(inv['args']['source_batch'])
    updates=inv['args']['social_updates'];assert read(batch/'training_complete.json')['status']=='complete'
    folders=[(source/f'social_s{s}_p{p}_control' if a=='retained' else batch/f'social_s{s}_p{p}_reset',a) for s,p,a in product(seeds,parts,arms)]
    ...
    for seed in seeds:
        ...
        paired.append(dict(seed=seed,**means,delta=means['retained']-means['reset']))
    result=dict(passed=True,completed_social_runs=len(rows),checked_scalar_values=value_checks,rows=rows,primary_by_seed=paired,
        primary_mean_difference=float(np.mean([x['delta'] for x in paired])),source_hashes=hashes,contrast='retained minus reset',recount_source_sha256=sha(__file__),
