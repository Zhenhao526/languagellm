...
def main():
    ...
    cfg=json.loads((SOURCE/'social_s31101_p1_control/config.json').read_text())
    ...
    for arm in ('retained','reset'):
        for group,mid in selected:
            ...
            for t in (0,600,2400):
                p=(SOURCE/f'social_s31101_p1_control/protocol_{t:04d}_d0.npz' if arm=='retained' else BATCH/f'social_s31101_p1_reset/protocol_{t:04d}_d0.npz')
    ...
    (BATCH/'fixed_examples.json').write_text(json.dumps(dict(rows=data,source_hashes=hashes,
        specification_sha256=hashlib.sha256((ROOT.parent/'redesign_v0.13/展示样例预设.md').read_bytes()).hexdigest()),ensure_ascii=False,indent=2)+'\n')
    lines=['# 固定场景中的消息形成记录','','沿用[v13展示规范](../../../redesign_v0.13/展示样例预设.md)的固定来源和场景，整批完成后提取本轮两条件记录，未按成败换例。本轮展示不新增预定科学检验。地点编号为0–5，每格依次为消息、食物地点/水地点、共同成败。符号只是整数词表的显示，没有人工规定含义。','',
