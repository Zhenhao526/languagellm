...
def audit_batch(batch,audit):
    initial_run_count=audit.counts['runs']
    ...
    for seed,p,arm in product(args['seeds'],args['partitions'],inv['arms']):
        ...
        audit.counts['runs']+=1
    expected=len(args['seeds'])*len(args['partitions'])*len(inv['arms']);audit.check(audit.counts['runs']-initial_run_count==expected and done['new_social_runs']==expected,'all_new_runs_audited',str(batch));audit.counts['training_communications']+=expected*args['social_updates']*args['batch'];audit.counts['training_actions']+=2*expected*args['social_updates']*args['batch']
    return inv
