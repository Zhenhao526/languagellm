"""Iterated newcomer replacement with a finite social-learning bottleneck."""
...
def source_hashes():
    files = [ROOT / "iterated_replacement.py", ROOT / "iterated_design.json", PROJECT / "redesign_v0.35" / "newcomer_adaptation.py"]
...
def input_hashes():
    ...
    for seed in SEEDS:
        ...
        for part in PARTITIONS:
            ...
            files.append(FIXED / "social" / f"s{seed}_p{part}_dual_complementary" / "final.pt")
...
def load_initial_population(seed, part, prepared):
    # Every schedule starts from the same fixed-A residents so that only the
    # post-replacement schedule differs across conditions.
    return _ADAPT.load_residents(seed, part, prepared, "fixed_A")[0]
...
def run(out: Path, seeds=SEEDS, parts=PARTITIONS, conditions=CONDITIONS, generations=GENERATIONS, updates=UPDATES_PER_GENERATION):
    ...
    formal = list(seeds) == list(SEEDS) and list(parts) == list(PARTITIONS) and list(conditions) == list(CONDITIONS) and generations == GENERATIONS and updates == UPDATES_PER_GENERATION
    write(out / "invocation.json", {"formal": formal, "version": "v0.36-iterated-replacement", "seeds": list(seeds), "partitions": list(parts), "conditions": list(conditions), "schedules": list(SCHEDULES), "generations": generations, "updates_per_generation": updates, "replacement_order": list(REPLACEMENT_ORDER[:generations]), "source_hashes": hashes, "input_hashes": inputs, "torch_version": str(torch.__version__), "numpy_version": np.__version__, "platform": platform.platform(), "threads": 1, "new_dino_inferences": 0, "new_private_fits": 0, "study_scope": "iterated cultural transmission with frozen residents and communication-only newcomer adaptation"})
    for path in hashes:
    ...
    for seed, part, condition in itertools.product(seeds, parts, conditions):
        ...
        agents = load_initial_population(seed, part, prepared)
        views_train = _ADAPT.load_views(FIXED, seed, part, "train"); views_test = _ADAPT.load_views(FIXED, seed, part, "test")
        chain = out / "chains" / f"s{seed}_p{part}_{condition}"; chain.mkdir(parents=True, exist_ok=True)
        config = {"seed": seed, "partition": part, "condition": condition, "generations": generations, "updates_per_generation": updates, "replacement_order": list(REPLACEMENT_ORDER[:generations]), "initial_residents": "v0.34 fixed-A endpoint", "training_schedule": {"fixed_A": "A_every_update", "rotating_AB": "A_even_B_odd_global_update", "random_ABC": "deterministic_random_A_B_C"}[condition], "evaluation_schedules": list(SCHEDULES), "newcomer_training": "communication_modules_only", "private_types": list(PRIVATE_TYPES), "base_source": str((FIXED / "social" / f"s{seed}_p{part}_dual_complementary").resolve())}
        ...
        for generation in range(1, generations + 1):
            ...
            write(folder / "config.json", {"generation": generation, "replaced_identity": held, "private_type": PRIVATE_TYPES[held], "updates": updates, "reset": reset_record, "condition": condition})
            schedule_counts = {schedule: 0 for schedule in SCHEDULES}; trace_names = []
            with (folder / "training.jsonl").open("w") as log:
                for step in range(updates):
                    loss, fixture_worlds, traces, schedule, global_step = train_one_step(current, views_train, tables["train"], seed, part, generation, step, held, condition)
...
def main():
    ...
    if args.dev:
        result = run(args.out.resolve(), seeds=(SEEDS[0],), parts=(1,), conditions=("fixed_A",), generations=min(args.generations, 2), updates=min(args.updates, 30))
