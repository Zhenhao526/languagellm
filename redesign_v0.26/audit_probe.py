...
def pixels(rows,qa):
    for row in rows:
        ...
        # Batch001 recorded crop SHA but not the derivative paths/full SHA.
        batch=Path(row['processed_path']).parent.parent
        crop_path=Path(row.get('crop_path',batch/'blind'/f'{ident}_crop.png'))
        qa.check(sha(crop_path)==row['crop_sha256'],ident+'/crop SHA')
        if 'full_sha256' in row:qa.check(sha(row['full_path'])==row['full_sha256'],ident+'/full SHA')
        ...
        qa.check(np.array_equal(crop,np.asarray(Image.open(crop_path).convert('RGB'))),ident+' exact224')
...
def audit(out):
    ...
    write(out/'audit_qa.json',dict(passed=True,status='passed',checks=qa.checks,source_sha256=sha(__file__),coverage=result['coverage'],files={name:sha(out/name) for name in ('analysis.json','comparison.json','audit_execution.json')},limits=result['limits']))
    return result
