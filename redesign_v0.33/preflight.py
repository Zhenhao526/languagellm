"""Frozen v0.33 formal-execution gate; no performance-based filtering."""
...
def main():
    ...
    for path, digest in formal_inputs.items():
        ...
        if p.is_relative_to(formal_source):
            ...
            bound += 1
    run.write(SMOKE / "terminal_receipt.json", dict(session_id=71989, exit_code=0, status="complete", observed_by="root tools.write_stdin", reported_program_seconds=complete["seconds"]))
    ...
    run.write(ROOT / "preflight_qa.json", dict(passed=True, version="v0.33", created_utc=datetime.now(timezone.utc).isoformat(), source_hashes=run.source_hashes(), input_hashes=formal_inputs, smoke_output=str(SMOKE), smoke_completion_sha256=run.sha(SMOKE / "training_complete.json"), smoke_protocol_tables=24, initial_exactly_paired=True, conditions=list(conditions), team_schedule="four team slots; every agent receiver once and both dual sender roles once", full_view_inheritance_max_error=max(run.read(SMOKE / "cache" / "s99528_p1" / "view_manifest.json")["full_view_errors"].values()), original_bound_inputs=bound, checks={str(path): run.sha(path) for path in (ROOT / "implementation_review.json", SMOKE / "raw_validation.json", SMOKE / "audit_execution.json")}, criterion_uses_performance_threshold=False, development_performance_not_used_for_selection=True, synthetic_tests=dict(support=3, views=2, dual=1, analyzer_self_tests=4), scope="Team inventory, full-view equality, complementary masks, paired condition fixtures, independent smoke statistics and trace replay"))
