"""Train the v0.33 single- and dual-sender population protocols."""
...
@torch.no_grad()
def social_eval(agents, cache, worlds, part, condition, folder, step, endpoint):
    scores = {}
    for slot, team in enumerate(support.team_slots(condition)):
        receiver = int(team[0])
        ...
        key = f"team{slot}"
        np.savez_compressed(folder / f"protocol_{step:04d}_{key}.npz", **raw)
