...
from camp import make_agents,individual_practice,remake_agents
...
from world import NonSquareImageBank as ImageBank
...
def source_hashes():
    files=[ROOT/n for n in ('run_formation.py','world.py','metrics.py','prepare_data.py','固定执行方案.md','前置科学审查.md')]
...
def main():
    ...
    assert args.updates>0;torch.set_num_threads(1);seeds=[99528] if args.dev else SEEDS;parts=[1] if args.dev else [1,2,3]
    hashes=source_hashes();inputs={str(ROOT/'data'/n):sha(ROOT/'data'/n) for n in ('feature_cache.pt','selection.json','freeze.json','encoder_receipt.json','anchor_qa.json')}
    assert read(ROOT/'data/encoder_receipt.json')['status']=='complete' and read(ROOT/'data/anchor_qa.json')['passed']
    preflight=None
    if not args.dev:
    ...
    write(out/'invocation.json',dict(formal=not args.dev,seeds=seeds,partitions=parts,updates=args.updates,source_hashes=hashes,input_hashes=inputs,
        ...
        primary='new12 pooled J at2400 all_old minus old_old; source nested partitions/directions/masks',study_scope='fixed_budget_development_formation_replication',test_worlds=180,train_worlds=720))
    bank=ImageBank();tables={split:world.table(bank,split) for split in ('train','test')}
    assert {k:len(w['map_id']) for k,w in tables.items()}=={'train':720,'test':180}
    np.savez_compressed(out/'normalization.npz',center=bank.center,scale=np.asarray(bank.scale))
    write(out/'image_pools.json',{f'{split}_{kind}':indices.tolist() for (split,kind),indices in bank.pools.items()})
