...
def run(out: Path, seeds=SEEDS, parts=PARTITIONS, conditions=CONDITIONS, assignments=tuple(ASSIGNMENTS), updates=UPDATES):
    ...
    for seed, part, assignment, condition in itertools.product(seeds, parts, assignments, conditions):
        ...
        for step in range(updates + 1):
            ...
            optimizer = getattr(run, "_optimizer", None)
            # Keep optimizer state chain-local without storing it in outputs.
            if step == 0:
                optimizer = torch.optim.Adam(optimizer_params, lr=0.0007)
                run._optimizer = optimizer
        ...
        # The optimizer is intentionally discarded at chain boundaries. Avoid
        # carrying state between independent seeds/conditions.
        if hasattr(run, "_optimizer"):
            delattr(run, "_optimizer")
