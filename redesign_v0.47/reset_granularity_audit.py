"""Independent NumPy audit for the v0.47 reset-granularity batch."""
...
def replay_trace(audit, path, seed, part, mode, adaptation_schedule, train_worlds):
    ...
    def draw(prob,u):
        # Production sampling uses torch.cumsum on float32 probabilities.  On
        # this CPU build torch accumulates the prefix in higher precision and
        # casts each prefix back to float32.  Reproduce that deterministic
        # reduction in NumPy so equality at a sampling boundary follows the
        # production ``cdf < u`` rule exactly.
...
def run(out: Path):
    audit=Checks(); invocation=read(OUT/"invocation.json"); complete=read(OUT/"training_complete.json"); audit.require(invocation.get("formal") is True and invocation.get("version")=="v0.47-reset-granularity","formal invocation"); audit.require(invocation.get("adaptation_schedule")==ADAPTATION_SCHEDULE,"adaptation schedule"); audit.require(tuple(invocation.get("reset_modes",()))==RESET_MODES,"reset mode coverage"); audit.require(complete.get("formal") is True and complete.get("status")=="complete" and complete.get("runs")==1296,"formal completion")
    ...
    for seed,part,assignment,culture,reset_mode in itertools.product(SEEDS,PARTITIONS,ASSIGNMENTS,CULTURES,RESET_MODES):
        mode,condition=culture.split("__",1); chain=OUT/"social"/f"s{seed}_p{part}_{assignment}_{culture}__reset_{reset_mode}"; cfg=read(chain/"config.json"); audit.require(cfg["seed"]==seed and cfg["partition"]==part and cfg["assignment"]==assignment and cfg["resident_culture"]==culture and cfg["adaptation_schedule"]==ADAPTATION_SCHEDULE and cfg["reset_mode"]==reset_mode and cfg["adaptation_role_mode"]==mode and cfg["newcomer_identity"]==0,f"{chain.name} config"); expected_modules={"sender_only":["send_context","send_embedding","send_recur","send_out"],"receiver_only":["receive_embedding","actor"],"both":["send_context","send_embedding","send_recur","send_out","receive_embedding","actor"]}[reset_mode]; audit.require(cfg["reset_record"]["reset_mode"]==reset_mode and cfg["reset_record"]["reset_modules"]==expected_modules,f"{chain.name} reset modules")
        ...
        for step,line in enumerate(logs):
            row=json.loads(line); audit.require(row["update"]==step+1 and row["global_step"]==step and row["schedule"]==schedule_for(ADAPTATION_SCHEDULE,seed,part,step) and row["role_mode"]==mode and row["reset_mode"]==reset_mode,f"{chain.name} log {step}"); expected={str(slot):list(role_for(mode,seed,part,step,slot)) for slot in range(4)}; audit.require(row["role_permutations"]==expected,f"{chain.name} log roles {step}"); audit.require(np.isfinite(float(row["loss"])) and all(np.isfinite(float(v)) for v in row["norms"].values()),f"{chain.name} log finite")
    ...
    result={"status":"complete","formal":True,"probe":"reset_granularity","runs":groups,"checks":audit.checks,"scalar_comparisons":audit.comparisons,"maximum_replay_absolute_difference":audit.max_error,"coverage":dict(audit.coverage),"source_sha256":sha(ROOT/"reset_granularity_audit.py"),"production_modules_imported":False,"model_calls":0,"limits":["Audit replays deterministic fixtures and stored traces/actions; it does not independently retrain weights.","Reset-mode analysis checks saved scores against v0.43 resident endpoints and computes recovery summaries."]}
    out.mkdir(parents=True,exist_ok=True); (out/"reset_granularity_audit.json").write_text(json.dumps(result,ensure_ascii=False,indent=2,allow_nan=False)+"\n"); shutil.copy2(ROOT/"reset_granularity_audit.py",out/"reset_granularity_audit_source.py"); return result


def main():
    parser=argparse.ArgumentParser(); parser.add_argument("--out",type=Path,required=True); args=parser.parse_args(); started=time.monotonic(); result=run(args.out.resolve()); result["seconds"]=time.monotonic()-started; (args.out.resolve()/"reset_granularity_audit.json").write_text(json.dumps(result,ensure_ascii=False,indent=2,allow_nan=False)+"\n"); print(json.dumps({"status":result["status"],"runs":result["runs"],"checks":result["checks"],"scalar_comparisons":result["scalar_comparisons"],"maximum_replay_absolute_difference":result["maximum_replay_absolute_difference"],"seconds":result["seconds"]},ensure_ascii=False))
