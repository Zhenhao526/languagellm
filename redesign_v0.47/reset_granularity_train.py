"""Train a newcomer after sender, receiver, or both communication sides are reset."""
...
SCHEDULE_NAMESPACE, FIXTURE_NAMESPACE, ROLE_NAMESPACE, RESET_NAMESPACE = 47040, 47041, 47042, 47043
...
RESET_GROUPS = {
    "sender_only": COMMUNICATION_MODULES[:4],
    "receiver_only": COMMUNICATION_MODULES[4:],
    "both": COMMUNICATION_MODULES,
}
...
def make_newcomer(permutation, resident_agent, seed, part, assignment_index, culture_index, reset_mode):
    # Start from the resident endpoint so that an isolated reset measures
    # sender/receiver recovery rather than an unrelated visual initialization.
    newcomer = copy.deepcopy(resident_agent)
    reset = reset_seed(seed, part, assignment_index, culture_index)
    reset_record = reset_communication_parts(newcomer, reset, reset_mode)
    newcomer.train()
    return newcomer, reset_record


def evaluate_compact(permutation, agents, views, worlds, partition, folder, update):
    ...
    scores = {}
    for slot, team in enumerate(permutation.T.TEAMS["A"]):
        pass
    for schedule in SCHEDULES:
        ...
        for slot, team in enumerate(permutation.T.TEAMS[schedule]):
            ...
            for role_perm in ROLE_PERMS:
                ...
                role_tokens.append(tokens); role_actions.append(actions)
...
def adapt_step(permutation, agents, views_train, worlds, seed, part, step, role_mode, reset_mode):
    ...
    for slot, team in enumerate(permutation.T.TEAMS[schedule]):
        ...
        for resource, sender in enumerate(senders):
            if sender == 0 and reset_mode in ("sender_only", "both"):
                newcomer_losses.append(sender_losses[resource])
        if team[0] == 0 and reset_mode in ("receiver_only", "both"):
            newcomer_losses.append(receiver_loss)
        fixtures[slot], traces[slot], roles[slot] = fixture_data, trace, role_perm
    loss = sum(newcomer_losses) / len(newcomer_losses)
    return loss, fixtures, traces, roles, schedule


def run(out: Path, seeds=SEEDS, parts=PARTITIONS, assignments=ASSIGNMENTS, cultures=CULTURES, reset_modes=RESET_MODES, updates=UPDATES):
    ...
    seeds, parts, assignments, cultures = tuple(seeds), tuple(parts), tuple(assignments), tuple(cultures)
    formal = seeds == SEEDS and parts == PARTITIONS and assignments == ASSIGNMENTS and cultures == CULTURES and tuple(reset_modes) == RESET_MODES and updates == UPDATES
    checkpoints = sorted(set(t for t in CHECKPOINTS if t <= updates) | {updates})
    hashes, inputs = source_hashes(), input_hashes()
    write(out / "invocation.json", {"formal": formal, "version": "v0.47-reset-granularity", "seeds": list(seeds), "partitions": list(parts), "assignments": list(assignments), "cultures": list(cultures), "role_modes": list(ROLE_MODES), "conditions": list(CONDITIONS), "adaptation_schedule": ADAPTATION_SCHEDULE, "reset_modes": list(reset_modes), "updates": updates, "checkpoints": checkpoints, "newcomer_identity": 0, "newcomer_reset": "resident_endpoint_selective_communication_reset", "source_hashes": hashes, "input_hashes": inputs, "torch_version": str(torch.__version__), "numpy_version": np.__version__, "platform": platform.platform(), "threads": 1, "study_scope": "crossed sender-only, receiver-only and full communication reset recovery inside v0.43 resident cultures"})
    for path in hashes:
    ...
    for seed, part, assignment, culture_label, reset_mode in itertools.product(seeds, parts, assignments, cultures, reset_modes):
        ...
        newcomer, reset_record = make_newcomer(permutation, agents[0], seed, part, assignment_index[assignment], culture_index[culture_label], reset_mode)
        agents[0] = newcomer
        # Only newcomer communication modules are trainable; all resident
        # agents keep their endpoint parameters and never receive gradients.
        for identity, agent in enumerate(agents):
            if identity != 0:
                agent.requires_grad_(False)
        groups = permutation.T.trainable_groups(agents[0])
        active_groups = {name: group for name, group in groups.items() if reset_mode == "both" or (reset_mode == "sender_only" and name == "sender") or (reset_mode == "receiver_only" and name == "receiver")}
        optimizer = torch.optim.Adam([p for group in active_groups.values() for p in group], lr=0.0007)
        chain = out / "social" / f"s{seed}_p{part}_{assignment}_{culture_label}__reset_{reset_mode}"
        chain.mkdir(parents=True, exist_ok=True)
        write(chain / "config.json", {"seed": seed, "partition": part, "assignment": assignment, "resident_culture": culture_label, "resident_role_mode": mode, "resident_condition": condition, "adaptation_schedule": ADAPTATION_SCHEDULE, "adaptation_role_mode": mode, "reset_mode": reset_mode, "newcomer_identity": 0, "updates": updates, "checkpoints": checkpoints, "communication_initialization": "resident_endpoint_selective_reset", "reset_record": reset_record, "evaluation_schedules": list(SCHEDULES), "train_map_count": 60, "test_map_count": 120})
        curve = []
        for step in range(updates + 1):
            if step in checkpoints:
                for agent in agents: agent.eval()
                scores = evaluate_compact(permutation, agents, views, test_worlds, part, chain, step)
            ...
            if step == updates:
                break
            loss, fixture_slots, trace_slots, role_slots, schedule = adapt_step(permutation, agents, views, train_worlds, seed, part, step, mode, reset_mode)
            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            norms = {role: float(torch.nn.utils.clip_grad_norm_(group, 2.0)) for role, group in active_groups.items()}
            if not all(np.isfinite(value) for value in norms.values()): raise ValueError("non-finite adaptation gradient")
            optimizer.step()
            ...
            with (chain / "training.jsonl").open("a") as log:
                log.write(json.dumps({"update": step + 1, "global_step": step, "schedule": schedule, "role_mode": mode, "reset_mode": reset_mode, "role_permutations": {str(slot): list(role_slots[slot]) for slot in role_slots}, "loss": float(loss.detach()), "norms": norms}, ensure_ascii=False) + "\n")
        rows.append({"seed": seed, "partition": part, "assignment": assignment, "resident_culture": culture_label, "reset_mode": reset_mode, "adaptation_schedule": ADAPTATION_SCHEDULE, "file": str(chain.relative_to(out))})
        total += 1
        print(json.dumps({"phase": "reset_granularity_adaptation", "chain": chain.name, "runs": total, "seconds": round(time.monotonic() - started, 2)}, ensure_ascii=False), flush=True)
    ...
    write(out / "training_complete.json", {"status": "complete", "formal": formal, "probe": "reset_granularity", "runs": total, "updates_per_run": updates, "resident_cultures": list(cultures), "reset_modes": list(reset_modes), "adaptation_schedule": ADAPTATION_SCHEDULE, "checkpoints": checkpoints, "trace_files_expected": total * 8, "protocol_files_expected": total * len(checkpoints) * len(SCHEDULES) * 4, "seconds": time.monotonic() - started, "source_hashes": hashes, "input_hashes": inputs, "files": files})
    return {"status": "complete", "formal": formal, "runs": total, "seconds": time.monotonic() - started, "source_sha256": sha(ROOT / "reset_granularity_train.py")}


def source_hashes():
    paths = [ROOT / "reset_granularity_train.py", ROOT / "reset_granularity_design.json", V043 / "permutation_train.py", V043 / "permutation_design.json", V039 / "triad_train.py", V039 / "triad_design.json"]
...
def main():
    ...
    if args.dev:
        result = run(args.out.resolve(), seeds=(SEEDS[0],), parts=(PARTITIONS[0],), assignments=(ASSIGNMENTS[0],), cultures=(CULTURES[0],), reset_modes=(RESET_MODES[0],), updates=min(args.updates, 5))
    else:
