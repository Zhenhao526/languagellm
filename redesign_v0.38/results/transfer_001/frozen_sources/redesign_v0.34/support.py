"""Frozen v0.34 partner schedules and condition-paired world fixtures."""
...
DESIGN_SHA256 = "fc38f6bdd975a64074f0db133ffc0f7b05bf68bff45027a1ac5d8aa3a312708a"
...
def team_slots(condition, step=None, schedule=None):
    ...
    if schedule is None:
        schedule = "A"
        if condition == "dual_complementary" and step is not None and int(step) % 2:
            schedule = "B"
    schedules = _DESIGN["partner_topology"]
    key = f"schedule_{schedule}"
    if key not in schedules:
        raise ValueError(f"unknown partner schedule: {schedule}")
    return tuple(tuple(int(x) for x in team) for team in schedules[key])
