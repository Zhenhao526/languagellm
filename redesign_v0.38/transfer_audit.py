"""Bounded trace and endpoint audit for the v0.38 origin-transfer factorial."""
...
def check_trace(path: Path, train_worlds, transmission, seed, part, generation, checks):
    ...
    checks.exact(np.asarray(schedule), np.asarray(expected_schedule(transmission, seed, part, generation, global_step, step)), "trace schedule replay")
    slots = {}
    ...
    for seed, part, formation, transmission in itertools.product(SEEDS, PARTITIONS, FORMATION_CONDITIONS, TRANSMISSION_CONDITIONS):
        ...
        for generation in GENERATIONS:
            ...
            for schedule in SCHEDULES:
                for slot in range(4):
                    path = folder / f"protocol_{schedule}_{generation:04d}_team{slot}.npz"; rel = str(path.relative_to(out)); checks.require(rel in complete["files"], "protocol completion binding"); check_raw(npz(path), worlds, checks, f"protocol {formation}/{transmission}/{seed}/{part}/{generation}/{schedule}/{slot}"); checks.add("protocol_tables")
            if generation > 0:
                checks.exact(item["replaced_identity"], REPLACEMENT_ORDER[generation - 1], "replacement identity"); counts = item["schedule_counts"]; checks.exact(sorted(counts), list(SCHEDULES), "schedule count keys"); checks.exact(sum(counts.values()), 300, "schedule count total")
                for name, global_step in (("train_0001.npz", (generation - 1) * 300), ("train_0300.npz", generation * 300 - 1)):
                    path = folder / name; rel = str(path.relative_to(out)); checks.require(rel in complete["files"], "trace completion binding"); check_trace(path, train_worlds, transmission, seed, part, generation, checks); checks.exact(npz(path)["global_step"], np.asarray([global_step], dtype=np.int64), "global step")
        ...
        checks.add("chains")
    result = {"passed": True, "status": "passed_origin_transfer_trace_audit", "formal": True, "probe": "origin_transfer", "checks": checks.count, "coverage": dict(checks.coverage), "source_hashes": invocation["source_hashes"], "input_hashes": invocation["input_hashes"], "training_complete_sha256": sha(out / "training_complete.json"), "audit_source_sha256": sha(ROOT / "transfer_audit.py"), "exclusions": ["No optimizer-state replay; the audit checks endpoint tables, categorical traces, schedule replay, reward arithmetic and completion bindings."]}; write(out / "transfer_audit.json", result); shutil.copy2(ROOT / "transfer_audit.py", out / "transfer_audit_source.py"); return result
...
def main():
    ...
    try:
        result = run(args.out.resolve()); result["seconds"] = time.monotonic() - started; write(args.out.resolve() / "transfer_audit.json", result); print(json.dumps({k: result[k] for k in ("passed", "status", "checks", "coverage", "seconds")}, ensure_ascii=False))
    except Exception as error:
        stamp = time.time_ns(); write(args.out.resolve() / f"transfer_audit_failure_{stamp}.json", {"passed": False, "error": repr(error), "traceback": traceback.format_exc(), "source_sha256": sha(ROOT / "transfer_audit.py")}); raise
