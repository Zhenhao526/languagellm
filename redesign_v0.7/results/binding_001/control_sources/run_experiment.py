"""Matched raw-visual locality exploration; independent reward-trained interfaces."""
...
CONDITIONS = {f'split{split}_{representation}': dict(split=split, representation=representation,
    schedule='direct', vocab=7, length=2, known=False, blocked=False)
    for split in MATCHINGS for representation in REPRESENTATIONS}
CONDITIONS.update({f'full_{representation}': dict(split=0, representation=representation,
    schedule='direct', vocab=7, length=2, known=False, blocked=False) for representation in REPRESENTATIONS})
CONDITIONS['full_blocked'] = dict(split=0, representation='identity', schedule='direct',
                                 vocab=7, length=2, known=False, blocked=True)
...
def train(seed, name, plan, prepared, bank, out, updates, batch, eval_n):
    ...
    write_json(out / 'config.json', dict(seed=seed, condition=name, plan=plan, updates=updates, batch=batch,
        ...
        training_seed_offset=40000000, initial_sha256=init,
    ...
    with (out/'training.jsonl').open('w') as log:
        for update in range(updates+1):
            if update in checkpoints:
                scores = evaluate(agents,banks,bank,plan,seed+49100000,1200)
            ...
            stats,learning,_=rollout(agents,banks,bank,train_plan,seed*100000+40000000+bid+1,
    ...
    final=evaluate(agents,banks,bank,plan,seed+49200000,eval_n,out)
