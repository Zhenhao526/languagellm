"""Independent read-only audit of all 52 binding runs and 12 personal controls.
...
REPRESENTATIONS=('identity','permute','orthogonal')
CONDITIONS={f'split{s}_{representation}':dict(split=s,representation=representation,
    schedule='direct',vocab=7,length=2,known=False,blocked=False)
    for s in MATCHINGS for representation in REPRESENTATIONS}
CONDITIONS.update({f'full_{r}':dict(split=0,representation=r,schedule='direct',vocab=7,length=2,
    known=False,blocked=False) for r in REPRESENTATIONS})
CONDITIONS['full_blocked']=dict(split=0,representation='identity',schedule='direct',vocab=7,
    length=2,known=False,blocked=True)
...
def training_world(seed,identity,pool,photo_pools):
    rng=np.random.default_rng(seed*100000+40000000+identity+1)
    rows=[]
    for scout in (0,1):
...
def evaluation_world(seed,photo_pools):
    rng=np.random.default_rng(seed+49200000);rows=[]
    for scout in (0,1):
...
def controls_audit(audit,root,ns,pools,allow_incomplete):
    ...
    if gate_path.exists():
        gate=read(gate_path)
        audit.check(gate['controls_complete']==12 and gate['all_passed'] and gate['social_runs_planned']==52 and
    ...
    if not allow_incomplete:audit.check(len(completed)==12 and not pending,'all_twelve_personal_controls_complete',pending)
    return dict(completed=len(completed),pending=pending,gates=gates,
        ...
        scope='Each person has 2400 x 512 personal choices; social runs have 2400 x 512 pair episodes. Control weights are independent copies.')


def main():
    p=argparse.ArgumentParser()
    p.add_argument('--root',type=Path,default=ROOT/'results/binding_001')
    p.add_argument('--out',type=Path)
    p.add_argument('--allow-incomplete',action='store_true')
    p.add_argument('--development-triplet',nargs=2,type=int,metavar=('SEED','SPLIT'),
                   help='With --allow-incomplete, audit controls and one complete main representation triplet.')
    ...
    ns={'__file__':str(ROOT/'camp.py'),'__name__':'binding_audit_frozen_camp'}
    ...
    for version in ('redesign_v0.4','redesign_v0.5','redesign_v0.6','redesign_v0.7'):
    ...
    for seed in SEEDS:
      for name,plan in CONDITIONS.items():
        ...
        expected=dict(seed=seed,condition=name,plan=plan,updates=2400,batch=512,eval_n=9600,
            ...
            training_seed_offset=40000000,source_hashes={Path(p).name:h for p,h in hashes.items() if Path(p).parent==ROOT})
        ...
        for mode in MODES:
            ...
            with np.load(folder/f'final_{mode}.npz') as z:
                ...
                if plan['blocked'] or mode=='blank':delivered[:]=0
                    ...
                    for scout in (0,1):
                        rng=np.random.default_rng(seed+49200000+10001+scout)
                        for goal in (0,1):
    ...
    for seed in SEEDS:
      for split in (0,1,2,3):
        prefix=f's{seed}_split{split}_' if split else f's{seed}_full_'
        names=[prefix+r for r in REPRESENTATIONS]
        ...
        for name in names[1:]:
            ...
            for a,b in zip(base,other):
                audit.check(all(a[k]==b[k] for k in ('update','batch_identity','active_sites','allowed_sites',
                    'map_pool','world_sha256','entropy_weight')),'representation_matched_worlds',[seed,split,a['update'],name])
        initial=[load(root/name/'initial.pt') for name in names]
        without_transform=lambda states:[{k:v for k,v in s.items() if k!='input_transform'} for s in states]
        audit.check(all(equal_states(without_transform(initial[0]),without_transform(x)) for x in initial[1:]),
                    'representations_same_initial_parameters',[seed,split])
        matched.append(dict(seed=seed,split=split))
      identity=f's{seed}_full_identity';blocked=f's{seed}_full_blocked'
      if identity in rows_by_run and blocked in rows_by_run:
        audit.check(equal_states(load(root/identity/'initial.pt'),load(root/blocked/'initial.pt')),
                    'full_blocked_same_initial_state',seed)
        audit.check(all(all(a[k]==b[k] for k in ('world_sha256','batch_identity','entropy_weight','map_pool'))
            for a,b in zip(rows_by_run[identity],rows_by_run[blocked])), 'full_blocked_same_world_stream',seed)
    if not args.allow_incomplete:
        audit.check(not pending and len(completed)==52,'all_52_runs_complete',pending)
        audit.check(len(people)==8,'all_eight_new_people_present')
        audit.check(len(matched)==16,'all_sixteen_representation_triplets_complete')
    report=dict(status='failed' if audit.failures else 'incomplete' if pending or controls['pending'] else 'passed',planned_runs=52,
        ...
        matched_representation_triplets=matched,preparations=people,historical_preparations_compared=history_count,
        ...
        boundaries=['No training or full optimizer replay. New prepared seed provenance is established by untouched sender/value fingerprints and source/state checks.',
          'All world reconstructions use independent six-site matching supports; each representation receives identical raw worlds at a given seed/split.',
          'Float32 aggregate means allow 1e-7 representation tolerance; exact integer counts govern personal-control gates.',
