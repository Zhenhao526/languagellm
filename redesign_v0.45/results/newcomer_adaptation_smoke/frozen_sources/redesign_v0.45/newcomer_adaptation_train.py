"""Train a freshly reset newcomer inside v0.43 resident cultures."""
...
def adapt_step(permutation, agents, views_train, worlds, seed, part, step, role_mode):
    schedule = schedule_for("random_ABC", seed, part, step)
    newcomer_losses = []
...
def run(out: Path, seeds=SEEDS, parts=PARTITIONS, assignments=ASSIGNMENTS, cultures=CULTURES, updates=UPDATES):
    ...
    formal = seeds == SEEDS and parts == PARTITIONS and assignments == ASSIGNMENTS and cultures == CULTURES and updates == UPDATES
    checkpoints = sorted(set(t for t in CHECKPOINTS if t <= updates) | {updates})
    hashes, inputs = source_hashes(), input_hashes()
    write(out / "invocation.json", {"formal": formal, "version": "v0.45-newcomer-adaptation", "seeds": list(seeds), "partitions": list(parts), "assignments": list(assignments), "cultures": list(cultures), "role_modes": list(ROLE_MODES), "conditions": list(CONDITIONS), "adaptation_schedule": "random_ABC", "updates": updates, "checkpoints": checkpoints, "newcomer_identity": 0, "newcomer_reset": "communication_modules_only", "source_hashes": hashes, "input_hashes": inputs, "torch_version": str(torch.__version__), "numpy_version": np.__version__, "platform": platform.platform(), "threads": 1, "study_scope": "social recovery of a freshly reset communication newcomer inside v0.43 resident cultures"})
    for path in hashes:
    ...
    for seed, part, assignment, culture_label in itertools.product(seeds, parts, assignments, cultures):
        ...
        write(chain / "config.json", {"seed": seed, "partition": part, "assignment": assignment, "resident_culture": culture_label, "resident_role_mode": mode, "resident_condition": condition, "adaptation_schedule": "random_ABC", "adaptation_role_mode": mode, "newcomer_identity": 0, "updates": updates, "checkpoints": checkpoints, "communication_initialization": "fresh_random_reset_newcomer_only", "reset_record": reset_record, "evaluation_schedules": list(SCHEDULES), "train_map_count": 60, "test_map_count": 120})
        curve = []
        for step in range(updates + 1):
            ...
            if step == updates:
                break
            loss, fixture_slots, trace_slots, role_slots, schedule = adapt_step(permutation, agents, views, train_worlds, seed, part, step, mode)
        ...
        rows.append({"seed": seed, "partition": part, "assignment": assignment, "resident_culture": culture_label, "file": str(chain.relative_to(out))})
        total += 1
    ...
    write(out / "training_complete.json", {"status": "complete", "formal": formal, "probe": "newcomer_adaptation", "runs": total, "updates_per_run": updates, "resident_cultures": list(cultures), "adaptation_schedule": "random_ABC", "checkpoints": checkpoints, "trace_files_expected": total * 8, "protocol_files_expected": total * len(checkpoints) * len(SCHEDULES) * 4, "seconds": time.monotonic() - started, "source_hashes": hashes, "input_hashes": inputs, "files": files})
    return {"status": "complete", "formal": formal, "runs": total, "seconds": time.monotonic() - started, "source_sha256": sha(ROOT / "newcomer_adaptation_train.py")}


def source_hashes():
    paths = [ROOT / "newcomer_adaptation_train.py", ROOT / "newcomer_adaptation_design.json", V043 / "permutation_train.py", V043 / "permutation_design.json", V039 / "triad_train.py", V039 / "triad_design.json"]
...
def main():
    ...
    if args.dev:
        result = run(args.out.resolve(), seeds=(SEEDS[0],), parts=(PARTITIONS[0],), assignments=(ASSIGNMENTS[0],), cultures=(CULTURES[0],), updates=min(args.updates, 5))
    else:
