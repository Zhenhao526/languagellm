"""Independent NumPy audit for v0.45 newcomer adaptation."""
...
def run(out: Path):
    audit=Checks(); invocation=read(OUT/"invocation.json"); complete=read(OUT/"training_complete.json"); audit.require(invocation.get("formal") is True and invocation.get("version")=="v0.45-newcomer-adaptation","formal invocation"); audit.require(complete.get("formal") is True and complete.get("status")=="complete" and complete.get("runs")==432,"formal completion")
    ...
    groups=0
    for seed,part,assignment,culture in itertools.product(SEEDS,PARTITIONS,ASSIGNMENTS,CULTURES):
        mode,condition=culture.split("__",1); chain=OUT/"social"/f"s{seed}_p{part}_{assignment}_{culture}"; cfg=read(chain/"config.json"); audit.require(cfg["seed"]==seed and cfg["partition"]==part and cfg["assignment"]==assignment and cfg["resident_culture"]==culture and cfg["adaptation_schedule"]=="random_ABC" and cfg["adaptation_role_mode"]==mode and cfg["newcomer_identity"]==0,f"{chain.name} config")
        ...
        for step,line in enumerate(logs):
            row=json.loads(line); audit.require(row["update"]==step+1 and row["global_step"]==step and row["schedule"]==schedule_for(seed,part,step) and row["role_mode"]==mode,f"{chain.name} log {step}"); expected={str(slot):list(role_for(mode,seed,part,step,slot)) for slot in range(4)}; audit.require(row["role_permutations"]==expected,f"{chain.name} log roles {step}"); audit.require(np.isfinite(float(row["loss"])) and all(np.isfinite(float(v)) for v in row["norms"].values()),f"{chain.name} log finite")
    ...
    audit.require(groups==432,"group coverage"); audit.require(audit.coverage["traces"]==432*8,"trace coverage"); audit.require(audit.coverage["protocol_files"]==432*36,"protocol coverage")
    result={"status":"complete","formal":True,"probe":"newcomer_adaptation","runs":groups,"checks":audit.checks,"scalar_comparisons":audit.comparisons,"maximum_replay_absolute_difference":audit.max_error,"coverage":dict(audit.coverage),"source_sha256":sha(ROOT/"newcomer_adaptation_audit.py"),"production_modules_imported":False,"model_calls":0,"limits":["Audit replays deterministic fixtures and stored traces/actions; it does not independently retrain weights.","Endpoint analysis checks saved scores against the v0.43 protocol endpoints and computes recovery summaries."]}
    out.mkdir(parents=True,exist_ok=True); (out/"newcomer_adaptation_audit.json").write_text(json.dumps(result,ensure_ascii=False,indent=2,allow_nan=False)+"\n"); shutil.copy2(ROOT/"newcomer_adaptation_audit.py",out/"newcomer_adaptation_audit_source.py"); return result


def main():
    parser=argparse.ArgumentParser(); parser.add_argument("--out",type=Path,required=True); args=parser.parse_args(); started=time.monotonic(); result=run(args.out.resolve()); result["seconds"]=time.monotonic()-started; (args.out.resolve()/"newcomer_adaptation_audit.json").write_text(json.dumps(result,ensure_ascii=False,indent=2,allow_nan=False)+"\n"); print(json.dumps({"status":result["status"],"runs":result["runs"],"checks":result["checks"],"scalar_comparisons":result["scalar_comparisons"],"maximum_replay_absolute_difference":result["maximum_replay_absolute_difference"],"seconds":result["seconds"]},ensure_ascii=False))
