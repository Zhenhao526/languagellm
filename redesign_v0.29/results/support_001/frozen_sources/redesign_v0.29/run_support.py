"""Fresh communication under two fixed, exactly degree-matched supports."""
...
def source_hashes():
    files=[ROOT/n for n in ('run_support.py','support.py','metrics.py','code_relabelings.npy','固定执行方案.md')]
    files += [PROJECT/n for n in ('paper_program/mechanism_pressure_20260916/support_enumeration.json','redesign_v0.28/world.py',
...
def fit_social(seed,p,arm,prepared,cache,tables,source,out,args):
    ...
    write(folder/'config.json',dict(seed=seed,partition=p,arm=arm,updates=args.updates,reset=resets,checkpoints=checkpoint_times(args.updates),
        frozen_hashes=frozen_hashes,cache=str((source/'cache'/f's{seed}_p{p}_all').resolve()),source=str(source),batch_per_direction=252,
    ...
    torch.save(save(),folder/'final.pt');write(folder/'result.json',dict(status='complete',scores=curve[-1]['scores'],updates=args.updates,
        seconds=time.monotonic()-started,messages=args.updates*504,actions=args.updates*1008,frozen_verified=True))
...
def main():
    ...
    if not args.dev:
        gate=read(ROOT/'preflight_qa.json');assert gate['passed'] and gate['source_hashes']==hashes and args.updates==2400
    ...
    write(out/'invocation.json',dict(formal=not args.dev,seeds=seeds,partitions=parts,updates=args.updates,source=str(source),source_hashes=hashes,input_hashes=inputs,
        ...
        social_arms=list(support.ARMS),started_utc=datetime.now(timezone.utc).isoformat(),batch_per_direction=252,
        primary='common_target6 pooled J at2400 target_paths3 minus target_paths2; nested source / panels / directions / masks',
        study_scope='fixed_budget_development_support_contrast',new_private_fits=0,test_worlds=180,train_worlds=720))
    ...
    write(out/'training_complete.json',dict(status='complete',formal=not args.dev,social_runs=ns,pair_updates=ns*args.updates,
        messages=ns*args.updates*504,actions=ns*args.updates*1008,seconds=time.monotonic()-started,source_hashes=hashes,input_hashes=inputs,
