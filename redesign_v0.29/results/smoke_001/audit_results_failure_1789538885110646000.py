"""Bounded independent v29 execution audit; no production support/update imports.
...
def seed_value(seed,p,d):
    return int(np.random.SeedSequence([29029,seed,p,d,1]).generate_state(1,dtype=np.uint64)[0]>>np.uint64(1))
...
def fixture(seed,p,d,step,arm):
    groups=stats.groups(p,arm);shared=groups['shared_train13'];unique=np.setdiff1d(groups['train18'],shared)
    rng=lambda stream:np.random.default_rng(np.random.SeedSequence([29029,seed,p,d,step,stream]))
    maps=np.tile(np.r_[shared,unique],7)[rng(0).permutation(126)]
    photos=rng(1).random((126,2));base=maps*12+(photos[:,0]*6).astype(np.int64)*2+(photos[:,1]*2).astype(np.int64)
    return dict(indices=np.r_[base,base+360],uniforms=rng(2).random((252,4)).astype(np.float32))
...
def check_fixture(seed,p,step,arm,w,raw,log,cache,c):
    ...
    for d in (0,1):
        ...
        shared=np.isin(w['map_id'][idx],stats.groups(p,arm)['shared_train13'])
        c.exact(int(shared.sum()),182,'shared actual worlds182/252')
        c.exact(w['positions'][idx][shared],w['positions'][other['indices']][shared],'shared positions paired')
        counts=np.bincount(w['map_id'][idx],minlength=30);wanted=np.zeros(30,int);wanted[stats.groups(p,arm)['train18']]=14
        c.exact(counts,wanted,'exact training support and equal map exposures')
        for resource in (0,1):c.exact(np.bincount(w['positions'][idx,resource],minlength=6),np.full(6,42),'exact position margins')
        c.exact(w['shown'][idx],np.repeat(np.arange(2),126),'paired masks')
        c.exact(w['map_id'][idx[:126]],w['map_id'][idx[126:]],'same maps two masks')
        ...
        c.coverage['sampled_direction_fixtures']+=1;c.coverage['sampled_message_trajectories']+=252
...
def audit(out,c):
    ...
    c.exact(inv['social_arms'],list(stats.ARMS),'all arms');c.exact(inv['batch_per_direction'],252,'direction batch')
    for seed,p in itertools.product(seeds,panels):
        ...
        for arm in stats.ARMS:
            ...
            c.exact((res['status'],res['updates'],res['messages'],res['actions'],res['frozen_verified']),('complete',steps,steps*504,steps*1008,True),'run terminal budgets')
            c.coverage['social_runs']+=1
    n=len(seeds)*len(panels)*2
    c.exact(len(list((out/'social').glob('*/result.json'))),n,'exact run inventory')
    for k,v in dict(social_runs=n,pair_updates=n*steps,messages=n*steps*504,actions=n*steps*1008,new_private_fits=0,new_dino_inferences=0).items():c.exact(done[k],v,'complete '+k)
...
def main():
    ...
    try:
        ...
        dependencies={str(Path(m.__file__).resolve()):sha(m.__file__) for m in list(sys.modules.values()) if getattr(m,'__file__',None) and str(Path(m.__file__).resolve()).startswith(str(PROJECT)) and Path(m.__file__).suffix=='.py'}
