...
def adapt_step(permutation, agents, views_train, worlds, seed, part, step, role_mode, resident_mode):
    ...
    selected = list(losses_by_identity[0]["sender"]) + list(losses_by_identity[0]["receiver"])
    ...
    if resident_step:
        ...
        for identity in (1, 2, 3):
            for role in resident_roles:
                selected.extend(losses_by_identity[identity][role])
    if not selected:
        raise ValueError("empty adaptation loss")
...
def run(out: Path, seeds=SEEDS, parts=PARTITIONS, assignments=ASSIGNMENTS, cultures=CULTURES, resident_modes=RESIDENT_ADAPTATION_MODES, updates=UPDATES):
    ...
    write(out / "invocation.json", {"formal": formal, "version": "v0.48-coadaptation-horizon", "seeds": list(seeds), "partitions": list(parts), "assignments": list(assignments), "cultures": list(cultures), "role_modes": list(ROLE_MODES), "conditions": list(CONDITIONS), "adaptation_schedule": ADAPTATION_SCHEDULE, "newcomer_reset_mode": NEWCOMER_RESET_MODE, "resident_adaptation_modes": list(resident_modes), "resident_update_interval": RESIDENT_UPDATE_INTERVAL, "resident_update_budget": RESIDENT_UPDATE_BUDGET, "newcomer_learning_rate": NEWCOMER_LR, "resident_learning_rate": RESIDENT_LR, "updates": updates, "checkpoints": checkpoints, "newcomer_identity": 0, "study_scope": "600-update bounded resident co-adaptation after a full newcomer communication reset"})
    for path in hashes:
