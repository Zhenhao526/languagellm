...
from qwen_language_v3.environment import AGENTS, LOCATIONS, World
...
def ready_joint():
    w = World(17, 0)
    a, b = w.roles["wood"], w.roles["partner"]
...
class EnvironmentTests(unittest.TestCase):
        ...
        for seed in (0, 17, 6101, 6148):
            for variant in range(48):
                with self.subTest(seed=seed, variant=variant):
                    ...
                    self.assertEqual(len(w.items), 8)
    ...
    def test_local_resource_frequencies_do_not_encode_the_hidden_target(self):
        for variant in range(16):
            ...
            for observer in (w.roles["wood"], w.roles["partner"]):
    ...
    def test_roads_are_permanently_open_and_no_events_fire(self):
        w = World(7558, 22)
        for step in range(12):
            self.assertFalse(w.event["enabled"])
            self.assertFalse(w.event["occurred"])
            actions = {}
            for a in AGENTS:
                self.assertEqual(w.observe(a)["known_closed_roads"], [])
                here = w.positions[a]
                destination = LOCATIONS[(LOCATIONS.index(here) + 1) % len(LOCATIONS)]
                actions[a] = {"kind": "move", "destination": destination}
            feedback = w.step(actions)
            self.assertTrue(all(x["action_succeeded"] for x in feedback.values()))
        self.assertEqual(w.status, "timeout")
        self.assertFalse(w.event["occurred"])

    def test_full_information_control_has_current_state_without_future(self):
        w = World(17, 1)
        obs = w.full_information_observe("A")
        self.assertEqual(sum(len(s["items"]) for s in obs["all_current_sites"]), 8)
    ...
    def test_long_wood_cannot_be_picked_up_or_delivered_by_one_actor(self):
        w, a, b, oid = ready_joint()
        handle = w.handles[a][oid]
        self.assertFalse(any(e["action"]["kind"] == "pickup"
                             and e["action"].get("item") == handle
                             for e in w.action_menu(a)))
        old_items = deepcopy(w.items)
        for kind in ("pickup", "deliver"):
            actions = waits()
            actions[a] = act(w, a, kind, oid)
            self.assertFalse(w.step(actions)[a]["action_succeeded"])
            self.assertEqual(w.items, old_items)
            self.assertIsNone(w.inventory[a]["cargo"])
        actions = waits()
        for actor, partner in ((a, b), (b, a)):
            actions[actor] = act(w, actor, "carry_together", oid,
        ...
        self.assertFalse(w.step(actions)[a]["action_succeeded"])
        self.assertFalse(w.items[oid]["delivered"])
        self.assertEqual(w.items[oid]["carriers"], sorted((a, b)))

    def test_delivery_still_requires_every_attribute_and_the_destination(self):
        w = World(6101, 1)
        a = w.roles["wood"]
        oid = next(oid for oid, x in w.items.items()
                   if x["kind"] == "木材" and x["target_for_witness"])
        actions = waits()
        actions[a] = act(w, a, "pickup", oid)
        self.assertTrue(w.step(actions)[a]["action_succeeded"])
        actions = waits()
        actions[a] = {"kind": "move", "destination": w.goals[0]["destination"]}
        self.assertTrue(w.step(actions)[a]["action_succeeded"])
        actions = waits()
        actions[a] = act(w, a, "deliver", oid)
        for field, wrong in (("kind", "纤维"), ("length", "长"),
                             ("condition", "湿"), ("destination", w.layout["wood_site"])):
            with self.subTest(field=field):
                clone = World.from_state_dict(w.state_dict())
                clone.goals[0][field] = wrong
                before_items = deepcopy(clone.items)
                feedback = clone.step(actions)
                self.assertFalse(feedback[a]["action_succeeded"])
                self.assertEqual(clone.score, 0)
                self.assertEqual(clone.inventory[a]["cargo"], oid)
                self.assertEqual(clone.items, before_items)
        self.assertTrue(w.step(actions)[a]["action_succeeded"])
        self.assertTrue(w.items[oid]["delivered"])
        self.assertIsNone(w.inventory[a]["cargo"])
    ...
    def test_pickup_conflict_fails_for_every_contender(self):
        w = World(17, 0)
        fiber = next(oid for oid, x in w.items.items() if x["kind"] == "纤维")
        for a in ("A", "B"):
            w.positions[a] = w.items[fiber]["location"]
        actions = waits()
        for a in ("A", "B"):
            actions[a] = act(w, a, "pickup", fiber)
        feedback = w.step(actions)
        self.assertFalse(feedback["A"]["action_succeeded"])
        self.assertFalse(feedback["B"]["action_succeeded"])
        self.assertEqual(w.items[fiber]["carriers"], [])
    ...
    def test_processing_and_tool_actions_are_absent_and_cut_is_rejected(self):
        for variant in (0, 1, 22, 31, 47):
            w = World(6101, variant)
            self.assertTrue(all(x["processed"] for x in w.items.values()))
            self.assertEqual({x["kind"] for x in w.items.values()}, {"木材", "纤维"})
            self.assertTrue(all(inv["tool"] is None for inv in w.inventory.values()))
            for a in AGENTS:
                menu = w.action_menu(a)
                self.assertFalse(any(x["action"]["kind"] == "cut" for x in menu))
                self.assertNotIn("斧具", json.dumps(menu, ensure_ascii=False))
    ...
    def test_restore_rejects_states_from_the_old_processing_or_road_family(self):
        w = World(6101, 0)
        oid = next(iter(w.items))
        mutations = (
            lambda s: s["scenario_parameters"].update(family="river_valley_v0.2"),
            lambda s: s["event"].update(enabled=True),
            lambda s: s["event"].update(occurred=True),
            lambda s: s["items"][oid].update(processed=False),
            lambda s: s["items"][oid].update(kind="斧具"),
            lambda s: s["inventory"]["A"].update(tool=oid),
            lambda s: s["known_closed"]["A"].append(["营地", "林地"]),
        )
