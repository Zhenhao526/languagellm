    ...
    for row in curve:
        ...
        for d in (0,1):
            ...
            for mode,probs in [('native',native),('greedy',np.eye(49)[greedy@np.array([7,1])])]:
                for group,pool in pools.items():
                    mask=np.isin(mids,pool);by_map=np.zeros((36,49));normalized=probs/probs.sum(1,keepdims=True)
                    for world in np.flatnonzero(mask):by_map[positions[world,0]*6+positions[world,1]]+=normalized[world]/int(mask.sum())
                    c=by_map.T.reshape(49,6,6)
...
def audit_batch(batch,audit,preflight):
    ...
    audit.check(inv['arms']==list(ARMS),'declared_two_arms')
    if inv['formal']:
        audit.check(cfg['seeds']==list(SEEDS) and cfg['partitions']==[1,2,3] and cfg['private_updates']==cfg['social_updates']==2400 and cfg['batch']==512 and cfg['eval_n']==9600,'formal_fixed_matrix_budget')
        audit.check(complete['private_runs']==complete['social_runs']==24,'formal_all_48_stages_complete')
        gate=read(batch/'frozen_sources/preflight_qa.json');gate_source=ROOT/'results/smoke_002/audit_v13_successful_gate_88572afb22dc.py'
        audit.check(gate['passed'] and gate['source_hashes']==hashes and sha(gate_source)==gate['script_sha256'],'formal_success_gate_has_exact_archived_source')
    prepared_fingerprints=[]
    for seed in cfg['seeds']:
        report=read(batch/f'preparation_{seed}.json');prepared_state=load(batch/f'prepared_{seed}.pt');prepared_fingerprints.append(state_digest(prepared_state))
        audit.check(report['seed']==seed and all(x['updates']==200 and x['choices']==12800 for x in report['two_sites']),'new_seed_resource_preparation_200x64',seed)
    audit.check(len(set(prepared_fingerprints))==len(cfg['seeds']),'prepared_seed_starts_pairwise_distinct')
    for seed,p in product(cfg['seeds'],cfg['partitions']):
        ...
        for arm in ARMS:
            ...
            audit.check(c['updates']==cfg['private_updates'] and c['batch_pairs_per_person']==cfg['batch'] and c['learning_rate']==.0007 and c['gradient_clip']==2 and c['coefficient']==(0. if arm=='control' else .1) and c['entropy_off_after']==2100 and c['fitted_prefixes']==list(PRIVATE) and c['capability_results_never_select_sources'],'private_fixed_parameters',folder.name)
            ...
            audit.check(same(final,load(folder/f"checkpoint_{c['updates']:04d}.pt")),'private_final_matches_last_checkpoint',folder.name)
            ...
            for step,line in enumerate(lines,1):
                ...
                for who,person in enumerate(line['persons']):
                    ...
                    u=np.random.default_rng(person['action_seed']).random((2*c['batch_pairs_per_person'],1)).astype(np.float32)
                    audit.check(named_arrays_sha({'uniform':u})==person['action_uniform_sha256'],'private_full_uniform_stream_reconstructed',[folder.name,step,who])
                    audit.check(person['components']['weight']==c['coefficient'] and person['components']['entropy_coefficient']==(.02 if step<=2100 else 0.),'private_objective_and_entropy_fixed',[folder.name,step,who])
                    audit.check(np.isfinite(person['gradient_norm']) and np.isfinite(list(person['components'].values())).all(),'private_all_update_finite',[folder.name,step,who])
            ...
            audit.check(sc['updates']==cfg['social_updates'] and sc['batch']==cfg['batch'] and sc['eval_n']==cfg['eval_n'] and sc['learning_rate']==.0007 and sc['entropy_off_after']==2100 and sc['role_loss_weights']=={'sender':.5,'receiver':.5} and sc['private_outcomes_never_filter_runs'],'social_fixed_parameters',social.name)
            audit.check(same(load(social/'final.pt'),load(social/f"checkpoint_{sc['updates']:04d}.pt")),'social_final_matches_last_checkpoint',social.name)
            ...
            for step,line in enumerate(lines):
                ...
                audit.check(line['update']==step+1 and line['entropy_weight']==(.02 if step<2100 else 0.),'social_all_update_entropy_schedule',[social.name,step])
                for who,person in enumerate(line['agents']):
                    audit.check([q['role'] for q in person['parts']]==['sender','receiver'] and all(q['n']==sc['batch']//2 for q in person['parts']),'social_equal_two_roles_and_directions',[social.name,step,who])
                    totals=[q['policy_loss']+q['value_loss']-line['entropy_weight']*q['entropy'] for q in person['parts']]
                    audit.check(abs(float(np.mean(totals))-person['loss'])<2e-6 and all(np.isfinite(v) for v in person['gradient_norm_by_role'].values()),'social_logged_loss_weights_and_finite',[social.name,step,who])
    ...
    runs=len(cfg['seeds'])*len(cfg['partitions'])*2
    audit.check(audit.counts['private_runs']==audit.counts['social_runs']==runs,'all_planned_stages_audited')
    audit.check(audit.counts['private_person_updates']==runs*cfg['private_updates']*2 and audit.counts['social_updates']==runs*cfg['social_updates'],'all_planned_update_records_audited')
    audit.counts['private_actual_single_goal_actions']=audit.counts['private_person_updates']*cfg['batch']*2
    audit.counts['social_training_communications']=audit.counts['social_updates']*cfg['batch']
    audit.counts['social_training_actions']=audit.counts['social_training_communications']*2
    return inv

def main():
    parser=argparse.ArgumentParser();parser.add_argument('--root',type=Path,default=ROOT/'results/spatial_001');parser.add_argument('--preflight',action='store_true');args=parser.parse_args();batch=args.root.resolve();archive=batch/'audit_execution_source.py'
    if archive.exists() and sha(archive)!=sha(__file__):archive.rename(archive.with_name('audit_execution_source_'+sha(archive)[:12]+'.py'))
    archive.write_bytes(Path(__file__).read_bytes());audit=Audit();mathematical_tests(audit);inv=audit_batch(batch,audit,args.preflight)
    ...
    result=dict(passed=not audit.failures,created_utc=datetime.now(timezone.utc).isoformat(),batch=str(batch),source_hashes=inv['source_hashes'],checks=dict(audit.checks),check_count=sum(audit.checks.values()),counts=dict(audit.counts),failures=audit.failures,script_sha256=sha(__file__),independent_helper_sha256={str(p):sha(p) for p in (PROJECT/'redesign_v0.11/audit_query_replay.py',PROJECT/'redesign_v0.12/audit_receiver.py')},rng_audit=dict(path=str(rng_path),sha256=sha(rng_path)) if rng else None,scope='Independent legal-triple/JSD math; reconstructed all training worlds and private action uniforms; frozen states, Adam step counts, exact initialization/transfer; step1 private/social policy/loss/gradient/Adam exact replay and step2101 policy/loss/gradient replay; all final modes policy/world/statistics replay; every private capability checkpoint logits and every social protocol checkpoint logits replay; terminal private all720 JSD and all protocol probability metrics independently recomputed. RNG namespace enumeration separately conducted by root.',development_revision='smoke001 retained: private and social RNG namespace13013 collided; smoke002 executed socialworld namespace13014; no formal training had started before fix.')
