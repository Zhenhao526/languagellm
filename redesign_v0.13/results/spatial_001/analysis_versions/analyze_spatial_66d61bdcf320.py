...
def plots(result,out):
    ...
    axes[0].set(xticks=range(3),xticklabels=['Old18','Added6','Sealed6'],ylim=(0,102),ylabel='Greedy joint action success (%)',title='Private action capability')
    axes[0].legend(frameon=False,fontsize=8)
    for ax,title,ylabel in [(axes[1],'Policy consistency over all720 permutations','Mean JSD (nats)'),(axes[2],'Action uncertainty','Mean branch policy entropy (nats)')]:
    ...
    for ax,g,title in zip(axes,GROUPS,('Old18: socially trained','Added6: never trained','Sealed6: primary never-trained set')):
        for arm in ARMS:
            for seed in seeds:
    ...
    axes[0].set(title='Same native sender distribution',ylabel='Common30 joint success (%)',ylim=(-2,102));axes[0].legend(frameon=False,fontsize=6)
    axes[1].set(title='Greedy sender + greedy receiver',ylabel='Common30 natural N (%)',ylim=(-2,102));axes[1].legend(frameon=False,fontsize=8)
    ...
    fig.suptitle('Exact49-code analysis on fixed16 photo pairs; N changes sender policy and is not subtracted from the native oracle',fontsize=9)
...
def report(r):
    ...
    lines+=['','社会normal是逐符号贪心发信与贪心接收N。固定9600世界的照片从原8张test池抽取，和精确协议评价的固定4×4照片口径不同，不直接相减或把两次测量当独立复制。新增6和封存6均未进入两阶段任务反馈，主指标固定封存组，不选择两组中表现较好者。','',
