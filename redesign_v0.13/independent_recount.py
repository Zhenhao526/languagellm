...
def main():
    ...
    inv=read(batch/'invocation.json');seeds=inv['args']['seeds'];parts=inv['args']['partitions'];arms=inv['arms']
    updates=inv['args']['social_updates'];assert read(batch/'training_complete.json')['status']=='complete'
    folders=[batch/f'social_s{s}_p{p}_{a}' for s,p,a in product(seeds,parts,arms)]
    assert all((f/'result.json').exists() for f in folders)
    rows=[];hashes={};value_checks=0
    for folder in folders:
        cfg=read(folder/'config.json');p=cfg['partition'];row={k:cfg[k] for k in ('seed','partition','arm')};row['modes']={}
    ...
    for seed in seeds:
        ...
        paired.append(dict(seed=seed,**means,delta=means['equivariant']-means['control']))
    result=dict(passed=True,completed_social_runs=len(rows),checked_scalar_values=value_checks,rows=rows,primary_by_seed=paired,
        primary_mean_difference=float(np.mean([x['delta'] for x in paired])),source_hashes=hashes,recount_source_sha256=sha(__file__),
