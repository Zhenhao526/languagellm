"""Finite v0.33 evaluation for single- and dual-sender token protocols."""
...
def donor_indices(raw,p,arm):
    ...
    for i in targets:
        ...
        # Keep all four combinations of the two food and two water donors.
        # The ordering is f0-w0, f0-w1, f1-w0, f1-w1.
...
def partner_pairs(raw,p,arm):
    """Full two-edge success for each fixed-resource pair in target12.

    For every food (or water) location, the target has two partner locations.
    Rows are matched on the same test photograph pair and mask, so a fixed
    one-resource output cannot score both rows of a group.
    """
    target=np.flatnonzero(np.isin(raw['map_id'],support.groups(p,arm)['target12']))
    codes=raw['tokens'][:,0]*7+raw['tokens'][:,1];acts=raw['receiver_logits'].argmax(-1)[codes]
    correct=(acts==raw['positions']).all(-1)
    out={}
    for kind,name in ((0,'food_partner_pair_J'),(1,'water_partner_pair_J')):
        vals=[]
        for location in range(6):
            loc=target[raw['positions'][target,kind]==location]
            for photo in np.unique(raw['photo_ids'][loc],axis=0):
                for shown in (0,1):
                    rows=loc[(raw['shown'][loc]==shown)&(raw['photo_ids'][loc]==photo).all(-1)]
                    assert len(rows)==2
                    vals.append(bool(correct[rows].all()))
    ...
    for scope,ids in support.groups(p,arm).items():
        ...
        for label,mask in [('pooled',selected),('food_only',selected&(raw['shown']==0)),('water_only',selected&(raw['shown']==1))]:
            ...
            if scope=='target12':
                ...
                if label=='pooled': block[label].update(pairs)
        result[scope]=block
    return result
