"""Read-only v0.8 complementary-reward protocol probes, fixed before this batch's training.
...
ANALYSIS_PLAN = dict(
    timing='Fixed before v0.8 formal training; identical protocol tests adapted from v0.7',
    scope='All 60 planned runs and 120 directions; each receives 100 whole-code recoding references',
...
def markdown(result):
    ...
    lines += ['', 'additive、mixed、joint使用相同identity视觉接口和双7符号信道，每场景均发一次消息、做两需求行动，只改变两资源成功的回报函数。每种子三个full条件及三个blocked条件均各计为一个split0运行，不复制到三个留出划分。自然消息平均查询成功和双需求同时正确是跨奖励条件可比较的行为指标。', '',
        '## 保持原生行为的完整消息重编码', '',
...
def main():
    ...
    parser.add_argument('batch', type=Path, nargs='?', default=ROOT / 'results/complementarity_001')
