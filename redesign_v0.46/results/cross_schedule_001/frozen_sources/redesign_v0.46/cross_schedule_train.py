"""Train a freshly reset newcomer across crossed adaptation schedules."""
...
def make_newcomer(permutation, base_agent, seed, part, assignment_index, culture_index):
    newcomer = copy.deepcopy(base_agent)
    permutation.replace_actor(newcomer)
    reset = reset_seed(seed, part, assignment_index, culture_index)
    reset_record = permutation.T.reset_communication(newcomer, reset)
    newcomer.train()
...
def adapt_step(permutation, agents, views_train, worlds, seed, part, step, role_mode, adaptation_schedule):
...
def run(out: Path, seeds=SEEDS, parts=PARTITIONS, assignments=ASSIGNMENTS, cultures=CULTURES, adaptation_schedules=ADAPTATION_SCHEDULES, updates=UPDATES):
    ...
    formal = seeds == SEEDS and parts == PARTITIONS and assignments == ASSIGNMENTS and cultures == CULTURES and tuple(adaptation_schedules) == ADAPTATION_SCHEDULES and updates == UPDATES
    checkpoints = sorted(set(t for t in CHECKPOINTS if t <= updates) | {updates})
    hashes, inputs = source_hashes(), input_hashes()
    write(out / "invocation.json", {"formal": formal, "version": "v0.46-cross-adaptation-schedules", "seeds": list(seeds), "partitions": list(parts), "assignments": list(assignments), "cultures": list(cultures), "role_modes": list(ROLE_MODES), "conditions": list(CONDITIONS), "adaptation_schedules": list(adaptation_schedules), "updates": updates, "checkpoints": checkpoints, "newcomer_identity": 0, "newcomer_reset": "communication_modules_only", "source_hashes": hashes, "input_hashes": inputs, "torch_version": str(torch.__version__), "numpy_version": np.__version__, "platform": platform.platform(), "threads": 1, "study_scope": "fully crossed social recovery of a freshly reset communication newcomer inside v0.43 resident cultures"})
    for path in hashes:
    ...
    for seed, part, assignment, culture_label, adaptation_schedule in itertools.product(seeds, parts, assignments, cultures, adaptation_schedules):
        ...
        newcomer, reset_record = make_newcomer(permutation, base_agents[0], seed, part, assignment_index[assignment], culture_index[culture_label])
        agents[0] = newcomer
        ...
        optimizer = torch.optim.Adam([p for group in groups.values() for p in group], lr=0.0007)
        chain = out / "social" / f"s{seed}_p{part}_{assignment}_{culture_label}__adapt_{adaptation_schedule}"
        chain.mkdir(parents=True, exist_ok=True)
        write(chain / "config.json", {"seed": seed, "partition": part, "assignment": assignment, "resident_culture": culture_label, "resident_role_mode": mode, "resident_condition": condition, "adaptation_schedule": adaptation_schedule, "adaptation_role_mode": mode, "newcomer_identity": 0, "updates": updates, "checkpoints": checkpoints, "communication_initialization": "fresh_random_reset_newcomer_only", "reset_record": reset_record, "evaluation_schedules": list(SCHEDULES), "train_map_count": 60, "test_map_count": 120})
        curve = []
        for step in range(updates + 1):
            ...
            if step == updates:
                break
            loss, fixture_slots, trace_slots, role_slots, schedule = adapt_step(permutation, agents, views, train_worlds, seed, part, step, mode, adaptation_schedule)
            ...
            with (chain / "training.jsonl").open("a") as log:
                log.write(json.dumps({"update": step + 1, "global_step": step, "schedule": schedule, "role_mode": mode, "role_permutations": {str(slot): list(role_slots[slot]) for slot in role_slots}, "loss": float(loss.detach()), "norms": norms}, ensure_ascii=False) + "\n")
        rows.append({"seed": seed, "partition": part, "assignment": assignment, "resident_culture": culture_label, "adaptation_schedule": adaptation_schedule, "file": str(chain.relative_to(out))})
        total += 1
        print(json.dumps({"phase": "newcomer_adaptation", "chain": chain.name, "runs": total, "seconds": round(time.monotonic() - started, 2)}, ensure_ascii=False), flush=True)
    ...
    write(out / "training_complete.json", {"status": "complete", "formal": formal, "probe": "cross_adaptation_schedules", "runs": total, "updates_per_run": updates, "resident_cultures": list(cultures), "adaptation_schedules": list(adaptation_schedules), "checkpoints": checkpoints, "trace_files_expected": total * 8, "protocol_files_expected": total * len(checkpoints) * len(SCHEDULES) * 4, "seconds": time.monotonic() - started, "source_hashes": hashes, "input_hashes": inputs, "files": files})
    return {"status": "complete", "formal": formal, "runs": total, "seconds": time.monotonic() - started, "source_sha256": sha(ROOT / "cross_schedule_train.py")}


def source_hashes():
    paths = [ROOT / "cross_schedule_train.py", ROOT / "cross_schedule_design.json", V043 / "permutation_train.py", V043 / "permutation_design.json", V039 / "triad_train.py", V039 / "triad_design.json"]
...
def main():
    ...
    if args.dev:
        result = run(args.out.resolve(), seeds=(SEEDS[0],), parts=(PARTITIONS[0],), assignments=(ASSIGNMENTS[0],), cultures=(CULTURES[0],), adaptation_schedules=(ADAPTATION_SCHEDULES[0],), updates=min(args.updates, 5))
    else:
