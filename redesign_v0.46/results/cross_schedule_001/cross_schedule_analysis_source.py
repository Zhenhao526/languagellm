"""Independent NumPy reanalysis of the v0.46 crossed adaptation batch."""
...
def nmi(x, y):
    ...
    for xv in np.unique(x):
        ...
        for yv in np.unique(y):
            pxy = np.mean((x == xv) & (y == yv)); py = np.mean(y == yv)
            if pxy > 0: mi += pxy * np.log(pxy / (px * py))
    hy = 0.0
    for yv in np.unique(y):
        py = np.mean(y == yv)
        if py > 0: hy -= py * np.log(py)
...
def sequence_metrics(raw_by_schedule):
    # Compare newcomer identity 0 against each resident identity on the same
    # semantic resource.  Team topology determines which team supplies each
    # sender, while role 012 fixes the evaluation axis.
    ...
    for schedule in SCHEDULES:
        ...
        for resource in range(RESOURCES):
            ...
            for identity in (1, 2, 3):
                resident = sender_table[resource, identity]
                token0.append(float(np.mean(newcomer[:, 0] == resident[:, 0])))
                token1.append(float(np.mean(newcomer[:, 1] == resident[:, 1])))
                pair.append(float(np.mean(np.all(newcomer == resident, axis=1))))
            # Position information is computed only for the newcomer sender.
...
def run(out: Path):
    ...
    def exact(actual, expected, label):
        a, b = np.asarray(actual), np.asarray(expected); require(a.shape == b.shape, label + " shape"); require(np.array_equal(a, b), label)
    def close(actual, expected, label, atol=1e-6, rtol=1e-6):
        nonlocal comparisons, max_error
        a, b = np.asarray(actual, dtype=float), np.asarray(expected, dtype=float); require(a.shape == b.shape, label + " shape"); require(np.isfinite(a).all() and np.isfinite(b).all(), label + " finite"); comparisons += int(a.size); err = float(np.max(np.abs(a-b))) if a.size else 0.0; max_error = max(max_error, err); require(np.allclose(a,b,atol=atol,rtol=rtol), label)
    invocation = read(OUT / "invocation.json"); complete = read(OUT / "training_complete.json")
    require(invocation.get("formal") is True and invocation.get("version") == "v0.46-cross-adaptation-schedules", "formal invocation")
    require(tuple(invocation.get("adaptation_schedules", ())) == ADAPTATION_SCHEDULES, "adaptation schedule coverage")
    require(complete.get("formal") is True and complete.get("status") == "complete" and complete.get("runs") == 1296, "formal completion")
    rows, sequence_rows = [], []
    for seed, part, assignment, culture_label, adaptation_schedule in itertools.product(SEEDS, PARTITIONS, ASSIGNMENTS, CULTURES, ADAPTATION_SCHEDULES):
        mode, condition = culture_label.split("__", 1)
        chain = OUT / "social" / f"s{seed}_p{part}_{assignment}_{culture_label}__adapt_{adaptation_schedule}"
        cfg = read(chain / "config.json")
        require(cfg["seed"] == seed and cfg["partition"] == part and cfg["assignment"] == assignment and cfg["resident_culture"] == culture_label and cfg["adaptation_role_mode"] == mode and cfg["adaptation_schedule"] == adaptation_schedule, f"{chain.name} config")
        curve = read(chain / "curve.json"); exact([int(x["update"]) for x in curve], list(UPDATES), f"{chain.name} curve updates")
            ...
            for si, schedule in enumerate(SCHEDULES):
                ...
                for slot in range(4):
                    ...
                    for role_i, role_key in enumerate(ROLE_KEYS):
                        ...
                        literal = metric(actions, raw["positions"], raw["map_id"], part)["target60"]
                        equivariant = metric(actions, raw["positions"][:, np.asarray(ROLE_PERMS[role_i], dtype=np.int64)], raw["map_id"], part)["target60"]
                        rows.append({"seed":seed,"partition":part,"assignment":assignment,"resident_culture":culture_label,"role_mode":mode,"condition":condition,"adaptation_schedule":adaptation_schedule,"update":update,"schedule":schedule,"slot":slot,"role_permutation":role_key,"literal":literal,"equivariant":equivariant})
                    coverage["protocol_files"] += 1
            sequence_rows.append({"seed":seed,"partition":part,"assignment":assignment,"resident_culture":culture_label,"role_mode":mode,"condition":condition,"adaptation_schedule":adaptation_schedule,"update":update,"schedule":"all",**sequence_metrics(raw_by_schedule)})
        coverage["chains"] += 1
    ...
    # Aggregate once while traversing the saved rows.  Re-filtering the full
    # row table for every culture/schedule/update/role cell is quadratic and
    # becomes prohibitively slow for the 1,296-chain batch.
    identity_groups = defaultdict(lambda: defaultdict(list))
    spread_groups = defaultdict(list)
    for row in rows:
        group = (row["resident_culture"], row["adaptation_schedule"], row["update"])
        if row["role_permutation"] == "012":
            ...
            for metric_name in METRICS[1:]:
                identity_groups[group][f"equivariant_{metric_name}"].append(row["equivariant"][metric_name])
        spread_groups[(row["resident_culture"], row["adaptation_schedule"], row["update"], row["seed"], row["partition"], row["assignment"], row["schedule"], row["slot"])].append(row["equivariant"]["J"])
    sequence_groups = defaultdict(lambda: defaultdict(list))
    for row in sequence_rows:
        group = (row["resident_culture"], row["adaptation_schedule"], row["update"])
        for metric_name in ("newcomer_resident_token0_agreement", "newcomer_resident_token1_agreement", "newcomer_resident_pair_agreement", "newcomer_token0_position_nmi", "newcomer_token1_position_nmi", "newcomer_pair_position_nmi"):
            sequence_groups[group][metric_name].append(row[metric_name])
    ...
    for culture in CULTURES:
        for adaptation_schedule in ADAPTATION_SCHEDULES:
            for update in UPDATES:
                group = (culture, adaptation_schedule, update)
                cell = summary[culture][adaptation_schedule][str(update)]
                ...
                for metric_name in METRICS[1:]:
                    cell[f"identity_equivariant_target60_{metric_name}"] = ms(identity_groups[group][f"equivariant_{metric_name}"])
                spread_values = [max(values) - min(values) for key, values in spread_groups.items() if key[:3] == group]
                cell["role_spread_target60_J"] = ms(spread_values)
                for metric_name in sequence_metric_names:
                    cell[metric_name] = ms(sequence_groups[group][metric_name])
    record = {"status":"complete","formal":True,"probe":"cross_adaptation_schedules","runs":1296,"updates":list(UPDATES),"cultures":list(CULTURES),"adaptation_schedules":list(ADAPTATION_SCHEDULES),"summary":summary,"rows":rows,"sequence_rows":sequence_rows,"checks":checks,"scalar_comparisons":comparisons,"maximum_metric_absolute_difference":max_error,"coverage":dict(coverage),"source_sha256":sha(ROOT/"cross_schedule_analysis.py"),"v043_training_complete_sha256":sha(V043/"results/permutation_001/training_complete.json"),"limits":["Resident formation culture and newcomer adaptation schedule are fully crossed, but resident parameters remain frozen during adaptation.","Newcomer shares a frozen visual frontend family with residents; perception learning is not tested.","Finite six-site two-token grounded protocol; recovery is not open-ended language learning."]}
    out.mkdir(parents=True, exist_ok=True)
    shutil.copy2(ROOT/"cross_schedule_analysis.py",out/"cross_schedule_analysis_source.py")
    return record


def main():
    parser=argparse.ArgumentParser(); parser.add_argument("--out",type=Path,required=True); args=parser.parse_args(); started=time.monotonic(); result=run(args.out.resolve()); result["seconds"]=time.monotonic()-started; (args.out.resolve()/"cross_schedule_analysis.json").write_text(json.dumps(result,ensure_ascii=False,indent=2,allow_nan=False)+"\n"); print(json.dumps({"status":result["status"],"runs":result["runs"],"checks":result["checks"],"scalar_comparisons":result["scalar_comparisons"],"maximum_metric_absolute_difference":result["maximum_metric_absolute_difference"],"seconds":result["seconds"]},ensure_ascii=False))
