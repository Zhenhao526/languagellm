...
from audit_execution import enumerate_sender, enumerate_receiver, independent_direction
...
def check_fixture(seed, panel, step, condition, world_table, raw, log, cache, agents, c):
    ...
    for slot, (i, j) in enumerate(matching):
        ...
        _, _, left = independent_direction(agents[i], agents[j], torch.from_numpy(cache['train', PRIVATE_TYPES[i]][idx]), f['uniforms'][:, :4], world_table['positions'][idx], ew)
        _, _, right = independent_direction(agents[j], agents[i], torch.from_numpy(cache['train', PRIVATE_TYPES[j]][idx]), f['uniforms'][:, 4:], world_table['positions'][idx], ew)
        apply_joint_trace(left, right, ew)
        expected_trace.update({prefix + f'i{i}_j{j}__{key}': value for key, value in left.items()})
        expected_trace.update({prefix + f'i{j}_j{i}__{key}': value for key, value in right.items()})
    ...
    for seed, panel in itertools.product(seeds, panels):
        ...
        for condition in stats.CONDITIONS:
            ...
            for step in sorted({0, times[-1] - 1} | ({2100} if times[-1] > 2100 else set())):
                check_fixture(seed, panel, step, condition, tables['train'], npz(folder / f'train_{step + 1:04d}.npz'), logs[step], cache, agents, c)
