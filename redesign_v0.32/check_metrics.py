...
class Tests(unittest.TestCase):
    def test_known_factored_protocol_and_shuffle_supports(self):
        ...
        for p,arm in itertools.product((1,2,3),support.ARMS):
            ...
            self.assertAlmostEqual(s['target12']['pooled']['within_shuffle_J'],1/12)
            self.assertEqual(s['target12']['pooled']['recombine_FW_J'],1)
            self.assertEqual(s['target12']['pooled']['recombine_WF_J'],0)
    ...
    def test_identity_bijection_and_coordinate_swap(self):
        ...
        np.testing.assert_array_equal(r['FW'][0],np.ones(72));np.testing.assert_array_equal(r['WF'][0],np.zeros(72))
