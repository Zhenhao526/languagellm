"""Read-only six-site protocol probes, fixed before this batch's training.
...
ANALYSIS_PLAN = dict(
    timing='Fixed before v0.6 formal training; adapted from v0.5 exploratory analysis',
    scope='Every planned run; all two-token conditions receive whole-code recoding references',
...
def markdown(result):
    ...
    lines += ['', '原子49码只报告自然消息表现，不人为拆成符号片段。full_direct与full_blocked各种子只有一个split0运行，不复制到三个留出划分。', '',
        '## 保持原生行为的完整消息重编码', '',
...
def main():
    ...
    parser.add_argument('batch', type=Path, nargs='?', default=ROOT / 'results/recombination_001')
    ...
    for seed, condition in expected_order:
        ...
        eligible = not args.full_only or condition in ('full_direct', 'full_blocked')
