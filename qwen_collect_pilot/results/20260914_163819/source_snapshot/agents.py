...
def build_messages(role, observation, history, received=None, known_protocol=False):
    ...
    if known_protocol:
        if history or role == 'C':
            raise ValueError('The capability control uses fresh collector contexts only')
        slots = '，'.join(f"槽位{item['slot']}放{item['resource']}" for item in observation['slots'])
        return [{'role':'user','content':f'你是{role}。约定{KNOWN_PROTOCOL[role]}{slots}本轮收到{received}。只输出应选槽位0或1。'}]
