...
def main():
    ...
    inv=read(batch/'invocation.json');seeds=inv['args']['seeds'];parts=inv['args']['partitions'];arms=['neither','policy_only_scale','value_only_scale','both'];reset_batch=Path(inv['args']['reset_batch']);scaled_batch=Path(inv['args']['scaled_batch'])
    updates=inv['args']['social_updates'];assert read(batch/'training_complete.json')['status']=='complete'
    folders=[(reset_batch/f'social_s{s}_p{p}_reset' if a=='neither' else scaled_batch/f'social_s{s}_p{p}_reset_scaled' if a=='both' else batch/f'social_s{s}_p{p}_{a}',a) for s,p,a in product(seeds,parts,arms)]
    ...
    for seed in seeds:
        ...
        contrasts=dict(route_difference=means['policy_only_scale']-means['value_only_scale'],policy_given_value0=means['policy_only_scale']-means['neither'],policy_given_value1=means['both']-means['value_only_scale'],value_given_policy0=means['value_only_scale']-means['neither'],value_given_policy1=means['both']-means['policy_only_scale'],interaction=means['both']-means['policy_only_scale']-means['value_only_scale']+means['neither'])
        paired.append(dict(seed=seed,**means,delta=contrasts['route_difference'],contrasts=contrasts))
