"""Read-only final comparison of v16 raw recount, all seed metrics and six contrasts."""
from pathlib import Path
from itertools import product
import hashlib,json
import numpy as np
ROOT=Path(__file__).resolve().parent
OUT=ROOT/'results/branches_001'
...
WEIGHTS={'route_difference':{'policy_only_scale':1,'value_only_scale':-1},
...
a=read(OUT/'branches_analysis.json');b=read(OUT/'independent_recount.json');numbers=read(OUT/'report_numbers.json')
assert a['status']=='complete' and b['passed'] and len(a['runs'])==len(b['rows'])==48
...
for name,weights in WEIGHTS.items():
    ...
    for k in next(iter(independent.values())):
        ...
        for i,s in enumerate(a['seeds']):check(target['seed_differences'][k][i],deltas[i],'six_contrasts_seed_all_metrics',str((name,s,k)))
        check(target['mean_differences'][k],np.mean(deltas),'six_contrasts_mean_all_metrics',name+k)
        check(target['difference_ranges'][k][0],min(deltas),'six_contrasts_range_all_metrics',name+k+'min')
        check(target['difference_ranges'][k][1],max(deltas),'six_contrasts_range_all_metrics',name+k+'max')
...
result=dict(passed=not errors,scalar_checks=sum(counts.values()),checks_by_category=counts,max_absolute_error=max_error,tolerance=1e-9,failures=errors,
 analysis_sha256=sha(OUT/'branches_analysis.json'),independent_recount_sha256=sha(OUT/'independent_recount.json'),report_numbers_sha256=sha(OUT/'report_numbers.json'),comparison_source_sha256=sha(__file__),extra_input_hashes=extra_inputs,
 scope='All independent raw endpoint and protocol values, saved learning curves, every analyzed metric in four source seeds, normalized AUC and all six contrasts; raw NumPy expected reward supplements the independent recount. No training, inference, new estimand or result selection.')
