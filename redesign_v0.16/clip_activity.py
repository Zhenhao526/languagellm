"""Post-outcome exploratory audit of already logged clipping activity; no model calls."""
...
for seed in args['seeds']:
    for partition in args['partitions']:
        for arm in arms:
            ...
            original='reset' if arm=='neither' else 'reset_scaled' if arm=='both' else arm
...
out=dict(status='complete',created_utc=datetime.now(timezone.utc).isoformat(),
    scope='Exploratory after the v16 primary outcome was read; logged pre-clip norms only, no new training, inference, fitted predictor or change to fixed primary/auxiliary outcomes.',
...
(B/'clip_activity_exploratory.json').write_text(json.dumps(out,indent=2)+'\n')
