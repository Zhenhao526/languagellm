...
def main():
    ...
    expected = {r['id'] for key, n in selection['quotas'].items()
                for r in [x for x in queue['reserve'][key] if x['id'] not in consumed][:n]}
    chosen = {r['metadata']['id']: r for r in selection['rows']}
    check(set(chosen) == expected and len(expected) == 17, 'Selection differs from fixed next17')
    check(len({r['metadata']['cluster_id'] for r in chosen.values()}) == 17, 'Duplicate reserved cluster')
    ...
    check(dup['old_available_pixels'] == 90 and len(dup['old_unavailable']) == 11 and dup['previous_batch_pixels'] == 43, 'Historical pixel coverage')
    n = len(successes)
    check(dup['comparisons'] == 133*n+n*(n-1)//2, 'Pair coverage')
    hashes = dup['pixel_hash_rows']
    reproduced = []
    for i, item in enumerate(hashes[133:], 133):
    ...
    qa = {'status': 'passed', 'checks': count, 'failures': 0, 'scope': 'Offline technical audit, not visual or license acceptance',
        'planned': 17, 'processed': len(manifest['records']), 'downloaded': n, 'ordinary': len(expected_packet),
        ...
        'retry_after_checks': retry_checks, 'old_pixels': 90, 'old_pixel_gaps': 11, 'previous_batch_pixels': 43,
