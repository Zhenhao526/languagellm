...
def main():
    ...
    flags=[];full_revisions=[];dimension_records=[];format_records=[]
    ...
    for i,a in enumerate(new):
        ...
        format_records.append(dict(blind_id=a['blind_id'],decoded_format=decoded.format,mime=meta['mime'],frames=getattr(decoded,'n_frames',1),
            ordinary_single_image=decoded.format=={'image/jpeg':'JPEG','image/png':'PNG'}[meta['mime']] and getattr(decoded,'n_frames',1)==1))
        oriented_size=ImageOps.exif_transpose(decoded).size
        assert (meta['width'],meta['height']) in (decoded.size,oriented_size)
        dimension_records.append(dict(blind_id=a['blind_id'],raw_size=list(decoded.size),
            exif_oriented_size=list(oriented_size),metadata_size=[meta['width'],meta['height']],orientation=decoded.getexif().get(274)))
    ...
    write(OUT/'offline_finish_qa.json',dict(data_integrity_passed=True,retry_after_utc_qa_passed=all(w['utc_interval_satisfies_header'] for w in waits),downloaded=24,original_sha1_checks=24,legacy_crop_equal_checks=24,
        ...
        image_dimension_records=dimension_records,
        format_records=format_records,format_quarantine=[r['blind_id'] for r in format_records if not r['ordinary_single_image']],
        format_note='One MIMEimage/jpeg decodes as two-frameMPO. Its original identity is verified; first frame was rendered for review, but it remains quarantined from dataset acceptance rather than silently extending the ordinary JPEG/PNG criterion.',
