"""Build the Chinese v0.44 partner-transfer report."""
...
LABELS = {"static_role__fixed_A": "静态/A", "static_role__rotating_AB": "静态/A-B", "static_role__random_ABC": "静态/A-B-C", "random_role__fixed_A": "随机/A", "random_role__rotating_AB": "随机/A-B", "random_role__random_ABC": "随机/A-B-C"}
...
CATEGORY_LABELS = {"same_culture": "同一文化", "same_role_mode_different_topology": "同角色模式、不同拓扑", "different_role_mode_same_topology": "不同角色模式、同拓扑", "different_role_mode_and_topology": "角色模式和拓扑都不同"}
REPLACEMENTS = ("none", "slot0", "slot1", "slot2", "all")
REPLACEMENT_LABELS = {"none": "不替换", "slot0": "替换 sender 槽 0", "slot1": "替换 sender 槽 1", "slot2": "替换 sender 槽 2", "all": "替换全部 sender"}
...
def fmt(item, digits=2):
    return f"{pct(item['mean']):.{digits}f}%（SD {pct(item['sd']):.{digits}f}%）"


def avg_category(data, category, replacement, metric="equivariant_target_J"):
    return float(np.mean([data["category_summary"][category][culture][replacement][metric]["mean"] for culture in CULTURES]))
...
def main():
    ...
    native = avg_category(data, "same_culture", "none")
    lines = [
        ...
        "",
        f"正式输出包含 {complete['groups']} 个视觉组、{complete['cultures']} 个文化终点、{complete['role_permutations']} 种角色排列和 {len(REPLACEMENTS)} 种替换模式。设计：[partner_transfer_design.json]({(ROOT / 'partner_transfer_design.json').resolve()})；输入绑定：[invocation.json]({(out / 'invocation.json').resolve()})。",
        "",
    ...
    for category in CATEGORIES:
        cells = [f"{pct(avg_category(data, category, replacement)):.2f}%" for replacement in REPLACEMENTS]
        lines.append(f"| {CATEGORY_LABELS[category]} | " + " | ".join(cells) + " |")
    lines += [
        "",
        f"同一文化的 native 对照为 {pct(native):.2f}%；不替换时 donor 标签不参与消息构造，所有 donor 副本完全一致。不同文化只替换一个 sender 时仍保留约 8.87%–12.84% 的联合 J，全部 sender 替换后只剩 0.62%–1.21%，接近有限任务的低基线。literal 与 equivariant 的差异并不能解释这个下降：问题是 donor 的 token—receiver 码本不兼容，而不是资源轴是否同步。",
        "",
        "## 结果二：替换规模揭示组合解码的脆弱性",
        "",
        "`all` 替换的 schedule-A、role-012 兼容性矩阵如下。这是一个固定诊断切片，完整平均结果见上一表和图。对角线是同一文化，非对角线是独立文化。",
        "",
        "| recipient \\ donor | " + " | ".join(LABELS[c] for c in CULTURES) + " |",
        "|---|" + "---:|" * len(CULTURES),
    ]
    for recipient in CULTURES:
        vals = [data["summary"][recipient][donor]["all"]["A"]["012"]["equivariant_target60_J"]["mean"] for donor in CULTURES]
        lines.append("| " + LABELS[recipient] + " | " + " | ".join(f"{pct(v):.1f}%" for v in vals) + " |")
    lines += [
        "",
    ...
    for category in CATEGORIES:
        pairs = []
        for recipient in CULTURES:
            for donor in CULTURES:
                rm, rc = recipient.split("__", 1); dm, dc = donor.split("__", 1)
                if recipient == donor: c = "same_culture"
                elif rm == dm: c = "same_role_mode_different_topology"
                elif rc == dc: c = "different_role_mode_same_topology"
                else: c = "different_role_mode_and_topology"
                if c == category:
                    pairs.append(data["token_summary"][recipient + "__" + donor]["pair"]["mean"])
        lines.append(f"| {CATEGORY_LABELS[category]} | {pct(float(np.mean(pairs))):.2f}% |" )
    lines += [
        "",
        ...
        "## 可复核性",
        "",
        f"独立 NumPy 分析：[partner_transfer_analysis.json]({(out / 'partner_transfer_analysis.json').resolve()})，完成 {data['checks']:,} 项检查和 {data['scalar_comparisons']:,} 个标量比较，native v0.43 provenance mismatch=0，最大保存指标差异 {data['maximum_metric_absolute_difference']:.2e}。",
        f"独立动作/分数审计：[partner_transfer_audit.json]({(out / 'partner_transfer_audit.json').resolve()})，覆盖 {audit['groups']} 个视觉组、{audit['coverage']['score_cells']:,} 个 score cell，重算最大误差 {audit['maximum_replay_absolute_difference']:.1e}；审计未导入生产模型。",
        f"图形：[01_partner_transfer.png]({(out / 'figures' / '01_partner_transfer.png').resolve()})；视觉 QA：[visual_qa.json]({(out / 'visual_qa.json').resolve()})；审查：[结果审查.json]({(ROOT / '结果审查.json').resolve()})。",
        "",
