"""One fixed calibration scalar per frozen reset encoder; same v14 social experiment."""
...
sys.path.insert(0,str(PROJECT/'redesign_v0.14'))
import run_reset as v14
sys.path.insert(0,str(ROOT))
import scaled_interface as scaled
v13=v14.v13;v11=v14.v11;v10=v14.v10;v9=v14.v9;v8=v14.v8
ImageBank=v14.ImageBank;remake_agents=v14.remake_agents;projected_banks=v14.projected_banks;write_json=v14.write_json
select_parameters=v14.select_parameters;update_agents=v14.update_agents;worlds=v14.worlds;evaluate=v14.evaluate;protocol=v14.protocol
TIMES=v14.TIMES;SEEDS=v14.SEEDS
DEFAULT_SOURCE=PROJECT/'redesign_v0.13/results/spatial_001'
DEFAULT_RESET=PROJECT/'redesign_v0.14/results/reset_001'

def read(p):return json.loads(Path(p).read_text())
def sources():
    return list(dict.fromkeys([Path(__file__).resolve(),ROOT/'scaled_interface.py',ROOT/'固定执行方案.md',ROOT/'前置科学审查.md',ROOT/'前置审查.md',*v14.sources()]))

def source_receipt(args):
    retained=v14.source_receipt(args);inv=read(args.reset_batch/'invocation.json');done=read(args.reset_batch/'training_complete.json');assert done['status']=='complete'
    paths=[args.reset_batch/'invocation.json',args.reset_batch/'training_complete.json',args.reset_batch/'source_receipt.json']
    for seed in args.seeds:
        paths.append(args.reset_batch/f'prepared_{seed}.pt')
        for p in args.partitions:paths.extend(x for x in (args.reset_batch/f'social_s{seed}_p{p}_reset').iterdir() if x.is_file())
    if not args.smoke:
        paths.extend(args.reset_batch/n for n in ('audit_execution.json','independent_recount.json','audit_report_review.json','私人非语言经验的迁移研究报告.md'))
        manifest=read(args.reset_batch/'completion_manifest.json');assert manifest['status']=='complete'
        for path in paths:assert v8.sha(path)==manifest['artifacts'][str(path.relative_to(PROJECT))],str(path)
        assert read(args.reset_batch/'audit_execution.json')['passed'];paths.append(args.reset_batch/'completion_manifest.json')
    for path,digest in inv['source_hashes'].items():assert v8.sha(path)==digest,path
    return dict(retained=retained,reset=dict(source_batch=str(args.reset_batch),source_training_hashes=inv['source_hashes'],files={str(p):v8.sha(p) for p in paths}),
        reused_references='Both retained and reset are previously observed development evidence; no new independent source')

def calibration_receipt(args,hashes):
    path=args.calibration/'calibration_complete.json';done=read(path)
    assert done['source_hashes']==hashes and done['seeds']==args.seeds and done['partitions']==args.partitions
    assert done['status']=='complete' and done['persons']==len(args.seeds)*len(args.partitions)*2
    for name,digest in done['files'].items():assert v8.sha(args.calibration/name)==digest,name
    assert read(args.calibration/'source_receipt.json')==source_receipt(args)
    return dict(path=str(path),sha256=v8.sha(path),files=done['files'],scope='Fixed train-only calibration; no outcome-dependent scaling')
...
def fit(seed,p,arm,prepared,bank,args,hashes):
    ...
    assert arm in ('unit_replay','reset_scaled')
    scales=[1. if arm=='unit_replay' else float(q['alpha_float32']) for q in cal['persons']]
    agents=remake_agents(seed,prepared,7,2,'identity')
    for who,a in enumerate(agents):
        a.load_state_dict(base[who]);scaled.attach_scale(a,scales[who])
    ...
    source_inputs=dict(reset_initial=dict(path=str(reset),sha256=v8.sha(reset)),
        ...
        intervention='single fixed positive scalar after observe; all base tensors equal reset')
    selected=select_parameters(agents);frozen=v9.state_subset(agents)
    for a,state in zip(agents,frozen):state['visual_scale']=a.visual_scale.clone()
    ...
    cfg=dict(seed=seed,partition=p,arm=arm,updates=args.social_updates,batch=args.batch,eval_n=args.eval_n,
        ...
        source_checkpoint=dict(path=str(source),sha256=v8.sha(source)),source_inputs=source_inputs,scales_float32=scales,initial_sha256=initial,
...
def main():
    ...
    ap.add_argument('--out',type=Path,default=ROOT/'results/scaled_001')
    ap.add_argument('--source-batch',type=Path,default=DEFAULT_SOURCE)
    ap.add_argument('--reset-batch',type=Path,default=DEFAULT_RESET)
    ap.add_argument('--calibration',type=Path,default=ROOT/'calibration_001')
    ...
    ap.add_argument('--arms',nargs='+',default=['reset_scaled']);ap.add_argument('--smoke',action='store_true');args=ap.parse_args()
    torch.set_num_threads(1)
    for key in ('out','source_batch','reset_batch','calibration'):setattr(args,key,getattr(args,key).resolve())
    assert args.batch%2==0 and args.eval_n%120==0 and args.social_updates>0
    assert len(set(args.arms))==len(args.arms) and set(args.arms)<= {'unit_replay','reset_scaled'}
    hashes={str(p):v8.sha(p) for p in sources()};receipt=source_receipt(args)
    if not args.smoke:
        assert (args.source_batch,args.reset_batch,args.seeds,args.partitions,args.social_updates,args.batch,args.eval_n,args.arms)==(DEFAULT_SOURCE,DEFAULT_RESET,SEEDS,[1,2,3],2400,512,9600,['reset_scaled'])
    creceipt=None if args.calibrate_only else calibration_receipt(args,hashes)
    if not args.smoke and not args.calibrate_only:
    ...
    if not args.smoke and not args.calibrate_only:shutil.copy2(ROOT/'preflight_qa.json',snap/'preflight_qa.json')
    write_json(args.out/'source_receipt.json',receipt)
    if creceipt:write_json(args.out/'calibration_receipt.json',creceipt)
    write_json(args.out/'invocation.json',dict(started_utc=datetime.now(timezone.utc).isoformat(),formal=not args.smoke,
        phase='calibration' if args.calibrate_only else 'social',args={k:str(v) if isinstance(v,Path) else v for k,v in vars(args).items()},
        arms=args.arms,source_hashes=hashes,python=platform.python_version(),torch=str(torch.__version__),device='cpu'))
    bank=ImageBank()
    for seed in args.seeds:
        ...
        for p in args.partitions:
            if args.calibrate_only:
                r=args.source_batch/f'social_s{seed}_p{p}_control/initial.pt';z=args.reset_batch/f'social_s{seed}_p{p}_reset/initial.pt'
                scaled.calibrate_pair(seed,p,prepared,bank,torch.load(r,weights_only=True),torch.load(z,weights_only=True),args.out/f's{seed}_p{p}',source_hashes={**hashes,str(source):v8.sha(source),str(r):v8.sha(r),str(z):v8.sha(z)})
                print(json.dumps(dict(calibrated_seed=seed,partition=p,persons=2)),flush=True)
            else:
                for arm in args.arms:fit(seed,p,arm,prepared,bank,args,hashes)
    if args.calibrate_only:
        files={str(p.relative_to(args.out)):v8.sha(p) for p in args.out.glob('s*_p*/*') if p.is_file()}
        write_json(args.out/'calibration_complete.json',dict(status='complete',seeds=args.seeds,partitions=args.partitions,persons=2*len(args.seeds)*len(args.partitions),
            source_hashes=hashes,files=files,source_receipt_sha256=v8.sha(args.out/'source_receipt.json'),finished_utc=datetime.now(timezone.utc).isoformat(),train_only=True,training_updates=0))
