...
def main():
    ...
    text=['# 实际通信消息：固定展示样本','',
        f'固定第一来源{seed}、坐标重复p{p}、发送方向{d}、终点{t}，共同target12全部地图，每种mask均取字典序第一测试照片对。未按成功与否挑选。',
    ...
    for arm in support.ARMS:
        ...
        raw=dict(np.load(path));acts=raw['receiver_logits'].argmax(-1);ids=support.groups(p,arm)['target12']
        for mask in (0,1):
            for mid in ids:
                ...
                label='每目标1条三步路径' if arm=='aligned_paths1' else '仅3/12目标有三步路径'
                text.append(f"| {label} | {target[0]}/{target[1]} | {'食物' if mask==0 else '水'} | `{' '.join(SYMBOLS[x] for x in tok)}` | {action[0]}/{action[1]} | {'是' if correct else '否'} |")
