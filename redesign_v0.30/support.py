...
def paired_map_order(seed,p,d,step,arm):
    """Return a balanced map sequence with common-arm worlds row-paired.

    The seven shared maps occupy the same 70 base-world slots in both arms;
    the five arm-specific maps fill the remaining slots.  This preserves the
    intended 140/240 shared-world comparison while keeping the supports
    ...
    pool=ordered_pool(p,arm);common=np.intersect1d(pool,ordered_pool(p,other));unique=np.setdiff1d(pool,common)
    slot_order=rng(0).permutation(BASE_BATCH);common_slots=np.sort(slot_order[:len(common)*10])
    values=np.empty(BASE_BATCH,dtype=np.int64)
    common_values=np.repeat(common,10);values[common_slots]=common_values[rng(3).permutation(len(common_values))]
    unique_values=np.repeat(unique,10);values[np.setdiff1d(np.arange(BASE_BATCH),common_slots)]=unique_values[rng(4).permutation(len(unique_values))]
    return values
...
def fixture(seed,p,d,step,arm,train_table):
    ...
    rng=lambda stream: np.random.default_rng(np.random.SeedSequence([NAMESPACE,int(seed),int(p),int(d),int(step),int(stream)]))
    maps=paired_map_order(seed,p,d,step,arm)
