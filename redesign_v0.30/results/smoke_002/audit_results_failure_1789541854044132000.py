...
def check_fixture(seed,p,step,arm,w,raw,log,cache,c):
    ...
    for d in (0,1):
        ...
        shared=np.isin(w['map_id'][idx],np.intersect1d(stats.groups(p,arm)['train12'],stats.groups(p,other_arm)['train12']))
        c.exact(int(shared.sum()),140,'shared actual worlds140/240')
