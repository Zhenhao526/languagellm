...
def fixture(seed,p,d,step,arm):
    groups=stats.groups(p,arm);shared=groups['shared_train9'];unique=np.setdiff1d(groups['train12'],shared)
    rng=lambda stream:np.random.default_rng(np.random.SeedSequence([30030,seed,p,d,step,stream]))
    maps=np.tile(np.r_[shared,unique],10)[rng(0).permutation(120)]
...
def check_fixture(seed,p,step,arm,w,raw,log,cache,c):
    ...
    for d in (0,1):
        ...
        shared=np.isin(w['map_id'][idx],stats.groups(p,arm)['shared_train9'])
        c.exact(int(shared.sum()),180,'shared actual worlds180/240')
