...
ALIGNED=((0,2),(0,5),(1,0),(1,3),(2,1),(2,4),(3,1),(3,5),(4,2),(4,3),(5,0),(5,4))
SPARSE=((0,4),(0,5),(1,0),(1,5),(2,1),(2,3),(3,0),(3,1),(4,2),(4,3),(5,2),(5,4))
...
def groups(panel,arm):
    ...
    target={index[perm[i],perm[j]] for i,j in TARGET}
    selected=ALIGNED if arm=='aligned_paths1' else SPARSE
    train={index[perm[i],perm[j]] for i,j in selected}
    return {k:np.array(sorted(v),np.int64) for k,v in dict(train12=train,target12=target,
        held18=set(range(30))-train,common30=set(range(30))).items()}
...
def partner_pairs(raw,p,arm):
    """Both target edges for each resource/location/photo/mask group."""
    target=np.flatnonzero(np.isin(raw['map_id'],groups(p,arm)['target12']))
    code=raw['tokens'][:,0]*7+raw['tokens'][:,1]
    action=raw['receiver_logits'].argmax(-1)[code]
    correct=np.all(action==raw['positions'],axis=-1);out={}
        ...
        for loc in range(6):
            ...
            for photo in np.unique(raw['photo_ids'][rows0],axis=0):
                for shown in (0,1):
                    ...
                    if len(rows)!=2:raise ValueError('target partner group is not exactly two')
...
def score(raw,p,arm):
    ...
    rec=recombination(raw,p,arm);pairs=partner_pairs(raw,p,arm);result={}
...
def analyze(out,c):
    ...
    for seed in seeds:
        ...
        primary.append(dict(seed=seed,aligned=extract(a),sparse=extract(b),difference=extract(a)-extract(b),
