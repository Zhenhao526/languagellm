...
def fixture(seed,p,d,step,arm):
    ...
    other=stats.ARMS[1-stats.ARMS.index(arm)];common=np.intersect1d(groups['train12'],stats.groups(p,other)['train12']);unique=np.setdiff1d(groups['train12'],common)
    slots=rng(0).permutation(120);common_slots=np.sort(slots[:len(common)*10]);maps=np.empty(120,dtype=np.int64)
    common_values=np.repeat(common,10);maps[common_slots]=common_values[rng(3).permutation(len(common_values))]
    rest=np.setdiff1d(np.arange(120),common_slots);unique_values=np.repeat(unique,10);maps[rest]=unique_values[rng(4).permutation(len(unique_values))]
...
def check_fixture(seed,p,step,arm,w,raw,log,cache,c):
    ...
    for d in (0,1):
        ...
        shared_set=set(int(x) for x in shared_maps)
        left_rows=np.flatnonzero(np.isin(left[:,0],shared_maps));right_rows=np.flatnonzero(np.isin(right[:,0],shared_maps))
        c.exact(len(left_rows),140,'shared actual worlds140/240');c.exact(len(right_rows),140,'shared comparison worlds140/240')
        # Sort the multisets by full world identity; duplicate photo draws are
        # expected, so a dictionary would silently discard valid observations.
        lo=np.lexsort((left[left_rows,3],left[left_rows,2],left[left_rows,1],left[left_rows,0]))
        ro=np.lexsort((right[right_rows,3],right[right_rows,2],right[right_rows,1],right[right_rows,0]))
        c.exact(left[left_rows][lo],right[right_rows][ro],'shared world identities paired')
        c.exact(w['positions'][idx[left_rows][lo]],w['positions'][other['indices'][right_rows][ro]],'shared positions paired')
