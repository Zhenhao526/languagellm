...
from .run import DEFAULT_MODEL, inference_seed, write_json
...
def main():
    ...
    write_json(out_dir/'manifest.json', {'case_files':[str(p) for p in files], 'arms':['original','empty_text'],
    ...
    backend = Backend(DEFAULT_MODEL,out_dir/'inference.jsonl')
    results = []
    for path in files:
