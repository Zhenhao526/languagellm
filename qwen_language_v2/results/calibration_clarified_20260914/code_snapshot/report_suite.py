"""Read-only aggregation of v0.2 runs into a Chinese Markdown/JSON report.

Usage: python3 -m qwen_language_v2.report_suite RUN_DIR ... --out REPORT_DIR
...
def summarize_run(run_dir):
    ...
    for key, case in sorted(cases.items()):
        ...
        if complete is None and recorded_steps:
            row["record_status"] = "进行中或未写入完成记录"
        elif complete is None:
            row["record_status"] = "尚无执行记录"
        else:
...
def render_report(suite):
    ...
    lines += ["## 动作与实际发生的过程", "", "| 运行 | 条件/群体/段 | 等待 | 失败的非等待动作 | 曾携带物品 | 加工成功 | 交付被接收 | 新交付单位 |",
        "|---|---|---:|---:|---|---|---|---:|"]
    for run in runs:
        for row in run["episodes"]:
            ...
            lines.append(f"| {_cell(run['name'])} | {row['condition']}/{row['group']}/{row['episode']} | "
                ...
                f"{_yes(actions['processing_observed'])} | {_yes(actions['accepted_delivery_observed'])} | "
                f"{_fmt(actions['newly_delivered_units_from_states'], 0)} |")
    lines += ["", "等待单列，不因未交付而把等待记成执行失败。失败只计明确反馈action_succeeded=false的非等待动作；缺失反馈不算失败。共同交付可产生两人的成功动作记录，但交付单位从状态变化计算，不重复算两份。未完成段的动作也按已有记录展示。", ""]
    for run in runs:
