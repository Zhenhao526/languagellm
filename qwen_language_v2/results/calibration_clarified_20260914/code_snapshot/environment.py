...
AGENTS = ("A", "B", "C")
LOCATIONS = ("营地", "林地", "河滩")
RULES = """你们三人共同在12个物理动作步内完成营地任务板的两项交付需求。地图上营地、林地、河滩两两有道路，正常移动耗费一步。任务板只在营地可读；你只观察所在地的物品、同地伙伴与自己的携带物。其他地点当前情况不会自动共享。每人每步只能选一个菜单动作，三人的动作同时执行。每人有一个普通携带位和一个斧具位。原木必须由持斧具者在原地加工后才能交付。所有纤维无论长短、干湿都可单人拿取；短木材加工后也可单人拿取。只有长木材加工后需要两人选择相互匹配的物品、搭档、目的地才能共同移动，共同负载不能单人移动或卸下。共同负载需要两人同时选择共同交付或共同卸下；共同交付也可直接处理所在地的已加工长木材。普通交接需要接收者同地并选择等待，双方各占用本步动作。单人交付、共同交付均只能在需求指定地点被接收；若同一步开始时某项需求尚未满足，同一步提交的全部匹配物品均可接收，但计分最多达到所需数量；之后的动作步不再接收该项超额物品。同一物品只能计分一次。错误尝试耗费一步，不会给出正确目标。菜单只列物理上可以尝试的动作，不表示它能满足目标。道路可能在局部发生变化，远端变化不一定从这里可见，尝试通行时才可能发现。所有物品操作编号只属于你，伙伴的同一物品编号不同。其他主体未执行的动作和私有反馈不公开。分数取决于按时满足目标的比例，同完成度下越早完成越好。"""
